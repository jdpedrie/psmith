...
- **Complete test coverage on everything written — Swift / macOS client.** Same rule, two layers: every new public method on a `ClarkKit` Repository or ViewModel gets a Layer 1 integration test that drives it against a fresh local clarkd (`clients/ClarkSwift/Tests/ClarkKitTests/`). Every new SwiftUI view (or non-trivial state of an existing one) that's load-bearing for the user gets a Layer 2 snapshot test (`clients/clarkd-mac/Tests/ClarkMacSnapshotTests/`). The full plan, test inventory, and harness design lives in [docs/testing-plan.md](docs/testing-plan.md). Don't merge a Swift change without tests; layout regressions and "save doesn't actually save" bugs have eaten substantial time and tests are how we stop that. `make swift-test` runs both layers.
