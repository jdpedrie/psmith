...
Self-hosted AI chat orchestrator. Architecture and design decisions live in the documentation tree under [docs/](docs/README.md) — start with [docs/design/overview.md](docs/design/overview.md) and read it before working in this repo.

**Deferred work — tactical TODOs from in-flight implementation — lives in [docs/todo.md](docs/todo.md).** Update it whenever you defer something or complete an item there. Strategic deferrals (encryption tiers, sharing model, transform pipeline, etc.) live in the design docs under their relevant subsystem; the todo doc captures the implementation-level loose ends that would otherwise be lost across compactions.
...
- **Complete test coverage on everything written — Swift / macOS client.** Same rule, two layers: every new public method on a `PsmithKit` Repository or ViewModel gets a Layer 1 integration test that drives it against a fresh local psmithd (`clients/PsmithSwift/Tests/PsmithKitTests/`). Every new SwiftUI view (or non-trivial state of an existing one) that's load-bearing for the user gets a Layer 2 snapshot test (`clients/psmithd-mac/Tests/PsmithMacSnapshotTests/`). The full plan, test inventory, and harness design lives in [docs/testing-plan.md](docs/testing-plan.md). Don't merge a Swift change without tests; layout regressions and "save doesn't actually save" bugs have eaten substantial time and tests are how we stop that. `make swift-test` runs both layers.
