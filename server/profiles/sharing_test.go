...
// TestShippedBundlesImport guards the artifacts in docs/profiles against the
// format drifting out from under them.
//
// They are generated by scripts/gen-profile-bundles and committed, so a change
// to the bundle shape that nobody regenerates would leave a broken file in the
// tree that only fails when someone tries to use it. This imports each one for
// real and checks the chain arrives intact.
func TestShippedBundlesImport(t *testing.T) {
	t.Parallel()

	entries, err := os.ReadDir("../../docs/profiles")
	if err != nil {
		t.Fatalf("read docs/profiles: %v", err)
	}
	var bundles []string
	for _, e := range entries {
		if strings.HasSuffix(e.Name(), ".psmithprofile") {
			bundles = append(bundles, e.Name())
		}
	}
	if len(bundles) == 0 {
		t.Fatal("no bundles found in docs/profiles; the glob is wrong")
	}

	for _, name := range bundles {
		t.Run(name, func(t *testing.T) {
			svc, q := newTestSvc(t)
			u := mustCreateUser(t, q, "shipped-"+strings.TrimSuffix(name, ".psmithprofile"))

			payload, err := os.ReadFile(filepath.Join("../../docs/profiles", name))
			if err != nil {
			...
			if len(resp.Msg.Profiles) == 0 {
				t.Fatal("bundle imported zero profiles")
			}
			// A shipped bundle should land clean on a fresh account. Anything
			// here means it carries a reference the recipient cannot resolve,
			// which for a file we hand out is a bug in the file.
			for _, w := range resp.Msg.Warnings {
				t.Errorf("unexpected warning: %s", w.Message)
			}

			// Every non-root profile must have been rewired onto a row from
			// this same import, never left dangling.
			...
			for _, p := range resp.Msg.Profiles {
				...
				if !byID[*p.ParentProfileId] {
					t.Errorf("%q points at a parent outside the bundle", p.Name)
				}
			}
			if roots != 1 {
				t.Errorf("expected exactly one root profile, got %d", roots)
			}

			// The plugin attaches to the base and the children inherit it, so
			// exactly one row should exist across the whole chain.
			attached := 0
			for _, p := range resp.Msg.Profiles {
			...
			if attached == 0 {
				t.Error("no plugin rows survived the import")
			}
		})
	}
}
