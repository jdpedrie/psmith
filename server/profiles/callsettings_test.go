...
func tier(v psmithv1.ServiceTier) *psmithv1.ServiceTier {
	return &v
}
func harm(v psmithv1.HarmThreshold) *psmithv1.HarmThreshold {
	return &v
}
...
func TestMergeCallSettings_OnlyHigher(t *testing.T) {
	in := &psmithv1.CallSettings{Temperature: f64(0.5)}
...
func TestMergeCallSettings_OnlyLower(t *testing.T) {
	in := &psmithv1.CallSettings{TopP: f64(0.95)}
...
func TestMergeCallSettings_StopSequencesWholeFieldOverride(t *testing.T) {
	higher := &psmithv1.CallSettings{StopSequences: []string{"X", "Y"}}
	lower := &psmithv1.CallSettings{StopSequences: []string{"A", "B", "C"}}
	...
	higher2 := &psmithv1.CallSettings{Temperature: f64(0.1)}
...
func TestMergeCallSettings_PrecedenceMultipleLayers(t *testing.T) {
	...
	top := &psmithv1.CallSettings{Temperature: f64(0.7)} // wins on temp
	middle := &psmithv1.CallSettings{
		Temperature:     f64(0.4),  // shadowed by top
		MaxOutputTokens: i32(2048), // wins
	}
	bottom := &psmithv1.CallSettings{
...
func TestMergeCallSettings_ThinkingPerFieldMerge(t *testing.T) {
	...
	higher := &psmithv1.CallSettings{Thinking: &psmithv1.ThinkingSettings{Enabled: bp(true)}}
	lower := &psmithv1.CallSettings{Thinking: &psmithv1.ThinkingSettings{BudgetTokens: i32(4000)}}
	got := MergeCallSettings(higher, lower)
...
func TestMergeCallSettings_ThinkingHigherOverridesLower(t *testing.T) {
	higher := &psmithv1.CallSettings{Thinking: &psmithv1.ThinkingSettings{
		Enabled:      bp(false),
		BudgetTokens: i32(1000),
	}}
	lower := &psmithv1.CallSettings{Thinking: &psmithv1.ThinkingSettings{
		Enabled:      bp(true),
...
func TestMergeCallSettings_OpenAIPerFieldMerge(t *testing.T) {
	...
	higher := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{Seed: i32(42)}}
	lower := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{
...
func TestMergeCallSettings_OpenAIServiceTierEnumPickWins(t *testing.T) {
	higher := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{ServiceTier: tier(psmithv1.ServiceTier_SERVICE_TIER_PRIORITY)}}
	lower := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{ServiceTier: tier(psmithv1.ServiceTier_SERVICE_TIER_AUTO)}}
	got := MergeCallSettings(higher, lower)
	if got.GetOpenai().GetServiceTier() != psmithv1.ServiceTier_SERVICE_TIER_PRIORITY {
		t.Errorf("service_tier: %v want PRIORITY (higher wins)", got.GetOpenai().GetServiceTier())
	}

	higher2 := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{Seed: i32(1)}}
	got2 := MergeCallSettings(higher2, lower)
	if got2.GetOpenai().GetServiceTier() != psmithv1.ServiceTier_SERVICE_TIER_AUTO {
...
func TestMergeCallSettings_OpenAILogitBiasWholeFieldOverride(t *testing.T) {
	higher := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{LogitBias: map[int32]float64{100: 1.5}}}
	lower := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{LogitBias: map[int32]float64{200: -2.0, 300: 0.5}}}
...
func TestMergeCallSettings_OpenAIResponseFormatWholeFieldOverride(t *testing.T) {
	higher := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{
		ResponseFormat: &psmithv1.ResponseFormat{
			Kind: &psmithv1.ResponseFormat_JsonObject{JsonObject: true},
		},
	}}
	lower := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{
		ResponseFormat: &psmithv1.ResponseFormat{
			Kind: &psmithv1.ResponseFormat_JsonSchema{JsonSchema: &psmithv1.JsonSchema{
				Name: "answer", Schema: []byte(`{}`),
			}},
		},
	}}
	got := MergeCallSettings(higher, lower)
	if _, ok := got.GetOpenai().ResponseFormat.GetKind().(*psmithv1.ResponseFormat_JsonObject); !ok {
		t.Errorf("response_format: %T want JsonObject", got.GetOpenai().ResponseFormat.GetKind())
	}

	higher2 := &psmithv1.CallSettings{Openai: &psmithv1.OpenAIExtras{Seed: i32(1)}}
	got2 := MergeCallSettings(higher2, lower)
	js, ok := got2.GetOpenai().ResponseFormat.GetKind().(*psmithv1.ResponseFormat_JsonSchema)
	if !ok {
...
func TestMergeCallSettings_GooglePerFieldMerge(t *testing.T) {
	higher := &psmithv1.CallSettings{Google: &psmithv1.GoogleExtras{
		ResponseMimeType: sp("application/json"),
	}}
	lower := &psmithv1.CallSettings{Google: &psmithv1.GoogleExtras{
...
func TestMergeCallSettings_SafetySettingsPerFieldMerge(t *testing.T) {
	higher := &psmithv1.CallSettings{Google: &psmithv1.GoogleExtras{
		SafetySettings: &psmithv1.SafetySettings{
			Harassment: harm(psmithv1.HarmThreshold_HARM_THRESHOLD_BLOCK_NONE),
		},
	}}
	lower := &psmithv1.CallSettings{Google: &psmithv1.GoogleExtras{
		SafetySettings: &psmithv1.SafetySettings{
			Harassment:       harm(psmithv1.HarmThreshold_HARM_THRESHOLD_BLOCK_ONLY_HIGH),
			DangerousContent: harm(psmithv1.HarmThreshold_HARM_THRESHOLD_BLOCK_LOW_AND_ABOVE),
		},
	}}
	got := MergeCallSettings(higher, lower)
	ss := got.GetGoogle().GetSafetySettings()
	if ss.GetHarassment() != psmithv1.HarmThreshold_HARM_THRESHOLD_BLOCK_NONE {
		t.Errorf("harassment: %v want BLOCK_NONE (higher wins)", ss.GetHarassment())
	}
	if ss.GetDangerousContent() != psmithv1.HarmThreshold_HARM_THRESHOLD_BLOCK_LOW_AND_ABOVE {
...
func TestMergeCallSettings_AnthropicExtrasPresenceOnly(t *testing.T) {
	higher := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{}}
	got := MergeCallSettings(higher, nil)
	if got.GetAnthropic() == nil {
		t.Error("expected anthropic block to survive merge")
	}
	got2 := MergeCallSettings(nil, &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{}})
	if got2.GetAnthropic() == nil {
		t.Error("expected anthropic block to survive merge from lower")
	}
}

func ttl(v psmithv1.CacheTTL) *psmithv1.CacheTTL { return &v }

func TestMergeCallSettings_AnthropicCacheEnabled_HigherWins(t *testing.T) {
	higher := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheEnabled: bp(false)}}
	lower := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheEnabled: bp(true)}}
...
func TestMergeCallSettings_AnthropicCacheEnabled_FillsFromLower(t *testing.T) {
	higher := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{}}
	lower := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheEnabled: bp(false)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil || got.CacheEnabled == nil || *got.CacheEnabled != false {
...
func TestMergeCallSettings_AnthropicCacheTTL_HigherWins(t *testing.T) {
	higher := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheTtl: ttl(psmithv1.CacheTTL_CACHE_TTL_1H)}}
	lower := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheTtl: ttl(psmithv1.CacheTTL_CACHE_TTL_5M)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil || got.CacheTtl == nil || *got.CacheTtl != psmithv1.CacheTTL_CACHE_TTL_1H {
		t.Errorf("expected ttl=1h from higher, got %+v", got)
	}
}

func TestMergeCallSettings_AnthropicCacheTTL_FillsFromLower(t *testing.T) {
	higher := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheEnabled: bp(true)}}
	lower := &psmithv1.CallSettings{Anthropic: &psmithv1.AnthropicExtras{CacheTtl: ttl(psmithv1.CacheTTL_CACHE_TTL_1H)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil {
	...
	if got.CacheTtl == nil || *got.CacheTtl != psmithv1.CacheTTL_CACHE_TTL_1H {
...
func TestCallSettingsCodec_RoundTrip(t *testing.T) {
	in := &psmithv1.CallSettings{
		...
		Openai: &psmithv1.OpenAIExtras{
			Seed:        i32(7),
			ServiceTier: tier(psmithv1.ServiceTier_SERVICE_TIER_PRIORITY),
	...
	if out.GetOpenai().GetSeed() != 7 || out.GetOpenai().GetServiceTier() != psmithv1.ServiceTier_SERVICE_TIER_PRIORITY {
