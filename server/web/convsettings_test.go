...
func TestMergeCall_HigherWins(t *testing.T) {
	t.Parallel()
	hi, lo := 0.2, 0.9
	out := mergeCall(&psmithv1.CallSettings{Temperature: &hi}, &psmithv1.CallSettings{Temperature: &lo, TopP: &lo})
...
func TestConvSettings_PluginOverride(t *testing.T) {
	...
	got, _ := convos.GetConversationPlugins(userCtx, connect.NewRequest(&psmithv1.GetConversationPluginsRequest{ConversationId: fx.convID.String()}))
	...
	got2, _ := convos.GetConversationPlugins(userCtx, connect.NewRequest(&psmithv1.GetConversationPluginsRequest{ConversationId: fx.convID.String()}))
