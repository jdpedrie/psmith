...
func fillCallFields(vm *convSettingsVM, own, inh *psmithv1.CallSettings, ownConv, inhConv *psmithv1.ConversationSettings) {
...
func thinkEnabled(t *psmithv1.ThinkingSettings) *bool {
	if t == nil {
		return nil
	}
	return t.Enabled
}
func explicitCache(cs *psmithv1.CallSettings) *bool {
	if cs == nil {
		return nil
	}
	return cs.ExplicitCache
}
func includeThinking(s *psmithv1.ConversationSettings) *bool {
	if s == nil {
		return nil
	}
	return s.IncludeThinkingInHistory
}
func anthCache(a *psmithv1.AnthropicExtras) *bool {
	if a == nil {
		return nil
	}
	return a.CacheEnabled
}
func oaiParallel(o *psmithv1.OpenAIExtras) *bool {
	if o == nil {
		return nil
	}
...
func (h *Handler) inheritedCallSettings(ctx context.Context, conv *psmithv1.Conversation) *psmithv1.CallSettings {
	out := &psmithv1.CallSettings{}
	providerID, modelID := resolvedModel(conv)

	if providerID != "" && h.models != nil {
		if resp, err := h.models.ListAllUserModels(ctx, connect.NewRequest(&psmithv1.ListAllUserModelsRequest{})); err == nil {
	...
	if h.profiles != nil {
		if prof, err := h.profiles.GetProfile(ctx, connect.NewRequest(&psmithv1.GetProfileRequest{Id: conv.GetProfileId(), Resolve: true})); err == nil {
...
func (h *Handler) inheritedConvSettings(ctx context.Context, conv *psmithv1.Conversation) *psmithv1.ConversationSettings {
	out := &psmithv1.ConversationSettings{}
	if h.profiles != nil {
		if prof, err := h.profiles.GetProfile(ctx, connect.NewRequest(&psmithv1.GetProfileRequest{Id: conv.GetProfileId(), Resolve: true})); err == nil {
...
func resolvedModel(conv *psmithv1.Conversation) (providerID, modelID string) {
...
func (h *Handler) activeDriverType(ctx context.Context, conv *psmithv1.Conversation) string {
...
func mergeCall(higher, lower *psmithv1.CallSettings) *psmithv1.CallSettings {
	if higher == nil {
...
func mergeThinking(h, l *psmithv1.ThinkingSettings) *psmithv1.ThinkingSettings {
	if h == nil && l == nil {
		return nil
	}
	if h == nil {
		h = &psmithv1.ThinkingSettings{}
	}
	if l == nil {
		l = &psmithv1.ThinkingSettings{}
	}
	return &psmithv1.ThinkingSettings{Enabled: pickB(h.Enabled, l.Enabled), BudgetTokens: pickI(h.BudgetTokens, l.BudgetTokens)}
}

func mergeAnth(h, l *psmithv1.AnthropicExtras) *psmithv1.AnthropicExtras {
	if h == nil && l == nil {
		return nil
	}
	if h == nil {
		h = &psmithv1.AnthropicExtras{}
	}
	if l == nil {
		l = &psmithv1.AnthropicExtras{}
	}
	out := &psmithv1.AnthropicExtras{CacheEnabled: pickB(h.CacheEnabled, l.CacheEnabled)}
	if h.CacheTtl != nil {
...
func mergeOAI(h, l *psmithv1.OpenAIExtras) *psmithv1.OpenAIExtras {
	if h == nil && l == nil {
		return nil
	}
	if h == nil {
		h = &psmithv1.OpenAIExtras{}
	}
	if l == nil {
		l = &psmithv1.OpenAIExtras{}
	}
	return &psmithv1.OpenAIExtras{
...
func mergeGoog(h, l *psmithv1.GoogleExtras) *psmithv1.GoogleExtras {
	if h == nil && l == nil {
		return nil
	}
	if h == nil {
		h = &psmithv1.GoogleExtras{}
	}
	if l == nil {
		l = &psmithv1.GoogleExtras{}
	}
	return &psmithv1.GoogleExtras{CandidateCount: pickI(h.CandidateCount, l.CandidateCount)}
}
...
func (h *Handler) handleConvSettingsSave(w http.ResponseWriter, r *http.Request) {
	...
	if en, bud := formTri(r, "thinking_enabled"), formInt(r, "thinking_budget"); en != nil || bud != nil {
		cs.Thinking = &psmithv1.ThinkingSettings{Enabled: en, BudgetTokens: bud}
	}
	if ce := formTri(r, "anth_cache_enabled"); ce != nil {
		cs.Anthropic = &psmithv1.AnthropicExtras{CacheEnabled: ce}
	}
	if seed, fp, pp, par := formInt(r, "oai_seed"), formFloat(r, "oai_frequency_penalty"), formFloat(r, "oai_presence_penalty"), formTri(r, "oai_parallel_tool_calls"); seed != nil || fp != nil || pp != nil || par != nil {
		cs.Openai = &psmithv1.OpenAIExtras{Seed: seed, FrequencyPenalty: fp, PresencePenalty: pp, ParallelToolCalls: par}
	}
	if cc := formInt(r, "goog_candidate_count"); cc != nil {
		cs.Google = &psmithv1.GoogleExtras{CandidateCount: cc}
	}
	settings.CallSettings = cs
	settings.IncludeThinkingInHistory = formTri(r, "include_thinking_in_history")

	if _, err := h.convos.UpdateConversation(r.Context(), connect.NewRequest(&psmithv1.UpdateConversationRequest{Id: convID, Settings: settings})); err != nil {
...
func (h *Handler) convPlugins(ctx context.Context, convID string) (rows []convPluginRowVM, addable []pluginOptVM) {
	pipe, err := h.convos.ResolveConversationPipeline(ctx, connect.NewRequest(&psmithv1.ResolveConversationPipelineRequest{ConversationId: convID}))
	if err != nil {
		return nil, nil
	}
	...
	for _, e := range pipe.Msg.GetEntries() {
		...
		src := "Inherited"
		if e.GetSource() == psmithv1.ResolvedPipelineSource_RESOLVED_PIPELINE_SOURCE_CONVERSATION {
			src = "Override"
		}
	...
	if ov, err := h.convos.GetConversationPlugins(ctx, connect.NewRequest(&psmithv1.GetConversationPluginsRequest{ConversationId: convID})); err == nil {
...
func (h *Handler) handleConvPluginOverride(w http.ResponseWriter, r *http.Request) {
	...
	cur, err := h.convos.GetConversationPlugins(r.Context(), connect.NewRequest(&psmithv1.GetConversationPluginsRequest{ConversationId: convID}))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if _, err := h.convos.SetConversationPlugins(r.Context(), connect.NewRequest(&psmithv1.SetConversationPluginsRequest{
...
func currentConfigFor(plugins []*psmithv1.ConversationPlugin, name string) []byte {
...
func dropPlugin(plugins []*psmithv1.ConversationPlugin, name string) []*psmithv1.ConversationPlugin {
	out := plugins[:0]
	for _, p := range plugins {
		if p.GetPluginName() != name {
			out = append(out, p)
...
func upsertPlugin(plugins []*psmithv1.ConversationPlugin, name string, config []byte, disabled bool) []*psmithv1.ConversationPlugin {
	for _, p := range plugins {
		if p.GetPluginName() == name {
			p.Config = config
			p.Disabled = disabled
	...
	return append(plugins, &psmithv1.ConversationPlugin{PluginName: name, Ordinal: ord, Config: config, Disabled: disabled})
}
