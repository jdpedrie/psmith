...
func TestUpdate_StoresSecretEncryptedInDB(t *testing.T) {
	...
	_, err = svc.UpdateLangfuseConfig(ctxAsUser(user), connect.NewRequest(&psmithv1.UpdateLangfuseConfigRequest{
		Host:      &host,
		PublicKey: &pubKey,
...
func TestGet_NeverEchoesSecret(t *testing.T) {
	...
	pub := "pk"
	enabled := true
	if _, err := svc.UpdateLangfuseConfig(ctxAsUser(user), connect.NewRequest(&psmithv1.UpdateLangfuseConfigRequest{
		Host: &host, PublicKey: &pub, SecretKey: &plaintext, Enabled: &enabled,
	})); err != nil {
		t.Fatalf("Update: %v", err)
	}

	resp, err := svc.GetLangfuseConfig(ctxAsUser(user), connect.NewRequest(&psmithv1.GetLangfuseConfigRequest{}))
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	cfg := resp.Msg.Config
