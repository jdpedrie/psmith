...
| **1a — Backend foundation** | `Storage` interface + filesystem impl, `files` + `message_attachments` tables, `UploadFile` / `GetFileURL` / `ListFiles` RPCs, `WireMessage.Attachments` threaded through `history.Build`, signed-URL HTTP endpoint, cache-observability hash extension, Anthropic image translation. | shipped | — |
| **1b — iOS image input** | Composer paperclip + `PhotosPicker`, shared HEIC→JPEG / 2048-cap / EXIF-strip preprocessing in `ReeveKit`, `FilesRepository`, pending-attachment chips, `MessageAttachmentImage` thumbnails via signed URL. Image-only sends accepted. | shipped | — |
| **1c — Other-provider image translation** | OpenAI Chat (`image_url` data URL), OpenAI Responses (`input_image`), Google Gemini (`inline_data`). Per-driver wire-shape tests. iOS capability gate enables the paperclip on every driver. | shipped | — |
