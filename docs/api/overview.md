...
The server speaks the Connect protocol with a protobuf codec over h2c (cleartext HTTP/2), with TLS terminated by whatever the operator puts in front. The Go server stubs are generated into `gen/reeve/v1/reevev1connect`; the Swift client stubs into `clients/ReeveSwift/Sources/ReeveKit/Generated`. There are three RPC shapes in use: unary (most calls), server-streaming (`SubscribeStream`, `SubscribeAccountEvents`), and client-streaming (`UploadFile`).
...
Reeve uses standard Connect error codes with consistent meanings:
