...
- **Unify special-role message editability across clients.** System, context, and compression messages are deliberately distinct from user/assistant turns, but the clients disagree on HOW distinct: on Mac, system and context messages aren't editable at all; on iOS they are; and compression messages get a different editing affordance than everything else. Decide the editability matrix once (which roles are editable, via which affordance) and make Mac + iOS + web agree.

- **Pending-compression UI gating.** The server already enforces the pending-summary state (Compact and SendMessage both refuse while an un-promoted summary exists), but the clients don't reflect it: the compress button stays enabled and the composer stays live, so both actions fail with server errors instead of being disabled. When a compression summary is pending: disable the compress action, and replace the composer (input box + send button, or the entire bottom bar) with the review actions — Delete and Confirm buttons moved out of the message bubble. The message's "Edit…" affordance should become the normal long-press context-menu edit, not a special control.
