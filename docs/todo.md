...
- **Inverted (bottom-anchored) transcript list — SHIPPED 2026-07-21 (iOS).** The flipped list landed: newest message at content offset 0, entry/pill/deletes exact by construction, send pinned at the rest edge under a runway + stream clip, one exact id-solve at terminal. Both deterministic entry-park repros (fresh-install Scroll XL cold entry; pop-back from Contexts on a 5-message context) verified green, and a full send/stream/terminal cycle verified motionless at 236 messages. Architecture write-up in [clients/chat-scroll.md](clients/chat-scroll.md). Remaining polish, deliberately deferred: (a) the scroll-to-bottom pill sometimes shows during a pinned stream at the rest edge (cosmetic — tapping it is exact and harmless; the far-from-bottom signal misfires transiently during the send's estimate re-shuffle); (b) a reader parked in OLD history when a new message lands shifts by the new row's height (growth at the origin side moves origin-measured offsets; chat-standard behavior, accepted for v1); (c) the iOS 26 scroll-edge-fade regression on inverted scroll views (FB20540755) is worked around with a hand-rolled gradient — revisit when Apple fixes the extent math.
