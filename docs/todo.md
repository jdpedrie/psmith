...
- **Mid-stream history access after a send.** The send collapses the mounted window to the new turn, so during generation the user can't scroll above their question; history remounts silently at the next settled-at-bottom (finish the reply or tap the pill). Fine for normal use; if it ever bites, the fix is a scroll-up-triggered remount that runs under an at-bottom anchor handoff, not a plain prepend (see the failure taxonomy in [clients/chat-scroll.md](clients/chat-scroll.md) for why arbitrary-offset prepends move the viewport).
