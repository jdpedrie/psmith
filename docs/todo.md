...
- **Port the iOS chat-scroll architecture to Mac.** The Mac ConversationView (`clients/psmithd-mac/PsmithMac/ConversationView.swift`, `paneScrollBody`) still runs the pre-v7 design with all four failure classes the iOS side spent months eliminating: animated `scrollTo` per streaming chunk, `scrollTo(last, anchor: .top)` on message-count change (fires at terminal), `onScrollPhaseChange`-based user detection (misfires on system motion), and no width pin. The current iOS design — tail window + staged backfill + entry curtain + sizeChanges anchor + `isPositionedByUser` + exact-width pin — is documented in [clients/chat-scroll.md](clients/chat-scroll.md); the shared markdown theme (code/table horizontal scrollers) is already in place. Port when Mac scroll work resumes.
