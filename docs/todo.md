...
- **Surface truncated compaction summaries in the review UIs.** The server now persists `finish_reason` on `compression_summary` rows and sizes the compression output budget per model (min(model max_output_tokens, 32768), 8192 when unknown), which fixed the "compression stops mid-sentence" bug (hidden reasoning tokens were eating the fixed 8192 cap). Remaining client work: the compact review screens (iOS/Mac/web) should badge a summary whose `finish_reason` is `max_tokens`/`length`/`MAX_TOKENS` as truncated and suggest re-running, instead of presenting it as reviewable-complete. Also worth a look while there: `stream_runs` for compression carry no finish_reason at all — the message row is the only place it lands.
