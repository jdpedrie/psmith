...
- **Server perf follow-ups from the 2026-07-21 audit.** (1) `history.Build` fetches the ENTIRE context (`ListMessagesByContext`) then walks the parent chain in memory — O(context) instead of O(chain) on every send AND every `CountContextTokens` (which the client calls after each mutation); switch to `ListMessageAncestorChain` when a leaf is known. (2) `ListContextsByConversation` LEFT JOINs all messages per context for count/cost aggregates plus a correlated LIMIT 1 subquery per context; consider denormalizing the aggregates onto the context row. (Shipped in the same round: `messages(parent_id)` index for the stitch-delete reparent, `GetContextLeafMessage` NOT IN → NOT EXISTS.)
