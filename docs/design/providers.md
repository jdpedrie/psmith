...
Anthropic's request shape for thinking also split by model generation: older models take `thinking: {type: "enabled", budget_tokens: N}`, newer ones (Fable 5 / Opus 4.7 onward, Sonnet 4.6 onward) reject that with a 400 and require `thinking: {type: "adaptive"}` plus `output_config.effort`. The driver picks the first attempt from a per-model prefix table (`thinking_shape.go`), maps the user's budget-tokens setting onto an effort level (low under 8K, medium under 32K, high above), and — because the table is a guess, not a contract — retries a shape-mismatch 400 once with the other shape, before any output has streamed. A brand-new model the table doesn't know costs one doomed request and then works. The single thinking-budget knob in call settings stays the user-facing control for both shapes.

The same generation locks temperature at 1.0 (any other value 400s). The constraints table encodes that as a LockedAt temperature for the affected prefixes — the same single source of truth the clients read to render the slider locked and the openai driver already uses for gpt-5/o-series — and the anthropic driver omits the temperature parameter for those models. For a model the table doesn't know yet, the send-time remedy loop catches the sampling-rejection 400 and retries once with the sampling knobs stripped. The remedies compose: an unknown model needing both the adaptive flip and the sampling strip recovers in three requests, invisibly.
