...
- **files** (device). Its own five-tool catalog over a bookmarked notes folder: `files_list_notes` and `files_read_note` and `files_search_text` on by default, `files_append_note` and `files_create_note` off. Shares the broker; an unsupported call tells the user to bookmark a folder in settings. (Formerly `obsidian` — an Obsidian vault remains the flagship use.)
