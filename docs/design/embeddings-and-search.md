...
The `vector` extension has to be installed in the database before this migration runs. `CREATE EXTENSION` is not in the migration, because the migration runner usually lacks the database-level privilege it needs; `psmith install` runs an ensure-vector preflight, and the local-dev setup installs it into `template1` so every cloned test database inherits it. See [installation.md](../operations/installation.md).
...
The conversations service attaches the searcher and the caller info to the dispatch context right before it calls the tool. If no embedder is configured the searcher is nil, and the tool returns a friendly "search not configured, set PSMITH_EMBEDDER" error rather than panicking. The plugin needs the `tool_use` capability like any other tool provider, and it is one of the shipped plugins in [plugins.md](plugins.md).
