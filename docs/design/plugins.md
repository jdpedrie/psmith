...
`pluginapi/host/` is the other direction, the capabilities the host provides to a plugin: `Searcher`, `CallerInfo`, `DeviceToolBroker`, `ProviderResolver`, `PluginStateStore`.

`PluginStateStore` covers two scopes, and the distinction matters. `LoadNearest`/`Save` are branch-scoped: keyed to a message, read by walking the parent chain, so forking a conversation forks the state. That is right for recording what happened. `LoadConversation`/`SaveConversation` are conversation-scoped, one row overwritten in place, for intent about a send that has not happened yet. Conflating them fails concretely: queued intent cannot be keyed to a message on a conversation that has none, which is exactly when a user queues something. Interfaces and small value types only, with the concrete implementations server-side and injected on the dispatch context at `server/conversations/tool_loop.go`. The package is held to zero in-module dependencies, because a shim that grows a concrete dependency has stopped being a shim.
...
- **PanelProvider** — contributes a surface to the composer's add menu. The descriptor is a title and an icon; the body is `[]ContentPart`, the same structure ContentRenderer emits into a message. Clients draw panels with the fragment renderer they already have, so adding a plugin with a panel changes nothing in any client. The trade is that a panel can only say what the fragment vocabulary can say, which is deliberate: a plugin needing something new is an argument for growing the vocabulary, which every plugin then gets, rather than compiling a bespoke form into every app.
- **ActionHandler** — receives `plugin:<action>?<k>=<v>` actions dispatched from a panel it rendered. This is the return path the fragment vocabulary was missing: `compose:`, `send:` and `external:` all terminate in the client, so a panel could display state but never change it.
...
- **Outside a turn entirely**: PanelProvider and ActionHandler, driven by `GetPluginPanel` and `InvokePluginAction`. The latter returns the re-rendered panel, so an action never leaves the UI showing stale state while a second fetch is in flight.
