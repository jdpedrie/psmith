...
The iOS app lives in `clients/spaltd-ios/` and builds against the shared `SpaltKit` / `SpaltUI` package one directory up. The Xcode project is generated from `clients/spaltd-ios/project.yml` by [xcodegen](https://github.com/yonaskolb/XcodeGen); the `.xcodeproj` itself is gitignored and is not the source of truth. Never hand-edit the project file. Change `project.yml` and regenerate.
...
- A running `spaltd` to point the app at. The simulator can reach a local server at `http://localhost:8080`; a physical device needs a URL it can route to. See [../operations/installation.md](../operations/installation.md) to get the server up.

You do not need to install or run `buf` to build the app. The generated Swift bindings are checked into `SpaltKit/Generated/`; regenerate them only when a `.proto` changes (see [../operations/building-and-codegen.md](../operations/building-and-codegen.md)).
...
On first launch the app asks for the `spaltd` URL, probes it, then asks for credentials (the same users you create with `spalt useradd`). On the simulator, `http://localhost:8080` reaches a server running on the same Mac.

## The targets, one at a time

| Target | What it does |
|---|---|
| `make ios-project` | Runs `xcodegen generate` to write `SpaltiOS.xcodeproj` from `project.yml`. |
...
`ios-build` pins a project-local DerivedData path at `clients/spaltd-ios/.build`, so the build output lands at a predictable location (`.../Build/Products/Debug-iphonesimulator/SpaltiOS.app`) instead of a per-machine hash under `~/Library/Developer/Xcode/DerivedData`.

The bundle identifier is `dev.jdpedrie.SpaltiOS`.
...
2. Open `clients/spaltd-ios/SpaltiOS.xcodeproj` in Xcode.
...
- **Build looks stale or links a broken binary after changing a public `SpaltKit` type.** The incremental Swift cache can produce a bad binary across ABI changes. Wipe it: `rm -rf clients/spaltd-ios/.build` (and `clients/SpaltSwift/.build` if the package itself changed), then rebuild.
...
- [../operations/installation.md](../operations/installation.md) — standing up a `spaltd` to connect to.
