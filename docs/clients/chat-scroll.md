...
3. **There is no auto-follow, and the send pin never scrolls.** A send collapses the mounted window to the new turn (`mountedFromIndex = messages.count`): the question is the topmost mounted content — viewport top by construction, offset ~0 — and the response streams in below a motionless viewport. Positionless + anchorless, content growth below the fold cannot move the offset (verified: offset numerically frozen through 21s streams at heavy scale).
...
- **Entry curtain**: the transcript renders at opacity 0 (layout fully live) until the geometry handler first observes the viewport settled at the bottom **with `contentOffset.x` clean**, then fades in over 120ms; a 700ms timeout reveals regardless.
...
- **Send = window collapse**: `mountedFromIndex = messages.count`, fresh `ScrollPosition()`, anchor off, nothing scrolled. The pending row (then the real user row it swaps into) is the topmost mounted content; the reply streams below the motionless viewport. Mid-stream the user can scroll freely within the new turn (nothing holds a position to fight the drag) but cannot reach history above the question — the reference-app new-turn framing, accepted. If the send fails before a run starts, the nil-`pendingUserText` handler restores the cold-entry tail window.
- **Terminal**: nothing positional and no eager remount (expanding the window here was the historical stream-close desert). The id-stable content swap can't move a positionless viewport. History remounts silently the next time the user is settled at the bottom (finish reading the reply, or tap the pill) — the backfill trigger's normal condition.
...
- **Past-bottom clamp**: idle, not user-positioned, `distance < -1` → one non-animated `seekBottom()`. Covers container growth (keyboard dismissal), which the sizeChanges anchor ignores by design. One-shot per tick, never armed as a loop — arming it fed the convergence storm.
...
| Send pin lands mid-viewport or not at all | id-solve of the question against 1.3M pt of estimates | window collapse — pin by construction, no scrolling |
