...
- **Past-bottom clamp**: idle, not user-positioned, runway reclaimed, `distance < -1` → one non-animated absolute `scrollTo(y: contentHeight − viewportHeight)` (measured, exact — an id/edge solve can land ABOVE the bottom where the `< -1` guard never re-fires). Covers container growth (keyboard dismissal), which the sizeChanges anchor ignores by design. One-shot per tick, never armed as a loop — arming it fed the convergence storm.
