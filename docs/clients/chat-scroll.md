...
- **Staged backfill**: starts the instant the geometry handler observes the viewport settled at the bottom (`needsBackfill && distance < 8`, not sending, not streaming, not user-positioned), OR whenever the user nears the top of the mounted window (`offset < 2400`, no user-position gate — scrolling up is exactly the signal; without this, a paused walk left older history unreachable, user-reported). The walk moves `mountedFromIndex` to 0 in 40-row batches, one 80ms beat apart, entry anchor attached (`entryAnchorActive`, a single stable Bool). An ACTIVE finger (`scrollPhase == .tracking/.interacting`) delays the next batch rather than aborting the walk — `isPositionedByUser` latches true after any grab and stranded the walk until a rarely-reached settled-at-bottom; phase misreads only cost 60ms of batch delay. Sends and streams still abort; the triggers resume afterwards. Each prepend lands above the anchor-pinned viewport (log-verified lockstep at 420+ messages).
...
- **Pill + bottom-seek convergence**: the pill (TOP-anchored overlay, explicit user preference, 8pt top padding) arms `seekingBottom` — a dedicated flag, NOT shared with any follow state (the shared-flag version raced the user-scroll disengage; "pill does nothing") — and seeks bottom non-animated. During a stream the seek targets the EDGE, not the `"__streaming__"` id: an id-solve for a row far outside the realized band can silently no-op ("pill only works when no stream is running", user-reported). While armed, the geometry handler re-seeks whenever `distance > 64` and idle, until arrival (`distance < 8`) or user grab.

- **No pull-to-refresh on the transcript**: `.refreshable` fired a full `load()` that replaced messages under the mounted window mid-read, and the gesture collides with reading history at the top. Streams and events keep an open conversation live; refresh belongs to the chats list.
