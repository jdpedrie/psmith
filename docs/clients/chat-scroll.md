...
- **Send = rest-edge pin + shrinking runway + stream clip** (fifth design). The pin never scrolls: at send the runway spacer (`streamRunwayBudget = max(240, viewportHeight)`, rendered FIRST in the flipped stack, nearest offset 0) makes spacer + reply + question fill exactly one viewport, so the rest-edge frame IS "question at the top, runway below". While the reply fits the runway the shrinking spacer keeps content constant — motionless, zero commands. When the reply outgrows the runway, the streaming area is clipped to the runway height (`frame(maxHeight: budget, alignment: .bottom).clipped()` — content-space bottom = the reply's first lines through the flips), so content stays constant for the ENTIRE stream; the pre-inversion design kept the tail below the fold, this keeps it behind the clip. If the user had scrolled away before sending, one exact catch-up `scrollTo(y: 0)` fires; otherwise the insert lands at the visible rest edge naturally. The fourth design (held id-anchor pin) died in one live test: the head-insert's estimate re-map moved the world under the frozen offset and the held id never re-solved (log: offset frozen at 118 through ±43k CONTENT-LURCHes, whole stream invisible).
- **Terminal**: reclaim the runway (the clip releases with it; the settled row renders in full), then ONE exact id-solve — `scrollTo(id: question, anchor: UnitPoint(x: 0, y: 1))` after a 150ms layout beat — puts the question at the viewport top so reading starts where the reply does. The question row and everything newer are realized at that moment, so the solve is exact; for short replies it clamps to the rest edge (the same frame the stream showed). Skipped when the user owns the viewport; the position releases at rest 300ms later. The single-message terminal append (`GetMessage` + in-place append) is unchanged from the perf round.
