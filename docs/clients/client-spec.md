...
Psmith speaks ConnectRPC over HTTP/2. The server runs h2c (cleartext HTTP/2) behind whatever TLS termination the operator puts in front of it; the client connects with the Connect protocol and protobuf codec. There are also a few plain HTTP endpoints for things that do not fit the RPC mold (file download, the device-tool and elicitation respond endpoints, health). All of them, RPC and HTTP alike, authenticate with the same bearer token.

The protobuf schema is the source of truth for every message shape. Generate your client stubs from `proto/psmith/v1/*.proto`. Every RPC has a dedicated request and response message; no RPC returns a bare domain message or an empty, so responses always have headroom to grow.
...
`EventsService.SubscribeAccountEvents` is a server-streaming push channel for account-level changes: `ProfileChanged` (profile created/updated/deleted), `ConversationChanged` (any mutation of any conversation, from any client — including the subscriber's own), and `ProviderChanged` (a model provider or any of its models mutated; model-level changes report the owning provider). It has no replay: a subscription delivers events from the moment it connects, and a reconnect starts fresh. Treat it as a hint to refresh, not as a source of truth; the recovery story for a missed event is the same re-fetch you do everywhere. A client can ignore this entirely and just re-fetch on screen entry.

The reference clients react to `ConversationChanged` three ways: a debounced conversations-list refresh; when the changed conversation is open, a staleness check that costs one `GetConversation` and only re-fetches the chain when the active context, its leaf, or the conversation's `updated_at` moved; and — when the changed conversation is on screen with no locally-active stream — a scoped `ListActiveRuns(conversation_id)` that adopts a just-started run from another client, so live streaming appears within a beat instead of waiting for the list refresh's adopt sweep. The staleness check is also the echo suppressor: the client's own mutations arrive as events too, and after the local update the check sees nothing stale. The server bumps `updated_at` on message edits and deletes precisely because those mutations move neither the leaf nor any conversation column — without the bump, a cross-client edit would be invisible to the cheap check. `ProviderChanged` reloads the provider list wholesale. Foreground (iOS scene activation, Mac window focus, Mac ⌘R) runs the same reactions AND reconnects the events stream itself (`kick`): a subscription that died during a background suspend may be sitting in reconnect backoff (capped at 30s), and the pull refresh only covers the moment of activation — the kick restores the push channel for everything after it. Reconnect backoff should reset after any healthy connection, not only after a clean close, so a session's accumulated blips don't stretch every future recovery toward the cap.
