...
`ConversationView` is also where the hard-won scroll behavior lives — since 2026-07-21 an inverted (bottom-anchored) list: the transcript renders through a y-flip with the newest message at content offset 0, which makes entry, the scroll-to-bottom pill, and the send pin exact by construction. The full architecture, the flip's iOS 26 workarounds (scroll-edge fade, context-menu previews, mirrored safe-area margins), and the failure taxonomy live in [chat-scroll.md](chat-scroll.md).
