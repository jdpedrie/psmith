...
The generated templ files are checked in, so `go build ./...` and `make build` need no extra step. After editing a `.templ` file, run `make web-generate`. The UI is served by `spaltd` on its normal address; open `/` (it redirects to `/login`). See [operations/configuration.md](../operations/configuration.md) for the server's environment.
