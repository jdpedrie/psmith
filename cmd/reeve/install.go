...
func installCmd(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), `Usage: reeve install [flags]

Apply embedded schema migrations to the configured reeved database.
...
`)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "reeve install: open database: %v\n", err)
		return 1
	}
	defer sqlDB.Close()
	if err := sqlDB.Ping(); err != nil {
		fmt.Fprintf(os.Stderr, "reeve install: connect: %v\n", err)
	...
	if *statusOnly {
		...
		if err := goose.Status(sqlDB, "migrations"); err != nil {
			fmt.Fprintf(os.Stderr, "reeve install: status: %v\n", err)
			return 1
		}
		return 0
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "reeve install: read current version: %v\n", err)
		return 1
	}

	if *target >= 0 {
		if err := goose.UpTo(sqlDB, "migrations", *target); err != nil {
			fmt.Fprintf(os.Stderr, "reeve install: %v\n", err)
			return 1
		}
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "reeve install: read post-migrate version: %v\n", err)
		return 1
	}
