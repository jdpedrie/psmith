...
func installCmd(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), `Usage: spalt install [flags]

Apply embedded schema migrations to the configured spaltd database.
...
`)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: open database: %v\n", err)
		return 1
	}
	defer sqlDB.Close()
	if err := sqlDB.Ping(); err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: connect: %v\n", err)
	...
	if err := ensureExtensions(sqlDB); err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: ensure extensions: %v\n", err)
		return 1
	}

	goose.SetBaseFS(db.Migrations)
	if err := goose.SetDialect("postgres"); err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: set dialect: %v\n", err)
		return 1
	}
	...
	if *statusOnly {
		...
		if err := goose.Status(sqlDB, "migrations"); err != nil {
			fmt.Fprintf(os.Stderr, "spalt install: status: %v\n", err)
			return 1
		}
		return 0
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: read current version: %v\n", err)
		return 1
	}

	if *target >= 0 {
		if err := goose.UpTo(sqlDB, "migrations", *target); err != nil {
			fmt.Fprintf(os.Stderr, "spalt install: %v\n", err)
			return 1
		}
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "spalt install: read post-migrate version: %v\n", err)
		return 1
	}
...
// requiredExtensions are PostgreSQL extensions spaltd depends on at
		...
		if _, err := db.Exec(`CREATE EXTENSION IF NOT EXISTS ` + name); err != nil {
			return fmt.Errorf(
				"extension %q: %w (install it once as a superuser, "+
					"then re-run `spalt install`)", name, err)
		}
	}
	return nil
}
