...
// for SPALT_MASTER_KEY. Output is JUST the key string with a trailing
...
func genkeyCmd(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), `Usage: spalt genkey

Mint a fresh 32-byte AES-256-GCM master key, base64-encoded. Pipe
into SPALT_MASTER_KEY at deploy time:
...
provider/plugin config rows written under it.`)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "spalt genkey: %v\n", err)
		return 1
	}
	fmt.Println(key)
	return 0
