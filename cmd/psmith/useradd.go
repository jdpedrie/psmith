...
// useraddCmd creates a user row in the psmithd database. Returns the
...
func useraddCmd(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), `Usage: psmith useradd [flags]

Create a user account directly in the psmithd database. Bypasses the
...
`)
	...
	if strings.TrimSpace(*username) == "" {
		fmt.Fprintln(os.Stderr, "psmith useradd: -u <username> is required")
		fs.Usage()
		return 2
	}
	if *noAdmin {
	...
	if pw == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "psmith useradd: read password: %v\n", err)
			return 1
		}
	}
	if pw == "" {
		fmt.Fprintln(os.Stderr, "psmith useradd: empty password rejected")
		return 1
	}
	...
	if err := createUser(*username, pw, *admin, dbURL); err != nil {
		fmt.Fprintf(os.Stderr, "psmith useradd: %v\n", err)
		return 1
	}
	role := "user"
	if *admin {
