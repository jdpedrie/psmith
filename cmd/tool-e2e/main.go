...
// running psmithd. It logs in, finds the Personal Assistant profile (which
...
func main() {
	...
	profilesClient := psmithv1connect.NewProfilesServiceClient(authedHTTP, *addr)
	convClient := psmithv1connect.NewConversationsServiceClient(authedHTTP, *addr)
	streamsClient := psmithv1connect.NewStreamsServiceClient(authedHTTP, *addr)
	mpClient := psmithv1connect.NewModelProvidersServiceClient(authedHTTP, *addr)

	// 3. Find the target profile.
	listProf, err := profilesClient.ListProfiles(ctx, connect.NewRequest(&psmithv1.ListProfilesRequest{}))
	if err != nil {
	...
	pluginsResp, err := profilesClient.GetProfilePlugins(ctx, connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
		ProfileId: profileID,
	}))
	if err != nil {
	...
	createResp, err := convClient.CreateConversation(ctx, connect.NewRequest(&psmithv1.CreateConversationRequest{
		ProfileId: profileID,
	}))
	if err != nil {
	...
	sendReq := &psmithv1.SendMessageRequest{
		ConversationId: convID,
		Content:        *prompt,
	}
	if *providerName != "" || *modelID != "" {
		listProv, err := mpClient.ListUserModelProviders(ctx, connect.NewRequest(&psmithv1.ListUserModelProvidersRequest{}))
		if err != nil {
	...
	sub, err := streamsClient.SubscribeStream(ctx, connect.NewRequest(&psmithv1.SubscribeStreamRequest{
		StreamRunId:  runID,
		FromSequence: 0,
	}))
	if err != nil {
	...
	for sub.Receive() {
		...
		if ch := ev.GetChunk(); ch != nil {
			...
			switch ch.Type {
			case psmithv1.ChunkType_CHUNK_TYPE_TOOL_USE_START,
				psmithv1.ChunkType_CHUNK_TYPE_TOOL_USE_END,
				psmithv1.ChunkType_CHUNK_TYPE_TOOL_RESULT,
				psmithv1.ChunkType_CHUNK_TYPE_ERROR,
				psmithv1.ChunkType_CHUNK_TYPE_USAGE:
...
func readBackAndVerify(ctx context.Context, c psmithv1connect.ConversationsServiceClient, msgID string, counts map[string]int) {
	fmt.Printf("\n--- chunk counts ---\n")
	for k, v := range counts {
		fmt.Printf("  %-32s %d\n", k, v)
	}

	resp, err := c.GetMessage(ctx, connect.NewRequest(&psmithv1.GetMessageRequest{Id: msgID}))
	if err != nil {
