...
func tier(v spaltv1.ServiceTier) *spaltv1.ServiceTier {
	return &v
}
func harm(v spaltv1.HarmThreshold) *spaltv1.HarmThreshold {
	return &v
}
...
func TestMergeCallSettings_OnlyHigher(t *testing.T) {
	in := &spaltv1.CallSettings{Temperature: f64(0.5)}
...
func TestMergeCallSettings_OnlyLower(t *testing.T) {
	in := &spaltv1.CallSettings{TopP: f64(0.95)}
...
func TestMergeCallSettings_StopSequencesWholeFieldOverride(t *testing.T) {
	higher := &spaltv1.CallSettings{StopSequences: []string{"X", "Y"}}
	lower := &spaltv1.CallSettings{StopSequences: []string{"A", "B", "C"}}
	...
	higher2 := &spaltv1.CallSettings{Temperature: f64(0.1)}
...
func TestMergeCallSettings_PrecedenceMultipleLayers(t *testing.T) {
	...
	top := &spaltv1.CallSettings{Temperature: f64(0.7)} // wins on temp
	middle := &spaltv1.CallSettings{
		Temperature:     f64(0.4),  // shadowed by top
		MaxOutputTokens: i32(2048), // wins
	}
	bottom := &spaltv1.CallSettings{
...
func TestMergeCallSettings_ThinkingPerFieldMerge(t *testing.T) {
	...
	higher := &spaltv1.CallSettings{Thinking: &spaltv1.ThinkingSettings{Enabled: bp(true)}}
	lower := &spaltv1.CallSettings{Thinking: &spaltv1.ThinkingSettings{BudgetTokens: i32(4000)}}
	got := MergeCallSettings(higher, lower)
...
func TestMergeCallSettings_ThinkingHigherOverridesLower(t *testing.T) {
	higher := &spaltv1.CallSettings{Thinking: &spaltv1.ThinkingSettings{
		Enabled:      bp(false),
		BudgetTokens: i32(1000),
	}}
	lower := &spaltv1.CallSettings{Thinking: &spaltv1.ThinkingSettings{
		Enabled:      bp(true),
...
func TestMergeCallSettings_OpenAIPerFieldMerge(t *testing.T) {
	...
	higher := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{Seed: i32(42)}}
	lower := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{
...
func TestMergeCallSettings_OpenAIServiceTierEnumPickWins(t *testing.T) {
	higher := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{ServiceTier: tier(spaltv1.ServiceTier_SERVICE_TIER_PRIORITY)}}
	lower := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{ServiceTier: tier(spaltv1.ServiceTier_SERVICE_TIER_AUTO)}}
	got := MergeCallSettings(higher, lower)
	if got.GetOpenai().GetServiceTier() != spaltv1.ServiceTier_SERVICE_TIER_PRIORITY {
		t.Errorf("service_tier: %v want PRIORITY (higher wins)", got.GetOpenai().GetServiceTier())
	}

	higher2 := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{Seed: i32(1)}}
	got2 := MergeCallSettings(higher2, lower)
	if got2.GetOpenai().GetServiceTier() != spaltv1.ServiceTier_SERVICE_TIER_AUTO {
...
func TestMergeCallSettings_OpenAILogitBiasWholeFieldOverride(t *testing.T) {
	higher := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{LogitBias: map[int32]float64{100: 1.5}}}
	lower := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{LogitBias: map[int32]float64{200: -2.0, 300: 0.5}}}
...
func TestMergeCallSettings_OpenAIResponseFormatWholeFieldOverride(t *testing.T) {
	higher := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{
		ResponseFormat: &spaltv1.ResponseFormat{
			Kind: &spaltv1.ResponseFormat_JsonObject{JsonObject: true},
		},
	}}
	lower := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{
		ResponseFormat: &spaltv1.ResponseFormat{
			Kind: &spaltv1.ResponseFormat_JsonSchema{JsonSchema: &spaltv1.JsonSchema{
				Name: "answer", Schema: []byte(`{}`),
			}},
		},
	}}
	got := MergeCallSettings(higher, lower)
	if _, ok := got.GetOpenai().ResponseFormat.GetKind().(*spaltv1.ResponseFormat_JsonObject); !ok {
		t.Errorf("response_format: %T want JsonObject", got.GetOpenai().ResponseFormat.GetKind())
	}

	higher2 := &spaltv1.CallSettings{Openai: &spaltv1.OpenAIExtras{Seed: i32(1)}}
	got2 := MergeCallSettings(higher2, lower)
	js, ok := got2.GetOpenai().ResponseFormat.GetKind().(*spaltv1.ResponseFormat_JsonSchema)
	if !ok {
...
func TestMergeCallSettings_GooglePerFieldMerge(t *testing.T) {
	higher := &spaltv1.CallSettings{Google: &spaltv1.GoogleExtras{
		ResponseMimeType: sp("application/json"),
	}}
	lower := &spaltv1.CallSettings{Google: &spaltv1.GoogleExtras{
...
func TestMergeCallSettings_SafetySettingsPerFieldMerge(t *testing.T) {
	higher := &spaltv1.CallSettings{Google: &spaltv1.GoogleExtras{
		SafetySettings: &spaltv1.SafetySettings{
			Harassment: harm(spaltv1.HarmThreshold_HARM_THRESHOLD_BLOCK_NONE),
		},
	}}
	lower := &spaltv1.CallSettings{Google: &spaltv1.GoogleExtras{
		SafetySettings: &spaltv1.SafetySettings{
			Harassment:       harm(spaltv1.HarmThreshold_HARM_THRESHOLD_BLOCK_ONLY_HIGH),
			DangerousContent: harm(spaltv1.HarmThreshold_HARM_THRESHOLD_BLOCK_LOW_AND_ABOVE),
		},
	}}
	got := MergeCallSettings(higher, lower)
	ss := got.GetGoogle().GetSafetySettings()
	if ss.GetHarassment() != spaltv1.HarmThreshold_HARM_THRESHOLD_BLOCK_NONE {
		t.Errorf("harassment: %v want BLOCK_NONE (higher wins)", ss.GetHarassment())
	}
	if ss.GetDangerousContent() != spaltv1.HarmThreshold_HARM_THRESHOLD_BLOCK_LOW_AND_ABOVE {
...
func TestMergeCallSettings_AnthropicExtrasPresenceOnly(t *testing.T) {
	higher := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{}}
	got := MergeCallSettings(higher, nil)
	if got.GetAnthropic() == nil {
		t.Error("expected anthropic block to survive merge")
	}
	got2 := MergeCallSettings(nil, &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{}})
	if got2.GetAnthropic() == nil {
		t.Error("expected anthropic block to survive merge from lower")
	}
}

func ttl(v spaltv1.CacheTTL) *spaltv1.CacheTTL { return &v }

func TestMergeCallSettings_AnthropicCacheEnabled_HigherWins(t *testing.T) {
	higher := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheEnabled: bp(false)}}
	lower := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheEnabled: bp(true)}}
...
func TestMergeCallSettings_AnthropicCacheEnabled_FillsFromLower(t *testing.T) {
	higher := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{}}
	lower := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheEnabled: bp(false)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil || got.CacheEnabled == nil || *got.CacheEnabled != false {
...
func TestMergeCallSettings_AnthropicCacheTTL_HigherWins(t *testing.T) {
	higher := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheTtl: ttl(spaltv1.CacheTTL_CACHE_TTL_1H)}}
	lower := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheTtl: ttl(spaltv1.CacheTTL_CACHE_TTL_5M)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil || got.CacheTtl == nil || *got.CacheTtl != spaltv1.CacheTTL_CACHE_TTL_1H {
		t.Errorf("expected ttl=1h from higher, got %+v", got)
	}
}

func TestMergeCallSettings_AnthropicCacheTTL_FillsFromLower(t *testing.T) {
	higher := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheEnabled: bp(true)}}
	lower := &spaltv1.CallSettings{Anthropic: &spaltv1.AnthropicExtras{CacheTtl: ttl(spaltv1.CacheTTL_CACHE_TTL_1H)}}
	got := MergeCallSettings(higher, lower).GetAnthropic()
	if got == nil {
	...
	if got.CacheTtl == nil || *got.CacheTtl != spaltv1.CacheTTL_CACHE_TTL_1H {
...
func TestCallSettingsCodec_RoundTrip(t *testing.T) {
	in := &spaltv1.CallSettings{
		...
		Openai: &spaltv1.OpenAIExtras{
			Seed:        i32(7),
			ServiceTier: tier(spaltv1.ServiceTier_SERVICE_TIER_PRIORITY),
	...
	if out.GetOpenai().GetSeed() != 7 || out.GetOpenai().GetServiceTier() != spaltv1.ServiceTier_SERVICE_TIER_PRIORITY {
