...
func TestListPluginTypes_IncludesLetteredChoicesWithCapabilities(t *testing.T) {
	...
	resp, err := svc.ListPluginTypes(ctxAs(user), connect.NewRequest(&psmithv1.ListPluginTypesRequest{}))
	if err != nil {
	...
	var keepLastN, sysOverride *psmithv1.ConfigField
	...
	if keepLastN.Type != psmithv1.ConfigField_NUMBER {
	...
	if sysOverride.Type != psmithv1.ConfigField_TEXTAREA {
...
func TestGetProfilePlugins_EmptyByDefault(t *testing.T) {
	...
	resp, err := svc.GetProfilePlugins(ctxAs(user), connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
...
func TestGetProfilePlugins_CrossUserNotFound(t *testing.T) {
	...
	_, err := svc.GetProfilePlugins(ctxAs(bob), connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
		ProfileId: prof.ID.String(),
	}))
...
func TestGetProfilePlugins_InvalidUUID(t *testing.T) {
	...
	svc, qs := newTestSvc(t)
	user := mustCreateUser(t, qs, "alice")
	_, err := svc.GetProfilePlugins(ctxAs(user), connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
...
func TestSetProfilePlugins_HappyPath(t *testing.T) {
	...
	got, _ := svc.GetProfilePlugins(ctxAs(user), connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
		ProfileId: prof.ID.String(),
	}))
...
func TestSetProfilePlugins_ConfigRoundTripsAfterEncryption(t *testing.T) {
	...
	got, err := svc.GetProfilePlugins(ctxAs(user), connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
		ProfileId: prof.ID.String(),
	}))
	if err != nil {
		t.Fatalf("GetProfilePlugins: %v", err)
...
func TestSetProfilePlugins_CrossUserNotFound(t *testing.T) {
	...
	bob := mustCreateUser(t, qs, "bob")
	prof := makeProfilePlain(t, qs, alice.ID, nil)
	_, err := svc.SetProfilePlugins(ctxAs(bob), connect.NewRequest(&psmithv1.SetProfilePluginsRequest{
		ProfileId: prof.ID.String(),
	}))
	assertConnectCode(t, err, connect.CodeNotFound)
}
...
func names(types []*psmithv1.PluginType) []string {
