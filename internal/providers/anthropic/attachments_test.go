...
// document / video attachments don't break the send path — the
// driver hasn't implemented those yet, and the capability table on
// the client is supposed to gate them out, but a defensive drop
// here means a misbehaving client gets a graceful turn (the model
// just sees the text part) rather than a 4xx.
