...
func (d *Driver) Send(ctx context.Context, req providers.SendRequest) (<-chan providers.Chunk, error) {
	...
	if os.Getenv("SPALT_DEBUG_GEMINI_REQUEST") != "" {
...
// buildRequestBody translates Spalt's wire shape into Gemini's request body.
//
// Roles:
...
// previously-stored thought block (Spalt's storage shape:
...
// safetySettingsToWire translates Spalt's SafetySettings struct into the
...
// harmThresholdToWire maps Spalt's enum to Gemini's enum strings.
