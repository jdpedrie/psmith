...
// model-picker fields.
func (h *Handler) buildPluginForm(ctx context.Context, t *reevev1.PluginType, currentConfig []byte, models []modelVM) pluginConfigForm {
...
func displayName(t *reevev1.PluginType) string {
	if t == nil {
		return ""
	}
...
// raw JSON fallback).
func pluginHasForm(t *reevev1.PluginType) bool {
	if t == nil || customFormPlugins[t.GetName()] {
		return false
	}
	for _, f := range t.GetConfigFields() {
...
func fieldTypeName(t reevev1.ConfigField_Type) string {
	switch t {
	case reevev1.ConfigField_TEXTAREA:
		return "textarea"
	case reevev1.ConfigField_NUMBER:
		return "number"
	case reevev1.ConfigField_BOOLEAN:
		return "boolean"
	case reevev1.ConfigField_SELECT:
		return "select"
	case reevev1.ConfigField_MODEL_PICKER:
		return "model"
	default:
		return "text"
	}
...
func assemblePluginConfig(t *reevev1.PluginType, current []byte, get func(field string) string) ([]byte, error) {
...
func (h *Handler) pluginTypesByName(ctx context.Context) map[string]*reevev1.PluginType {
	out := map[string]*reevev1.PluginType{}
	if h.profiles == nil {
		return out
	}
	resp, err := h.profiles.ListPluginTypes(ctx, connect.NewRequest(&reevev1.ListPluginTypesRequest{}))
	if err != nil {
		return out
	}
