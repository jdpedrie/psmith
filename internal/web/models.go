...
func modelRowVM(providerID string, m *reevev1.UserModel, selected string, required capsVM) pickerModelVM {
...
func (h *Handler) requiredCaps(ctx context.Context, convID string) capsVM {
	...
	conv, err := h.convos.GetConversation(ctx, connect.NewRequest(&reevev1.GetConversationRequest{Id: convID}))
	if err != nil {
		return req
	}
	prof, err := h.profiles.GetProfile(ctx, connect.NewRequest(&reevev1.GetProfileRequest{
...
func missingCaps(have *reevev1.ModelCapabilities, required capsVM) []string {
	var missing []string
...
func costBucket(p *reevev1.ModelPricing) (bucket, hint string) {
...
func currentSettingsModel(conv *reevev1.Conversation) string {
