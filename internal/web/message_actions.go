...
func (h *Handler) handleEditForm(w http.ResponseWriter, r *http.Request) {
	...
	mid := r.PathValue("mid")
	resp, err := h.convos.GetMessage(r.Context(), connect.NewRequest(&reevev1.GetMessageRequest{Id: mid}))
	if err != nil {
...
func (h *Handler) handleEditSave(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.convos.EditMessage(r.Context(), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      mid,
...
func (h *Handler) handleRegenerate(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := h.convos.SendMessage(r.Context(), connect.NewRequest(&reevev1.SendMessageRequest{
