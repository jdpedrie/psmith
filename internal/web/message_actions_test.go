...
func activeContextID(t *testing.T, convos *conversations.Service, ctx context.Context, convID string) uuid.UUID {
	t.Helper()
	resp, err := convos.GetConversation(ctx, connect.NewRequest(&psmithv1.GetConversationRequest{Id: convID}))
	if err != nil {
...
func TestEditMessage(t *testing.T) {
	...
	got, err := convos.GetMessage(userCtx, connect.NewRequest(&psmithv1.GetMessageRequest{Id: mid.String()}))
	if err != nil {
...
func TestRegenerate(t *testing.T) {
	...
	if _, err := convos.UpdateConversation(userCtx, connect.NewRequest(&psmithv1.UpdateConversationRequest{
		Id:       fx.convID.String(),
		Settings: &psmithv1.ConversationSettings{DefaultProviderId: &pid, DefaultModelId: &mid},
	})); err != nil {
