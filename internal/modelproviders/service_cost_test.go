...
func TestListProviderCosts_GroupsByProviderAndSums(t *testing.T) {
	...
	resp, err := svc.ListProviderCosts(ctxAs(user), connect.NewRequest(&psmithv1.ListProviderCostsRequest{}))
	if err != nil {
	...
	got := map[string]*psmithv1.ProviderCost{}
...
func TestListProviderCosts_ScopedByCaller(t *testing.T) {
	...
	resp, err := svc.ListProviderCosts(ctxAs(alice), connect.NewRequest(&psmithv1.ListProviderCostsRequest{}))
	if err != nil {
		t.Fatalf("ListProviderCosts: %v", err)
	}
	if len(resp.Msg.Providers) != 1 {
