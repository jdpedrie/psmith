...
func TestReactivation_SendLandsInReactivatedContext(t *testing.T) {
	...
	if _, err := svc.ActivateContext(ctxAsUser(f.user), connect.NewRequest(&spaltv1.ActivateContextRequest{
...
func TestReactivation_Idempotent(t *testing.T) {
	...
	resp, err := svc.ActivateContext(ctxAsUser(f.user), connect.NewRequest(&spaltv1.ActivateContextRequest{
		ContextId: f.contextID.String(),
	}))
	if err != nil {
		t.Fatalf("ActivateContext: %v", err)
...
func TestReactivation_CrossUser(t *testing.T) {
	...
	_, err := svc.ActivateContext(ctxAsUser(bob), connect.NewRequest(&spaltv1.ActivateContextRequest{
		ContextId: f.contextID.String(),
	}))
...
func TestReactivation_AcrossCompression(t *testing.T) {
	...
	sresp, err := svc.SendMessage(ctxAsUser(f.user), connect.NewRequest(&spaltv1.SendMessageRequest{
		ConversationId: f.conv.ID.String(),
		Content:        "back in A",
		ProviderId:     &pid, ModelId: &mid,
	}))
