...
func TestFork_TwoForksFromSameParent_SiblingCountReflects(t *testing.T) {
	...
	// Fork 1.
	resp1, err := svc.SendMessage(ctxAsUser(f.user), connect.NewRequest(&spaltv1.SendMessageRequest{
		ConversationId:  f.conv.ID.String(),
		ParentMessageId: &u1Str,
		Content:         "fork-1",
	...
	resp2, err := svc.SendMessage(ctxAsUser(f.user), connect.NewRequest(&spaltv1.SendMessageRequest{
		ConversationId:  f.conv.ID.String(),
		ParentMessageId: &u1Str,
		Content:         "fork-2",
		ProviderId:      &pid, ModelId: &mid,
	...
	listResp, _ := svc.ListMessages(ctxAsUser(f.user), connect.NewRequest(&spaltv1.ListMessagesRequest{
		ContextId:     f.contextID.String(),
		LeafMessageId: &leaf,
	}))
	if len(listResp.Msg.Messages) != 4 {
...
func TestRegenerate_FromAssistantParent(t *testing.T) {
	...
	if resp.Msg.UserMessage.Role != spaltv1.MessageRole_MESSAGE_ROLE_ASSISTANT {
	...
	if tail[0].Role != spaltv1.MessageRole_MESSAGE_ROLE_ASSISTANT ||
		tail[1].Role != spaltv1.MessageRole_MESSAGE_ROLE_ASSISTANT {
