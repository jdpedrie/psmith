...
func TestSetCurrentLeaf_UpdatesCursor(t *testing.T) {
	...
	resp, err := svc.SetCurrentLeaf(ctxAsUser(f.user), connect.NewRequest(&psmithv1.SetCurrentLeafRequest{
...
func TestSetCurrentLeaf_CrossUserContext(t *testing.T) {
	...
	_, err := svc.SetCurrentLeaf(ctxAsUser(bob), connect.NewRequest(&psmithv1.SetCurrentLeafRequest{
		ContextId: f.contextID.String(),
...
func TestListMessages_SiblingCountPopulated(t *testing.T) {
	...
	leaf := a1.ID.String()
	resp, err := svc.ListMessages(ctxAsUser(f.user), connect.NewRequest(&psmithv1.ListMessagesRequest{
		ContextId:     f.contextID.String(),
		LeafMessageId: &leaf,
	}))
	if err != nil {
