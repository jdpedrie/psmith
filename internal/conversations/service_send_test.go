...
func TestSendMessage_Success_MaterializesAssistant(t *testing.T) {
	...
	if resp.Msg.StreamRun == nil || resp.Msg.StreamRun.Status != reevev1.StreamRunStatus_STREAM_RUN_STATUS_RUNNING {
...
func TestSendMessage_CrossUserConversation(t *testing.T) {
	...
	pid := f.provider.ID.String()
	mid := f.modelID
	_, err := svc.SendMessage(ctxAsUser(bob), connect.NewRequest(&reevev1.SendMessageRequest{
		ConversationId: f.conv.ID.String(),
		Content:        "hi",
		ProviderId:     &pid,
		ModelId:        &mid,
...
func TestSendMessage_FourLayerCallSettingsMerge(t *testing.T) {
	...
	provCS := &reevev1.CallSettings{Temperature: f64ptr(0.4)}
	...
	modelCS := &reevev1.CallSettings{TopP: f64ptr(0.9)}
	...
	profileCS := &reevev1.CallSettings{MaxOutputTokens: i32ptr(4096)}
	...
	convCS := &reevev1.ConversationSettings{
		CallSettings: &reevev1.CallSettings{Temperature: f64ptr(0.7)},
	}
	...
	pid := f.provider.ID.String()
	mid := f.modelID
	if _, err := svc.SendMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.SendMessageRequest{
		ConversationId: f.conv.ID.String(),
		Content:        "hello",
		ProviderId:     &pid,
		ModelId:        &mid,
...
func TestSendMessage_NoSupervisor_Unimplemented(t *testing.T) {
	...
	pid := uuid.New().String()
	mid := "x"
	_, err := svc.SendMessage(ctxAsUser(user), connect.NewRequest(&reevev1.SendMessageRequest{
