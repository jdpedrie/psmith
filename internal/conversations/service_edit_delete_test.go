...
func TestEditMessage_HappyPath(t *testing.T) {
	...
	resp, err := svc.EditMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      msg.ID.String(),
...
func TestEditMessage_RoleFlipUserToAssistant(t *testing.T) {
	...
	asst := reevev1.MessageRole_MESSAGE_ROLE_ASSISTANT
	resp, err := svc.EditMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      msg.ID.String(),
	...
	if resp.Msg.Message.Role != reevev1.MessageRole_MESSAGE_ROLE_ASSISTANT {
...
func TestEditMessage_RoleFlipFromSystemRejected(t *testing.T) {
	...
	asst := reevev1.MessageRole_MESSAGE_ROLE_ASSISTANT
	_, err := svc.EditMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.EditMessageRequest{
...
func TestEditMessage_RoleFlipToContextRejected(t *testing.T) {
	...
	cxRole := reevev1.MessageRole_MESSAGE_ROLE_CONTEXT
	_, err := svc.EditMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      msg.ID.String(),
		Content: "x",
		Role:    &cxRole,
...
func TestEditMessage_CrossUserNotFound(t *testing.T) {
	...
	_, err := svc.EditMessage(ctxAsUser(bob), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      f.systemMsgID.String(),
		Content: "x",
	}))
	assertCode(t, err, connect.CodeNotFound)
...
func TestDeleteMessage_NotFound(t *testing.T) {
	...
	f := seedSendable(t, q, driverType)
	_, err := svc.DeleteMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.DeleteMessageRequest{
		Id: uuid.New().String(),
	}))
	assertCode(t, err, connect.CodeNotFound)
}
...
func TestDeleteMessage_CrossUserNotFound(t *testing.T) {
	...
	_, err := svc.DeleteMessage(ctxAsUser(bob), connect.NewRequest(&reevev1.DeleteMessageRequest{
		Id: f.systemMsgID.String(),
	}))
	assertCode(t, err, connect.CodeNotFound)
}
...
func TestPromote_REPLACE_SeedsNewContextWithSummaryOnly(t *testing.T) {
	...
	if _, err := svc.EditMessage(ctxAsUser(f.user), connect.NewRequest(&reevev1.EditMessageRequest{
		Id:      summaryID.String(),
	...
	resp, err := svc.PromoteCompactionToNewContext(ctxAsUser(f.user), connect.NewRequest(&reevev1.PromoteCompactionToNewContextRequest{
...
func TestPromote_RejectsNonSummaryMessage(t *testing.T) {
	...
	_, err := svc.PromoteCompactionToNewContext(ctxAsUser(f.user), connect.NewRequest(&reevev1.PromoteCompactionToNewContextRequest{
...
func TestLock_BlocksMutationsWhileStreamRunning(t *testing.T) {
	...
	t.Run("UpdateConversation", func(t *testing.T) {
		title := "blocked"
		_, err := svc.UpdateConversation(ctxAsUser(f.user), connect.NewRequest(&reevev1.UpdateConversationRequest{
			Id: f.conv.ID.String(), Title: &title,
		}))
		assertCode(t, err, connect.CodeFailedPrecondition)
	})
	t.Run("DeleteConversation", func(t *testing.T) {
		_, err := svc.DeleteConversation(ctxAsUser(f.user), connect.NewRequest(&reevev1.DeleteConversationRequest{
			Id: f.conv.ID.String(),
		}))
		assertCode(t, err, connect.CodeFailedPrecondition)
	})
	t.Run("ActivateContext", func(t *testing.T) {
		_, err := svc.ActivateContext(ctxAsUser(f.user), connect.NewRequest(&reevev1.ActivateContextRequest{
