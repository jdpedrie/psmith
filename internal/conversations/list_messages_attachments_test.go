...
func TestService_ListMessages_PopulatesAttachments(t *testing.T) {
	...
	listResp, err := svc.ListMessages(ctx, connect.NewRequest(&psmithv1.ListMessagesRequest{
		ContextId: f.contextID.String(),
	}))
	if err != nil {
		t.Fatalf("ListMessages chain: %v", err)
	}
	var userMsg *psmithv1.Message
	for _, m := range listResp.Msg.Messages {
		if m.Role == psmithv1.MessageRole_MESSAGE_ROLE_USER && m.Content == "look" {
			userMsg = m
			break
		}
	}
