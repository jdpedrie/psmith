...
func (s *Server) registerProfileTools() {
	...
	s.register(
		"registered_plugins",
		"REQUIRED before any work on a profile's plugin pipeline. Returns every plugin compiled into this Psmith build with its machine name, display name, description, capabilities (which interfaces it implements), and config_fields (the typed schema for its per-instance config). Without calling this you don't know which plugins exist or what shape their config takes — set_profile_plugins will fail with bad_argument if you guess. Re-call after a Psmith restart in case the build changed.",
		`{"type":"object","properties":{},"additionalProperties":false}`,
...
func (s *Server) toolListProfiles(ctx context.Context, _ json.RawMessage) (ToolResult, error) {
	resp, err := s.profilesSvc.ListProfiles(ctx, connect.NewRequest(&psmithv1.ListProfilesRequest{}))
	if err != nil {
...
func (s *Server) toolGetProfile(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	resp, err := s.profilesSvc.GetProfile(ctx, connect.NewRequest(&psmithv1.GetProfileRequest{
		Id:      in.ID,
		Resolve: in.Resolve,
	}))
	if err != nil {
...
func (s *Server) toolCreateProfile(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	req := &psmithv1.CreateProfileRequest{
		Name:               in.Name,
...
func (s *Server) toolListPluginTypes(ctx context.Context, _ json.RawMessage) (ToolResult, error) {
	resp, err := s.profilesSvc.ListPluginTypes(ctx, connect.NewRequest(&psmithv1.ListPluginTypesRequest{}))
	if err != nil {
		return errorResult(err.Error()), nil
	}
...
func (s *Server) toolGetProfilePlugins(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	resp, err := s.profilesSvc.GetProfilePlugins(ctx, connect.NewRequest(&psmithv1.GetProfilePluginsRequest{
...
func (s *Server) toolGetUserPluginSettings(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	resp, err := s.profilesSvc.GetUserPluginSettings(ctx, connect.NewRequest(&psmithv1.GetUserPluginSettingsRequest{
...
func (s *Server) toolUpsertUserPluginSettings(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	if len(in.SecretFieldNames) > 0 {
		...
		for _, sf := range in.SecretFieldNames {
			...
			resp, err := ec.Elicit(ctx, elicit.Request{
				Message:         prompt + " — stored encrypted on this Psmith instance, never sent to the LLM provider.",
	...
	if _, err := s.profilesSvc.UpsertUserPluginSettings(ctx, connect.NewRequest(&psmithv1.UpsertUserPluginSettingsRequest{
		PluginName: in.PluginName,
		Config:     merged,
	})); err != nil {
		return errorResult(err.Error()), nil
...
func (s *Server) toolSetProfilePlugins(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	...
	plugins := make([]*psmithv1.ProfilePlugin, 0, len(in.Plugins))
	...
	resp, err := s.profilesSvc.SetProfilePlugins(ctx, connect.NewRequest(&psmithv1.SetProfilePluginsRequest{
		ProfileId: in.ProfileID,
		Plugins:   plugins,
	}))
	if err != nil {
...
func profileSummary(p *psmithv1.Profile) map[string]any {
...
func profileDetail(p *psmithv1.Profile) map[string]any {
	out := map[string]any{
		"id":          p.GetId(),
		"name":        p.GetName(),
		"description": p.GetDescription(),
...
func pluginTypeDetail(pt *psmithv1.PluginType) map[string]any {
	out := map[string]any{
		"name":         pt.GetName(),
...
func profilePluginDetail(p *psmithv1.ProfilePlugin) map[string]any {
	out := map[string]any{
