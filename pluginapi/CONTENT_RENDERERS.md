...
> *Status: shipped server-side and on both Mac + iOS clients. The renderer set is shared across platforms via the PsmithUI Swift package.*
...
Renderers ship in
`clients/PsmithSwift/Sources/PsmithUI/PluginRenderers/` so Mac
...
existing `MessageAttachment` path — that goes through Psmith's
...
func (PromoBanner) RenderContent(parts []ContentPart, role string) []ContentPart {
    ...
    return WalkText(parts, func(s string) []ContentPart {
        ...
        props, _ := json.Marshal(map[string]any{
            "message": "Try Psmith Pro!", "code": "AD",
        })
        return []ContentPart{
            NewTextPart(before),
...
3. **Add a SwiftUI view** in
   `clients/PsmithSwift/Sources/PsmithUI/PluginRenderers/<Name>Renderer.swift`.
