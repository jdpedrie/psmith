...
struct IntegrationFlowTests {
    ...
    func newUserEndToEnd() async throws {
        ...
        // mirrors ReeveMac's startup wiring.
        ...
        let profile = try await profilesVM.create(ReeveProfilePatch(
            name: "Default",
    ...
    func overrideLayers() async throws {
        ...
        try await providersVM.updateProviderDefaultSettings(
            ...
            settings: ReeveCallSettings(temperature: 0.4)
        )
        await providersVM.selectProvider(provider.id)
        ...
        let profile = try await profilesVM.create(ReeveProfilePatch(
            ...
            defaultSettings: ReeveProfileDefaults(
                defaultProviderID: provider.id,
                defaultModelID: "fake-model-1",
                callSettings: ReeveCallSettings(topP: 0.9)
            )
        ))
        ...
        let conv = try await client.conversations.create(
            ...
            settings: ReeveConversationSettings(callSettings: ReeveCallSettings(temperature: 0.7))
        )
    ...
    func editAModelFully() async throws {
        ...
        _ = try await providersVM.updateModelFull(
            ...
            defaultSettings: ReeveCallSettings(temperature: 0.5)
        )
