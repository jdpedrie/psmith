...
/// reeved subprocess. Covers tests #1–#35 from the testing plan.
///
...
struct ConversationsRepositoryTests {
    ...
    func createWithSettings() async throws {
        ...
        let settings = ReeveConversationSettings(
            ...
            callSettings: ReeveCallSettings(temperature: 0.55)
        )
    ...
    func updateTitleEmptyClears() async throws {
        ...
        // `optional title = ""`, which round-trips through ReeveConversation
    ...
    func updateSettingsReplacesBlob() async throws {
        ...
        let (fake, provider, model, profile) = try await Fixtures.seedReadyToChat(client: client)
        _ = fake
        let initial = ReeveConversationSettings(
            defaultProviderID: provider.id,
            defaultModelID: model.modelID,
            includeThinkingInHistory: true
        )
        ...
        let replacement = ReeveConversationSettings(includeThinkingInHistory: false)
    ...
    private func drainStream(client: ReeveClient, runID: String) async throws {
    ...
    /// We don't have a "list streams" API surfaced on ReeveKit, so we just
    /// give the server up to ~3s before returning. Good enough for tests.
    private func waitNoActiveStream(client: ReeveClient, conversationID: String) async throws {
