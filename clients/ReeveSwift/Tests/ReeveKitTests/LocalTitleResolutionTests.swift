...
struct LocalTitleResolutionTests {
    ...
    private func makeVM(
        ...
        // ReeveClient requires a real URL but we never call it.
    ...
    func resolvesKindFromOwnProfile() {
        ...
        #expect(vm.resolvedTitleProviderKind(profilesByID: ["p1": p]) == ReeveTitleProviderKind.appleFoundation)
    }

    @Test
    func resolvesKindFromParentChain() {
        let parent = ReeveProfile(
            id: "parent",
            name: "Parent",
            titleProviderKind: ReeveTitleProviderKind.appleFoundation
        )
        let child = ReeveProfile(
            id: "child",
            name: "Child",
            parentProfileID: "parent"
        )
        let vm = makeVM(conversationProfileID: "child")
        let map: [String: ReeveProfile] = ["parent": parent, "child": child]
        #expect(vm.resolvedTitleProviderKind(profilesByID: map) == ReeveTitleProviderKind.appleFoundation)
    }
    ...
    func returnsNilWhenNoneSet() {
        let p = ReeveProfile(id: "p1", name: "p1")
        let vm = makeVM(conversationProfileID: "p1")
    ...
    func cycleSafe() {
        ...
        let a = ReeveProfile(id: "a", name: "a", parentProfileID: "b")
        let b = ReeveProfile(id: "b", name: "b", parentProfileID: "a")
