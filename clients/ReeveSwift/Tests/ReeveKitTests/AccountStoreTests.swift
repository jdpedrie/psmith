...
struct AccountStoreTests {
    ...
    private func makeStore() -> (AccountStore, UserDefaults) {
        let suiteName = "ReeveTests.AccountStore.\(UUID().uuidString)"
    ...
    private func makeAccount(label: String = "test") -> Account {
        Account(
            host: URL(string: "https://reeve.example.com")!,
    ...
    func malformedDataReturnsEmpty() {
        ...
        defaults.set("not json".data(using: .utf8), forKey: "reeve.accounts.v1")
        #expect(store.loadAccounts().isEmpty)
    }
    ...
    func malformedActiveID() {
        let (store, defaults) = makeStore()
        defaults.set("not-a-uuid", forKey: "reeve.accounts.active_id.v1")
        #expect(store.loadActiveID() == nil)
    }
}
