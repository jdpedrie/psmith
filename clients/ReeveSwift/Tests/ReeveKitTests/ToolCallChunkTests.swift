...
/// on `ReeveChunk` and the phase state machine on `LiveToolCall`. The
...
struct ToolCallChunkTests {
    // MARK: - ReeveChunk payload parsers

    @Test
    func toolUseStartParse() {
        let payload = #"{"id":"call-1","name":"web_search","provider_opaque":"abc"}"#.data(using: .utf8)!
        let chunk = ReeveChunk(sequence: 0, type: .toolUseStart, payload: payload)
    ...
    func toolUseStartReturnsNilForWrongType() {
        let chunk = ReeveChunk(sequence: 0, type: .textDelta, payload: Data())
    ...
    func toolUseDeltaParse() {
        ...
        let chunk = ReeveChunk(sequence: 0, type: .toolUseDelta, payload: payload)
    ...
    func toolResultParse() {
        let payload = #"{"tool_use_id":"call-1","output":{"hits":3},"error":"","elapsed_ms":412}"#
            .data(using: .utf8)!
        let chunk = ReeveChunk(sequence: 0, type: .toolResult, payload: payload)
