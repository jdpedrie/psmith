...
public enum Fixtures {
    ...
    public static func minimalProfilePatch(name: String = "Test Profile") -> ReeveProfilePatch {
        ReeveProfilePatch(name: name)
    }

    /// Full-fat patch covering every optional field — used by tests that
    /// verify create() round-trips the entire shape.
    public static func fullProfilePatch(name: String = "Full Profile") -> ReeveProfilePatch {
        ReeveProfilePatch(
            name: name,
    ...
    public static func unknownPlugin(ordinal: Int32 = 0) -> ReeveProfilePlugin {
        ReeveProfilePlugin(
    ...
    public static func seedFakeProvider(
        client: ReeveClient,
        label: String = "Fake"
    ) async throws -> (provider: ReeveUserModelProvider, fake: FakeProvider) {
    ...
    ) async throws -> (fake: FakeProvider, provider: ReeveUserModelProvider, model: ReeveUserModel, profile: ReeveProfile) {
