...
/// `update` instead.
public struct ReeveLangfuseConfig: Sendable, Hashable, Codable {
...
/// render it inline.
public struct ReeveLangfuseTestResult: Sendable {
    public let ok: Bool
...
/// the wrapper exists so views consume `ReeveLangfuseConfig`
/// instead of the proto type directly.
public final class LangfuseRepository: Sendable {
    private let client: Reeve_V1_LangfuseServiceClientInterface

    public init(client: Reeve_V1_LangfuseServiceClientInterface) {
        self.client = client
    }
    ...
    /// throws on absence.
    public func get() async throws -> ReeveLangfuseConfig {
        let resp = await client.getLangfuseConfig(request: Reeve_V1_GetLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw ReeveError.from(err) }
        return ReeveLangfuseConfig(from: resp.message?.config ?? Reeve_V1_LangfuseConfig())
    }
    ...
    public func update(
        ...
        enabled: Bool? = nil
    ) async throws -> ReeveLangfuseConfig {
        var req = Reeve_V1_UpdateLangfuseConfigRequest()
    ...
    public func delete() async throws {
        let resp = await client.deleteLangfuseConfig(request: Reeve_V1_DeleteLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw ReeveError.from(err) }
    }
    ...
    public func test() async throws -> ReeveLangfuseTestResult {
        let resp = await client.testLangfuseConfig(request: Reeve_V1_TestLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw ReeveError.from(err) }
        let m = resp.message ?? Reeve_V1_TestLangfuseConfigResponse()
        return ReeveLangfuseTestResult(ok: m.ok, errorMessage: m.errorMessage, latencyMs: m.latencyMs)
    }
}
