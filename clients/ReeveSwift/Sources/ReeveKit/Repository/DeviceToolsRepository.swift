...
/// key.
public struct ReeveSupportedTool: Sendable, Hashable {
    ...
    init(from p: Reeve_V1_SupportedTool) {
        name = p.name
...
public final class DeviceToolsRepository: Sendable {
    private let client: Reeve_V1_DeviceToolsServiceClientInterface
    private let host: URL
    private let tokenStore: TokenStore
    private let session: URLSession

    public init(
        client: Reeve_V1_DeviceToolsServiceClientInterface,
        host: URL,
    ...
    public func registerCapabilities(
        ...
        var req = Reeve_V1_RegisterCapabilitiesRequest()
    ...
    public func listSupportedTools() async throws -> [ReeveSupportedTool] {
        let resp = await client.listSupportedTools(
            request: Reeve_V1_ListSupportedToolsRequest(), headers: [:])
        if let err = resp.error { throw ReeveError.from(err) }
        let tools = resp.message?.tools ?? []
        return tools.map(ReeveSupportedTool.init(from:))
    }
