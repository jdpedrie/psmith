...
/// connected ReeveClient — typically held on the AppModel.
///
/// Lifecycle:
///
...
public final class DeviceToolDispatcher {
    ...
    private let log = Logger(subsystem: "dev.jdpedrie.reeve", category: "DeviceTools")

    public init(client: ReeveClient, registry: DeviceToolRegistry = .shared) {
        self.client = client
    ...
    /// freely.
    public func handleChunk(_ chunk: ReeveChunk, conversationID: String) {
...
private func runHandler(
    ...
    info: ReeveChunk.DeviceToolUseInfo,
    log: Logger
