...
/// Shared editor for `ReeveCallSettings`. Reused by every entry point (provider
...
public struct CallSettingsForm: View {
    ...
    private var thinkingEnabledBinding: Binding<Bool?> {
        Binding(
            ...
            set: { newValue in
                var t = settings.thinking ?? ReeveThinkingSettings()
                t.enabled = newValue
    ...
    private var cacheEnabledBinding: Binding<Bool?> {
        Binding(
            ...
            set: { newValue in
                var a = settings.anthropic ?? ReeveAnthropicExtras()
    ...
    private var cacheTTLBinding: Binding<ReeveCacheTTL?> {
        Binding(
    ...
    private func openaiBinding<T>(_ keyPath: WritableKeyPath<ReeveOpenAIExtras, T>) -> Binding<T> where T: Sendable {
        Binding(
            get: { (settings.openai ?? ReeveOpenAIExtras())[keyPath: keyPath] },
            set: { newValue in
                var o = settings.openai ?? ReeveOpenAIExtras()
    ...
    private var serviceTierBinding: Binding<ReeveServiceTier?> {
        Binding(
    ...
    private func serviceTierLabel(_ t: ReeveServiceTier) -> String {
        switch t {
    ...
    private func responseFormatLabel(_ rf: ReeveResponseFormat) -> String {
        switch rf {
        case .text:        return "Text"
    ...
    private func updateOpenAIResponseFormat(_ rf: ReeveResponseFormat) {
        var o = settings.openai ?? ReeveOpenAIExtras()
        o.responseFormat = rf
        settings.openai = o.isEmpty ? nil : o
    }
    ...
    private func optBoolBinding(_ keyPath: WritableKeyPath<ReeveCallSettings, Bool?>) -> Binding<Bool?> {
        Binding(
    ...
    private func updateGoogle<T>(_ keyPath: WritableKeyPath<ReeveGoogleExtras, T>, _ value: T) {
        var g = settings.google ?? ReeveGoogleExtras()
        g[keyPath: keyPath] = value
        settings.google = g.isEmpty ? nil : g
    }

    private func googleBinding<T>(_ keyPath: WritableKeyPath<ReeveGoogleExtras, T>) -> Binding<T> where T: Sendable {
        Binding(
            get: { (settings.google ?? ReeveGoogleExtras())[keyPath: keyPath] },
            set: { updateGoogle(keyPath, $0) }
        )
    }

    private func googleStringBinding(_ keyPath: WritableKeyPath<ReeveGoogleExtras, String?>) -> Binding<String> {
        Binding(
    ...
    private func googleSafetyBinding(_ keyPath: WritableKeyPath<ReeveSafetySettings, ReeveHarmThreshold?>) -> Binding<ReeveHarmThreshold?> {
        Binding(
            get: { settings.google?.safetySettings?[keyPath: keyPath] },
            set: { newValue in
                var g = settings.google ?? ReeveGoogleExtras()
                var s = g.safetySettings ?? ReeveSafetySettings()
    ...
    private func safetyThresholdRow(
        title: String,
        value: Binding<ReeveHarmThreshold?>,
        inherited: ReeveHarmThreshold?
    ) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                fieldLabel(title)
    ...
    private func harmLabel(_ t: ReeveHarmThreshold) -> String {
        switch t {
