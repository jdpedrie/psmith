...
struct ConversationSettingsView: View {
    ...
    private var addablePluginTypes: [SpaltPluginType] {
        let active = Set(model.resolvedPluginPipeline.map { $0.pluginName })
    ...
    @ViewBuilder
    private func pluginRow(for entry: SpaltResolvedPipelineEntry) -> some View {
    ...
    @ViewBuilder
    private func sourceBadge(for source: SpaltResolvedPipelineSource) -> some View {
    ...
    private func registeredType(forName name: String?) -> SpaltPluginType? {
    ...
    private var effectiveModelCapabilities: SpaltModelCapabilities? {
    ...
    private var effectiveModelConstraints: SpaltModelConstraints? {
        guard let pid = effectiveProviderID, let mid = effectiveModelID else { return nil }
        return model.availableModels
            .first(where: { $0.providerID == pid && $0.modelID == mid })?
            .constraints
...
private struct PluginEditScreen: View {
    ...
    let pluginType: SpaltPluginType?
