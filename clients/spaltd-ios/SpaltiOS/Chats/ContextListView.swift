...
struct ContextListView: View {
    ...
    private var sorted: [SpaltContext] {
    ...
    private func parentLabel(for ctx: SpaltContext) -> String? {
...
private struct NewContextSheet: View {
    ...
    @State private var mode: SpaltCompressionMode = .replace
            ...
            Form {
                ...
                Section {
                    Picker("Prior framing", selection: $mode) {
                        Text("Replace").tag(SpaltCompressionMode.replace)
                        Text("Append").tag(SpaltCompressionMode.append)
                    }
