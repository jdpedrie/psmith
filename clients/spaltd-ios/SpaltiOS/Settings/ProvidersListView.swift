...
struct ProvidersListView: View {
    ...
    @State private var deleteCandidate: SpaltUserModelProvider?
    @State private var addingTemplate: SpaltProviderTemplate?
    ...
    private var availableTemplates: [SpaltProviderTemplate] {
        // SpaltUserModelProvider doesn't store catalogProviderID
    ...
    private func catalogKey(for provider: SpaltUserModelProvider) -> String {
    ...
    @ViewBuilder
    private func configuredRow(_ provider: SpaltUserModelProvider) -> some View {
        HStack(spacing: 10) {
    ...
    @ViewBuilder
    private func availableRow(_ template: SpaltProviderTemplate) -> some View {
        HStack(spacing: 10) {
    ...
    private func logoSlug(for provider: SpaltUserModelProvider) -> String? {
...
private struct ProviderDetailView: View {
    ...
    let initialProvider: SpaltUserModelProvider
    @Environment(AppModel.self) private var app

    init(provider: SpaltUserModelProvider) {
    ...
    private var provider: SpaltUserModelProvider {
        app.providers.providers.first(where: { $0.id == initialProvider.id }) ?? initialProvider
    }
    @State private var editing: Bool = false
    @State private var modelSettingsTarget: SpaltUserModel?
    @State private var testResultMessage: String?
    @State private var disableCandidate: SpaltUserModel?
    ...
    @ViewBuilder
    private func modelRow(_ m: SpaltUserModel) -> some View {
...
private struct ProviderDefaultSettingsScreen: View {
    ...
    @State private var draft: SpaltCallSettings = SpaltCallSettings()
    ...
    var body: some View {
        ...
        .onAppear {
            if !seeded {
                draft = provider.defaultSettings ?? SpaltCallSettings()
                seeded = true
            }
        }
        .onDisappear {
    ...
    private func save() async {
        ...
        do {
            ...
            app.providers.error = "Saving provider defaults failed: \(SpaltError.display(err))"
        }
    }
}
...
private struct ModelDefaultSettingsSheet: View {
    let initialProvider: SpaltUserModelProvider
    let model: SpaltUserModel
    @Environment(AppModel.self) private var app

    init(provider: SpaltUserModelProvider, model: SpaltUserModel) {
        self.initialProvider = provider
        self.model = model
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                if !seeded {
                    draft = model.defaultSettings ?? SpaltCallSettings()
                    seeded = true
                }
            }
        }
...
private struct DiscoverModelsScreen: View {
    let provider: SpaltUserModelProvider
    @Environment(AppModel.self) private var app
    @State private var discovered: [SpaltDiscoveredModel] = []
    ...
    @MainActor
    private func enableOne(_ m: SpaltDiscoveredModel) async {
        ...
        do {
            ...
            self.error = SpaltError.display(error)
        }
    }
}
...
private struct AddProviderSheet: View {
    ...
    let template: SpaltProviderTemplate?
    @Environment(AppModel.self) private var app
    @Environment(\.dismiss) private var dismiss
