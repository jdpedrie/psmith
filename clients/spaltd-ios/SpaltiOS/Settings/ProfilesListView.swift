...
struct ProfilesListView: View {
    ...
    @State private var deleteCandidate: SpaltProfile?
    ...
    var body: some View {
        ...
        // edit made elsewhere — another client, the Spalt Manager via
    ...
    private var sortedProfiles: [SpaltProfile] {
    ...
    @ViewBuilder
    private func row(_ profile: SpaltProfile) -> some View {
        HStack(spacing: 8) {
...
private struct ProfileViewerScreen: View {
    ...
    private var profile: SpaltProfile? {
        ...
        Group {
            if let p = profile {
                Form {
                    ...
                    if p.titleProviderKind == SpaltTitleProviderKind.appleFoundation {
    ...
    private func callSettingsSummary(_ cs: SpaltCallSettings) -> String {
        var parts: [String] = []
...
private struct ProfileEditSheet: View {
    ...
    // Compression
    @State private var compressionMode: SpaltCompressionMode? = nil
    ...
    // a Form.Section.
    @State private var callSettingsDraft: SpaltCallSettings = SpaltCallSettings()
    ...
    @State private var pluginsBaseline: [SpaltProfilePlugin] = []
    ...
    private var existing: SpaltProfile? {
            ...
            Form {
                ...
                Section("Compression") {
                    Picker("Mode", selection: $compressionMode) {
                        Text("Inherit").tag(SpaltCompressionMode?.none)
                        Text("Replace").tag(SpaltCompressionMode?.some(.replace))
                        Text("Append").tag(SpaltCompressionMode?.some(.append))
                    }
                    .pickerStyle(.menu)
                ...
                Section("Auto-titling") {
                    Picker("Generator", selection: $titleProviderKind) {
                        ...
                        if AppleFoundation.isAvailable {
                            Text("Apple Foundation (on-device)").tag(String?.some(SpaltTitleProviderKind.appleFoundation))
                        } else {
                            Text("Apple Foundation — not available on this device")
                                .foregroundStyle(.tertiary)
                                .tag(String?.some(SpaltTitleProviderKind.appleFoundation))
                        }
                    }
                    .pickerStyle(.menu)
                    ...
                    if titleProviderKind != SpaltTitleProviderKind.appleFoundation {
                        modelPickerRow(
                            label: "Cloud model",
    ...
    private var unattachedPluginTypes: [SpaltPluginType] {
    ...
    private func attachPlugin(_ pluginType: SpaltPluginType) {
    ...
    private var isDirty: Bool {
        ...
        return name != p.name
            ...
            || callSettingsDraft != (p.defaultSettings?.callSettings ?? SpaltCallSettings())
    ...
    private var inheritedCallSettings: SpaltCallSettings? {
        var resolved = SpaltCallSettings()
    ...
    private var defaultModelCapabilities: SpaltModelCapabilities? {
    ...
    private func save() async {
        ...
        do {
            ...
            // changed.
            if pluginsAreDirty {
                let plugins: [SpaltProfilePlugin] = try pluginsDraft.enumerated().map { ordinal, plugin in
                    let data = try JSONSerialization.data(withJSONObject: plugin.config, options: [.sortedKeys])
                    return SpaltProfilePlugin(
                        pluginName: plugin.pluginName,
...
private struct ParentPickerSheet: View {
    ...
    private var eligibleParents: [SpaltProfile] {
