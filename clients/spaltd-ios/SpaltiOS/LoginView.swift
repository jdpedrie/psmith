...
private struct ServerURLEntry: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Enter the URL where your Spalt server is running. We'll probe it to confirm before asking for your credentials.")
                .font(.callout)
                .foregroundStyle(.secondary)
            ...
            case .ok(let version):
                banner(
                    ...
                    text: version.isEmpty
                        ? "Reachable — spaltd answered."
                        : "Reachable — spaltd \(version)."
                )
            case .wrong(let detail):
                banner(
                    tint: .orange,
                    icon: "exclamationmark.triangle.fill",
                    text: "Server answered but isn't a Spalt server: \(detail)"
                )
