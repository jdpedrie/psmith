...
struct AppModelTests {
    ...
    func bootstrapRestoresSession() async throws {
        ...
        // expose a direct reader for tokenStore on ClarkClient, so seed a
    ...
    private func mintSessionToken(server: TestClarkdServer, username: String, password: String) async throws -> String {
    ...
    private func rawLoginToken(server: TestClarkdServer, username: String, password: String) async throws -> String {
        ...
        guard let msg = resp.message else {
            throw ClarkError.from(resp.error ?? ConnectError(code: .unknown, message: "login failed"))
        }
