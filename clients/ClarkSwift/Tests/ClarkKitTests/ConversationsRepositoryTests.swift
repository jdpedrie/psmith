...
/// clarkd subprocess. Covers tests #1–#35 from the testing plan.
///
...
struct ConversationsRepositoryTests {
    ...
    func createWithSettings() async throws {
        ...
        _ = fake
        let settings = ClarkConversationSettings(
            defaultProviderID: provider.id,
            defaultModelID: model.modelID,
            includeThinkingInHistory: true,
            callSettings: ClarkCallSettings(temperature: 0.55)
        )
    ...
    func updateTitleEmptyClears() async throws {
        ...
        // `optional title = ""`, which round-trips through ClarkConversation
    ...
    func updateSettingsReplacesBlob() async throws {
        ...
        let (fake, provider, model, profile) = try await Fixtures.seedReadyToChat(client: client)
        _ = fake
        let initial = ClarkConversationSettings(
            defaultProviderID: provider.id,
            defaultModelID: model.modelID,
            includeThinkingInHistory: true
        )
        ...
        let replacement = ClarkConversationSettings(includeThinkingInHistory: false)
    ...
    private func drainStream(client: ClarkClient, runID: String) async throws {
    ...
    /// We don't have a "list streams" API surfaced on ClarkKit, so we just
    /// give the server up to ~3s before returning. Good enough for tests.
    private func waitNoActiveStream(client: ClarkClient, conversationID: String) async throws {
