...
struct ProfilesRepositoryTests {
    ...
    func getResolvedFromParent() async throws {
        ...
        let parent = try await client.profiles.create(ClarkProfilePatch(
            name: "Parent",
            systemMessage: "I am the parent system prompt.",
            parentOnly: true
        ))
        let child = try await client.profiles.create(ClarkProfilePatch(
            name: "Child",
    ...
    @Test("create with full ClarkProfilePatch round-trips every field")
    func createFullPatch() async throws {
        let (client, _) = try await TestSession.freshUser(server: server, usernamePrefix: "prof-full")
        let parent = try await client.profiles.create(ClarkProfilePatch(name: "Parent", parentOnly: true))
        let patch = ClarkProfilePatch(
            name: "Child",
            parentProfileID: parent.id,
        ...
        let titled = try await client.profiles.create(ClarkProfilePatch(
            name: "Titled",
            titleGuide: "Use 4 words.",
            titleProviderKind: ClarkTitleProviderKind.appleFoundation
        ))
        #expect(titled.titleGuide == "Use 4 words.")
        #expect(titled.titleProviderKind == ClarkTitleProviderKind.appleFoundation)
    }
    ...
    func createParentOnly() async throws {
        ...
        let created = try await client.profiles.create(
            ClarkProfilePatch(name: "Trunk", parentOnly: true)
        )
    ...
    func createFavorite() async throws {
        ...
        let created = try await client.profiles.create(
            ClarkProfilePatch(name: "Star", favorite: true)
        )
        #expect(created.favorite == true)
    }
    ...
    func createEmptyName() async throws {
        ...
        do {
            _ = try await client.profiles.create(ClarkProfilePatch(name: ""))
            Issue.record("expected InvalidArgument")
        } catch let ClarkError.rpc(code, _) {
            #expect(code == .invalidArgument)
        }
    ...
    func updatePartialPatch() async throws {
        ...
        let created = try await client.profiles.create(ClarkProfilePatch(
            name: "Before",
        ...
        let updated = try await client.profiles.update(
            id: created.id,
            patch: ClarkProfilePatch(name: "After")
        )
    ...
    func updateNotFoundCrossUser() async throws {
        ...
        do {
            _ = try await clientB.profiles.update(
                id: owned.id, patch: ClarkProfilePatch(name: "stolen")
            )
    ...
    func deleteWithChildrenFailsPrecondition() async throws {
        ...
        let parent = try await client.profiles.create(ClarkProfilePatch(name: "Parent", parentOnly: true))
        _ = try await client.profiles.create(ClarkProfilePatch(
