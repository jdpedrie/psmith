...
/// Boots a real clarkd subprocess against an isolated, freshly-migrated
...
///   2. `<repoRoot>/bin/clarkd-test` (pre-built by the harness on first call),
///   3. otherwise, `go build -o bin/clarkd-test ./cmd/clarkd` is run lazily.
///
/// The server is created lazily via `TestClarkdServer.shared()` so XCTest's
...
public final class TestClarkdServer: @unchecked Sendable {
    ...
    nonisolated(unsafe) private static var sharedInstance: TestClarkdServer?

    /// Returns the process-wide shared server, booting it on first call.
    /// Subsequent callers wait for the boot to complete.
    public static func shared() throws -> TestClarkdServer {
        lock.lock()
        defer { lock.unlock() }
        if let s = sharedInstance { return s }
        let server = try TestClarkdServer.start()
        sharedInstance = server
        // Tear down on process exit. atexit can't capture self by reference,
        // so we read from the static slot.
        atexit {
            TestClarkdServer.lock.lock()
            let s = TestClarkdServer.sharedInstance
            TestClarkdServer.sharedInstance = nil
            TestClarkdServer.lock.unlock()
            s?.shutdown()
        }
        return server
    }
    ...
    public static func start() throws -> TestClarkdServer {
        ...
        let dbName = "clark_swift_test_\(UUID().uuidString.lowercased().replacingOccurrences(of: "-", with: ""))"
    ...
    private func waitUntilReady(timeoutSeconds: Double) throws {
        ...
        while Date() < deadline {
            if !process.isRunning {
                ...
                throw HarnessError.serverFailedToStart("clarkd exited before becoming ready:\n\(err)")
            }
        ...
        throw HarnessError.serverFailedToStart(
            "clarkd did not bind \(port) within \(timeoutSeconds)s. stderr:\n\(stderrBuffer.snapshotString())"
        )
    }
    ...
    public func shutdown() {
        ...
        do {
            try TestClarkdServer.dropDatabase(
                dbName: dbName,
...
enum HarnessError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .binaryBuildFailed(let s): return "go build of clarkd failed: \(s)"
...
private func ensureBinary(repoRoot: String) throws -> String {
    ...
    let binPath = (repoRoot as NSString).appendingPathComponent("bin/clarkd-test")
    let fm = FileManager.default
    if fm.fileExists(atPath: binPath) {
        return binPath
    }
    // Build it: `go build -o bin/clarkd-test ./cmd/clarkd`.
    ...
    proc.arguments = ["go", "build", "-o", "bin/clarkd-test", "./cmd/clarkd"]
    proc.currentDirectoryURL = URL(fileURLWithPath: repoRoot)
    let pipe = Pipe()
