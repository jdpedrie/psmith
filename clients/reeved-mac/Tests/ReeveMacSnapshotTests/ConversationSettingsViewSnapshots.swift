...
struct ConversationSettingsViewSnapshots {
    ...
    private func conversationFor(providerID: String, modelID: String) -> ReeveConversation {
        var settings = ReeveConversationSettings()
        settings.defaultProviderID = providerID
        settings.defaultModelID = modelID
        return ReeveConversation(
            id: "conv-1",
    ...
    func noOverrides() {
        ...
        let inherited = ReeveCallSettings(
            temperature: 1.0,
            topP: 0.95,
            maxOutputTokens: 4_096,
            stopSequences: [],
            topK: 50,
            thinking: ReeveThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
            conversationCallSettingsDraft: ReeveCallSettings(),
    ...
    func temperatureOverride() {
        var draft = ReeveCallSettings()
        draft.temperature = 0.45
        let inherited = ReeveCallSettings(temperature: 1.0, topP: 0.95, maxOutputTokens: 4_096)
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
    ...
    func thinkingOverride() {
        var draft = ReeveCallSettings()
        draft.thinking = ReeveThinkingSettings(enabled: true, budgetTokens: 4_096)
        let inherited = ReeveCallSettings(
            temperature: 1.0,
            thinking: ReeveThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
    ...
    func anthropicProvider() {
        ...
        var draft = ReeveCallSettings()
        draft.anthropic = ReeveAnthropicExtras(cacheEnabled: false, cacheTTL: .oneHour)
        let inherited = ReeveCallSettings(
            temperature: 1.0,
            anthropic: ReeveAnthropicExtras(cacheEnabled: true, cacheTTL: .fiveMinutes)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func openaiProvider() {
        ...
        let openaiModel = ReeveUserModel(
        ...
        var draft = ReeveCallSettings()
        draft.openai = ReeveOpenAIExtras(seed: 42, frequencyPenalty: 0.5)
        let inherited = ReeveCallSettings(
            temperature: 0.8,
            openai: ReeveOpenAIExtras(seed: 0, frequencyPenalty: 0)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func googleProvider() {
        ...
        let googleModel = ReeveUserModel(
        ...
        var draft = ReeveCallSettings()
        draft.google = ReeveGoogleExtras(
        ...
        let inherited = ReeveCallSettings(
            temperature: 0.7,
            google: ReeveGoogleExtras(
                safetySettings: ReeveSafetySettings(harassment: .blockNone),
                candidateCount: 1
            )
        )
        let model = SnapshotStubs.makeConversationViewModel(
