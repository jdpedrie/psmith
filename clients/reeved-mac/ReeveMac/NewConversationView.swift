...
struct NewConversationView: View {
    ...
    @State private var conversationCallSettings = ReeveCallSettings()
    @State private var resolvedProfileForInherit: ReeveProfile?
    ...
    private var modelRowList: some View {
        ...
        let grouped: [(provider: String, models: [ReeveUserModel])] = Dictionary(grouping: nonFavorites) { $0.providerID }
    ...
    private var newConversationModelCapabilities: ReeveModelCapabilities? {
        let pid = overrideProviderID ?? resolvedProfileForInherit?.defaultSettings?.defaultProviderID
