...
/// pricing isn't known.
private func costBucket(_ pricing: ReeveModelPricing?) -> String? {
...
/// both enabled-model rows (ReeveUserModel) and discovery rows
/// (ReeveDiscoveredModel) feed the same widget without making each
/// caller inflate a full ReeveUserModel.
struct ModelMetaSnapshot {
    ...
    let pricing: ReeveModelPricing?
    let knowledgeCutoff: String?
    let modalities: [String]
    let capabilities: ReeveModelCapabilities?
}
...
struct ModelMetaStrip: View {
    ...
    private var pricing: ReeveModelPricing? { snapshot.pricing }
