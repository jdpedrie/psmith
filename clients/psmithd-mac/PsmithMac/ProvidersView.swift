...
private struct ModelTestResultChip: View {
    ...
    private func successText(_ r: PsmithModelTestResult) -> String {
        var bits: [String] = []
...
private enum AddProviderSelection: Equatable {
    case template(PsmithProviderTemplate)
    case custom
}
...
private struct EditProviderForm: View {
    ...
    private var openAITemplates: [PsmithProviderTemplate] {
...
private struct DiscoverModelsInline: View {
    ...
    private var filtered: [PsmithDiscoveredModel] {
...
private struct DiscoveredModelRow: View {
    let model: PsmithDiscoveredModel
    let isSelected: Bool
    let onToggle: () -> Void
    @Environment(\.theme) private var theme
...
private struct ProviderDefaultSettingsTab: View {
    let provider: PsmithUserModelProvider
    @Bindable var model: ProvidersViewModel
    @State private var draft = PsmithCallSettings()
    ...
    var body: some View {
        ...
        .onDisappear {
            ...
            let original = provider.defaultSettings ?? PsmithCallSettings()
...
enum ModelEditMode {
    case adding(provider: PsmithUserModelProvider)
    case editing(model: PsmithUserModel, provider: PsmithUserModelProvider)

    var provider: PsmithUserModelProvider {
        switch self {
    ...
    var existingModel: PsmithUserModel? {
...
private struct ModelEditForm: View {
    ...
    private var provider: PsmithUserModelProvider { formMode.provider }
    ...
    /// Builds a PsmithModelPricing from the four price text fields. Returns nil
    ...
    private var capabilitiesValue: PsmithModelCapabilities {
    ...
    private func seedFromExistingIfNeeded() {
        ...
        defaultSettings = m.defaultSettings ?? PsmithCallSettings()
    }
