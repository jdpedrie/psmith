...
struct ConversationSettingsViewSnapshots {
    ...
    func noOverrides() {
        ...
        let inherited = PsmithCallSettings(
            temperature: 1.0,
            topP: 0.95,
            maxOutputTokens: 4_096,
            stopSequences: [],
            topK: 50,
            thinking: PsmithThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
            conversationCallSettingsDraft: PsmithCallSettings(),
    ...
    func temperatureOverride() {
        ...
        let inherited = PsmithCallSettings(temperature: 1.0, topP: 0.95, maxOutputTokens: 4_096)
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
    ...
    func thinkingOverride() {
        var draft = PsmithCallSettings()
        draft.thinking = PsmithThinkingSettings(enabled: true, budgetTokens: 4_096)
        let inherited = PsmithCallSettings(
            temperature: 1.0,
            thinking: PsmithThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
    ...
    func anthropicProvider() {
        ...
        var draft = PsmithCallSettings()
        draft.anthropic = PsmithAnthropicExtras(cacheEnabled: false, cacheTTL: .oneHour)
        let inherited = PsmithCallSettings(
            temperature: 1.0,
            anthropic: PsmithAnthropicExtras(cacheEnabled: true, cacheTTL: .fiveMinutes)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func openaiProvider() {
        ...
        let openaiModel = PsmithUserModel(
        ...
        var draft = PsmithCallSettings()
        draft.openai = PsmithOpenAIExtras(seed: 42, frequencyPenalty: 0.5)
        let inherited = PsmithCallSettings(
            temperature: 0.8,
            openai: PsmithOpenAIExtras(seed: 0, frequencyPenalty: 0)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func googleProvider() {
        ...
        let googleModel = PsmithUserModel(
        ...
        var draft = PsmithCallSettings()
        draft.google = PsmithGoogleExtras(
        ...
        let inherited = PsmithCallSettings(
            temperature: 0.7,
            google: PsmithGoogleExtras(
                safetySettings: PsmithSafetySettings(harassment: .blockNone),
                candidateCount: 1
            )
        )
        let model = SnapshotStubs.makeConversationViewModel(
