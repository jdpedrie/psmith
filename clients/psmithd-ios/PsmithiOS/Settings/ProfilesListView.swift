...
struct ProfilesListView: View {
    ...
    private var sortedProfiles: [PsmithProfile] {
    ...
    @ViewBuilder
    private func row(_ profile: PsmithProfile) -> some View {
        HStack(spacing: 8) {
...
private struct ProfileViewerScreen: View {
    ...
    private var profile: PsmithProfile? {
        ...
        Group {
            if let p = profile {
                Form {
                    ...
                    if p.titleProviderKind == PsmithTitleProviderKind.appleFoundation {
    ...
    private func callSettingsSummary(_ cs: PsmithCallSettings) -> String {
        var parts: [String] = []
...
private struct ProfileEditSheet: View {
    ...
    // a Form.Section.
    @State private var callSettingsDraft: PsmithCallSettings = PsmithCallSettings()
                ...
                Section("Auto-titling") {
                    Picker("Generator", selection: $titleProviderKind) {
                        ...
                        if AppleFoundation.isAvailable {
                            Text("Apple Foundation (on-device)").tag(String?.some(PsmithTitleProviderKind.appleFoundation))
                        } else {
                            Text("Apple Foundation — not available on this device")
                                .foregroundStyle(.tertiary)
                                .tag(String?.some(PsmithTitleProviderKind.appleFoundation))
                        }
                    }
                    .pickerStyle(.menu)
    ...
    private var unattachedPluginTypes: [PsmithPluginType] {
    ...
    private var inheritedCallSettings: PsmithCallSettings? {
        var resolved = PsmithCallSettings()
    ...
    private var defaultModelCapabilities: PsmithModelCapabilities? {
    ...
    private func save() async {
        ...
        let defaults = PsmithProfileDefaults(
...
private struct ParentPickerSheet: View {
    ...
    private var eligibleParents: [PsmithProfile] {
