...
struct PluginsListView: View {
        ...
        Group {
            if !loaded {
                ...
                EmptyStateView(
                    ...
                    description: "The server's binary was built without any plugins registered. Check your psmithd build."
                )
            } else {
                List {
    ...
    @ViewBuilder
    private func pluginRow(_ plugin: PsmithPluginType) -> some View {
        HStack(spacing: 10) {
    ...
    private func hasUnsetRequiredGlobals(_ plugin: PsmithPluginType) -> Bool {
