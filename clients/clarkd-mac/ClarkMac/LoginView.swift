        ...
        Group {
            ...
            case .server:
                ServerURLEntry(onValidated: { newURL in
                    // Persisting triggers ClarkMacApp's onChange to
...
private struct ServerURLEntry: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("Connect to clarkd")
                .font(.title2)
            Text("Enter the URL where your Clark server is running.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)

            VStack(alignment: .leading, spacing: 8) {
                TextField("https://clark.example.com", text: $urlText)
    ...
    private var statusBanner: some View {
        if inFlight {
            ...
            case .idle:
                EmptyView()
            case .ok(let version):
                Label(version.isEmpty ? "Reachable — clarkd answered." : "Reachable — clarkd \(version).",
                      systemImage: "checkmark.circle.fill")
                    .font(.callout)
                    .foregroundStyle(.green)
            case .wrong(let detail):
                Label("Server responded but isn't a Clark server: \(detail)",
...
private struct CredentialsEntry: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            // which clarkd they're authenticating against; clicking
    ...
    private func submit() async {
        ...
        do {
            ...
            errorMessage = ClarkError.display(error)
        }
        inFlight = false
    }
}
