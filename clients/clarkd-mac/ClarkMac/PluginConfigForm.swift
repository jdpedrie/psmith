...
/// Renders one row per `ClarkConfigField` against a shared `[String: Any]`
...
struct PluginConfigForm: View {
    ...
    @ViewBuilder
    private func fieldRow(_ field: ClarkConfigField) -> some View {
    ...
    @ViewBuilder
    private func control(for field: ClarkConfigField) -> some View {
        switch field.type {
    ...
    private func numberField(_ field: ClarkConfigField) -> some View {
    ...
    private func textField(_ field: ClarkConfigField) -> some View {
        let binding = Binding<String>(
            get: {
    ...
    private func textareaField(_ field: ClarkConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
                return decodeDefaultString(field) ?? ""
    ...
    private func booleanField(_ field: ClarkConfigField) -> some View {
    ...
    @ViewBuilder
    private func selectField(_ field: ClarkConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
    ...
    private func decodeDefaultString(_ field: ClarkConfigField) -> String? {
    ...
    private func decodeDefaultBool(_ field: ClarkConfigField) -> Bool? {
        guard !field.defaultJSON.isEmpty,
              let data = field.defaultJSON.data(using: .utf8) else { return nil }
