...
/// Pre-built ClarkKit domain models for snapshot tests. Every value is
...
public enum SnapshotFixtures {
    ...
    public static func user(
        ...
        ClarkUser(id: id, username: username, displayName: displayName, isAdmin: isAdmin)
    }
    ...
    public static func userModel(
        ...
        favorite: Bool = true,
        capabilities: ClarkModelCapabilities? = ClarkModelCapabilities(
    ...
    /// Note: `ClarkDiscoveredModel` doesn't expose a public memberwise
    ...
    /// fixture, or extend ClarkKit when that test ships.
    ...
    public static func conversations(profileID: String = "profile-default") -> [ClarkConversation] {
        [
    ...
    public static func contextChain(conversationID: String = "conv-1") -> [ClarkContext] {
        let root = context(
            id: "context-1",
            conversationID: conversationID,
    ...
    public static func assistantMessage(
        ...
        providerID: String? = "provider-anthropic",
        modelID: String? = "claude-opus-4-7",
        usage: ClarkMessageUsage? = SnapshotFixtures.standardUsage()
    ) -> ClarkMessage {
        ClarkMessage(
            id: id,
            contextID: contextID,
    ...
    public static func standardUsage() -> ClarkMessageUsage {
        ClarkMessageUsage(
            inputTokens: 1_280,
            outputTokens: 462,
            cacheReadTokens: 980,
    ...
    /// without scrolling.
    public static func sampleMessages() -> [ClarkMessage] {
        [
            systemMessage(),
            userMessage(),
            assistantMessage(),
    ...
    public static func pluginType(
        ...
        configFields: [ClarkConfigField] = SnapshotFixtures.letteredChoicesConfigFields()
    ) -> ClarkPluginType {
        ClarkPluginType(
            name: name,
            description: description,
    ...
    public static func letteredChoicesConfigFields() -> [ClarkConfigField] {
        [
            numberConfigField(
                name: "keep_last_n",
                display: "Keep last N",
    ...
    public static func selectShortConfigField(
        ...
        ClarkConfigField(
            ...
            defaultJSON: defaultJSON,
            options: [
                ClarkConfigOption(value: "compact", label: "Compact"),
                ClarkConfigOption(value: "verbose", label: "Verbose"),
                ClarkConfigOption(value: "academic", label: "Academic"),
            ]
        )
    }
    ...
    public static func selectLongConfigField(
        ...
        ClarkConfigField(
            ...
            defaultJSON: defaultJSON,
            options: [
                ClarkConfigOption(value: "neutral",   label: "Neutral"),
                ClarkConfigOption(value: "playful",   label: "Playful"),
                ClarkConfigOption(value: "formal",    label: "Formal"),
                ClarkConfigOption(value: "academic",  label: "Academic"),
                ClarkConfigOption(value: "concise",   label: "Concise"),
                ClarkConfigOption(value: "elaborate", label: "Elaborate"),
            ]
        )
    }
    ...
    public static func capabilitiesAllOn() -> ClarkModelCapabilities {
    ...
    public static func capabilitiesNoThinking() -> ClarkModelCapabilities {
        ClarkModelCapabilities(
    ...
    public static func commonInheritedCallSettings() -> ClarkCallSettings {
        ClarkCallSettings(
            temperature: 0.7,
            topP: 0.95,
            ...
            stopSequences: ["</done>"],
            topK: 40,
            thinking: ClarkThinkingSettings(enabled: true, budgetTokens: 8_192)
        )
    }

    /// Explicit Anthropic-driver extras, every field set so the section
    /// renders fully populated when used as the active settings.
    public static func anthropicExtrasCallSettings() -> ClarkCallSettings {
        var s = ClarkCallSettings()
        s.anthropic = ClarkAnthropicExtras(cacheEnabled: true, cacheTTL: .oneHour)
        s.thinking  = ClarkThinkingSettings(enabled: true, budgetTokens: 12_000)
        return s
    }
    ...
    public static func openaiExtrasCallSettings() -> ClarkCallSettings {
        var s = ClarkCallSettings()
        s.openai = ClarkOpenAIExtras(
            seed: 42,
    ...
    public static func googleExtrasCallSettings() -> ClarkCallSettings {
        var s = ClarkCallSettings()
        s.google = ClarkGoogleExtras(
    ...
    public static func profileChain() -> [ClarkProfile] {
        let root = profile(
            id: "profile-root",
    ...
    /// Anthropic default.
    public static func providers() -> [ClarkUserModelProvider] {
        [
    ...
    public static func enabledModels() -> [ClarkUserModel] {
        [
            ClarkUserModel(
                providerID: "provider-anthropic",
                modelID: "claude-opus-4-7",
    ...
    public static func providerTemplates() -> [ClarkProviderTemplate] {
        [
            ClarkProviderTemplate(
    ...
    public static func discoveredModels() -> [ClarkDiscoveredModel] {
        [
            ClarkDiscoveredModel(
                modelID: "claude-opus-4-7",
                displayName: "Claude Opus 4.7",
                contextWindow: 200_000,
                pricing: ClarkModelPricing(inputPerMillion: 15.0, outputPerMillion: 75.0, cacheReadPerMillion: 1.5, cacheWritePerMillion: 18.75),
                capabilities: ClarkModelCapabilities(streaming: true, thinking: true, toolUse: true, vision: true, promptCaching: true),
                alreadyEnabled: true
            ),
            ClarkDiscoveredModel(
                ...
                contextWindow: 200_000,
                pricing: ClarkModelPricing(inputPerMillion: 3.0, outputPerMillion: 15.0, cacheReadPerMillion: 0.30, cacheWritePerMillion: 3.75),
                capabilities: ClarkModelCapabilities(streaming: true, thinking: true, toolUse: true, vision: true, promptCaching: true),
            ...
            ClarkDiscoveredModel(
                ...
                displayName: "Claude Haiku 4.7",
                contextWindow: 200_000,
                pricing: ClarkModelPricing(inputPerMillion: 1.0, outputPerMillion: 5.0, cacheReadPerMillion: 0.10, cacheWritePerMillion: 1.25),
                capabilities: ClarkModelCapabilities(streaming: true, thinking: false, toolUse: true, vision: true, promptCaching: true),
                alreadyEnabled: true
            ),
            ClarkDiscoveredModel(
            ...
            ClarkDiscoveredModel(
                ...
                contextWindow: 200_000,
                pricing: ClarkModelPricing(inputPerMillion: 0.25, outputPerMillion: 1.25, cacheReadPerMillion: nil, cacheWritePerMillion: nil),
                capabilities: ClarkModelCapabilities(streaming: true, thinking: false, toolUse: true, vision: true, promptCaching: false),
                alreadyEnabled: true
            ),
        ]
    }

    /// One ClarkProfilePlugin attached to a profile. JSON config encodes
    ...
    public static func attachedPlugin(
        ...
        return ClarkProfilePlugin(pluginName: pluginName, ordinal: 0, config: data)
    }
}
