...
/// Builders for fully-populated ClarkKit view models that drive snapshot
/// rendering without ever calling out to a clarkd server.
///
/// Strategy: every `@Observable` view model in ClarkKit exposes its
/// pre-load state through public `var` properties. We construct a real
/// instance against a `ClarkClient` aimed at a non-routable host (so any
...
///
/// ClarkMac-internal observables (`Navigator`, `WindowState`, `AppMode`)
/// live in the executable target — the test target wires those up itself
/// after `@testable import ClarkMac`.
public enum SnapshotStubs {
    /// Fake host URL used for the no-op ClarkClient. The host won't be
    ...
    @MainActor
    public static func makeClient() -> ClarkClient {
        ClarkClient(
            host: nullHost,
    ...
    public static func makeAppModel(
        providers: [ClarkUserModelProvider] = [SnapshotFixtures.userModelProvider()],
        models: [ClarkUserModel] = [SnapshotFixtures.userModel()],
    ...
    public static func makeConversationViewModel(
        conversation: ClarkConversation = SnapshotFixtures.conversation(),
        client: ClarkClient? = nil,
        messages: [ClarkMessage] = SnapshotFixtures.sampleMessages(),
        contexts: [ClarkContext] = [SnapshotFixtures.context()],
        availableModels: [ClarkUserModel] = [SnapshotFixtures.userModel()],
        ...
        editingMessage: ClarkMessage? = nil,
        conversationCallSettingsDraft: ClarkCallSettings = ClarkCallSettings(),
        resolvedCallSettings: ClarkCallSettings? = nil,
        settingsResolvedProfile: ClarkProfile? = nil,
    ...
    public static func makeProvidersModel(
        providers: [ClarkUserModelProvider] = SnapshotFixtures.providers(),
        enabledModels: [ClarkUserModel] = SnapshotFixtures.enabledModels(),
        selectedID: String? = "provider-anthropic",
        detailMode: ProvidersDetailMode = .viewing,
        templates: [ClarkProviderTemplate] = SnapshotFixtures.providerTemplates(),
    ...
    public static func makeProfilesModel(
        ...
        availableModels: [ClarkUserModel] = SnapshotFixtures.enabledModels(),
        providers: [ClarkUserModelProvider] = SnapshotFixtures.providers(),
        pluginTypes: [ClarkPluginType] = [SnapshotFixtures.pluginType()],
        profilePlugins: [String: [ClarkProfilePlugin]] = [:]
    ) -> ProfilesViewModel {
