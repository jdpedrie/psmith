...
/// Boots a real spaltd subprocess against an isolated, freshly-migrated
...
///   1. `SPALT_TEST_BIN` env var if set,
///   2. `<repoRoot>/bin/spaltd-test` (pre-built by the harness on first call),
///   3. otherwise, `go build -o bin/spaltd-test ./cmd/spaltd` is run lazily.
///
/// The server is created lazily via `TestSpaltdServer.shared()` so XCTest's
...
public final class TestSpaltdServer: @unchecked Sendable {
    ...
    nonisolated(unsafe) private static var sharedInstance: TestSpaltdServer?

    /// Returns the process-wide shared server, booting it on first call.
    /// Subsequent callers wait for the boot to complete.
    public static func shared() throws -> TestSpaltdServer {
        lock.lock()
        defer { lock.unlock() }
        if let s = sharedInstance { return s }
        let server = try TestSpaltdServer.start()
        sharedInstance = server
        // Tear down on process exit. atexit can't capture self by reference,
        // so we read from the static slot.
        atexit {
            TestSpaltdServer.lock.lock()
            let s = TestSpaltdServer.sharedInstance
            TestSpaltdServer.sharedInstance = nil
            TestSpaltdServer.lock.unlock()
            s?.shutdown()
        }
        return server
    }
    ...
    public static func start() throws -> TestSpaltdServer {
        ...
        let dbName = "spalt_swift_test_\(UUID().uuidString.lowercased().replacingOccurrences(of: "-", with: ""))"
        ...
        env["SPALT_DSN"] = testDSN
        env["SPALT_ADDR"] = listenAddr
        env["SPALT_BOOTSTRAP_ADMIN_USERNAME"] = "admin"
        env["SPALT_BOOTSTRAP_ADMIN_PASSWORD"] = "admin-password-not-secret"
        // Disable the catalog refresh loop — tests don't need it and it
        // makes outbound network calls.
        env["SPALT_CATALOG_REFRESH_INTERVAL"] = "0"
        ...
        let server = TestSpaltdServer(
            ...
            dsn: testDSN,
            dbName: dbName,
            adminUsername: env["SPALT_BOOTSTRAP_ADMIN_USERNAME"]!,
            adminPassword: env["SPALT_BOOTSTRAP_ADMIN_PASSWORD"]!,
            process: proc,
            stdoutPipe: outPipe,
            stderrPipe: errPipe,
            stdoutBuffer: outBuf,
    ...
    private func waitUntilReady(timeoutSeconds: Double) throws {
        ...
        while Date() < deadline {
            if !process.isRunning {
                ...
                throw HarnessError.serverFailedToStart("spaltd exited before becoming ready:\n\(err)")
            }
        ...
        throw HarnessError.serverFailedToStart(
            "spaltd did not bind \(port) within \(timeoutSeconds)s. stderr:\n\(stderrBuffer.snapshotString())"
        )
    }
    ...
    public func shutdown() {
        ...
        do {
            try TestSpaltdServer.dropDatabase(
                dbName: dbName,
...
enum HarnessError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .binaryBuildFailed(let s): return "go build of spaltd failed: \(s)"
...
private func ensureBinary(repoRoot: String) throws -> String {
    if let override = ProcessInfo.processInfo.environment["SPALT_TEST_BIN"], !override.isEmpty {
        return override
    }
    let binPath = (repoRoot as NSString).appendingPathComponent("bin/spaltd-test")
    let fm = FileManager.default
    if fm.fileExists(atPath: binPath) {
        return binPath
    }
    // Build it: `go build -o bin/spaltd-test ./cmd/spaltd`.
    ...
    proc.arguments = ["go", "build", "-o", "bin/spaltd-test", "./cmd/spaltd"]
    proc.currentDirectoryURL = URL(fileURLWithPath: repoRoot)
    let pipe = Pipe()
...
extension TestSpaltdServer {
    fileprivate static func dropDatabase(dbName: String, rootDSN: String) throws {
        ...
        // spaltd hasn't fully released its pool.
        let (rc, out) = try runPSQLInContainer(
            user: user, password: pass, db: root,
