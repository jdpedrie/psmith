...
/// spaltd subprocess. Covers tests #1–#35 from the testing plan.
///
...
struct ConversationsRepositoryTests {
    ...
    func createWithSettings() async throws {
        ...
        _ = fake
        let settings = SpaltConversationSettings(
            defaultProviderID: provider.id,
            defaultModelID: model.modelID,
            includeThinkingInHistory: true,
            callSettings: SpaltCallSettings(temperature: 0.55)
        )
    ...
    func updateTitleEmptyClears() async throws {
        ...
        // `optional title = ""`, which round-trips through SpaltConversation
    ...
    func updateSettingsReplacesBlob() async throws {
        ...
        let (fake, provider, model, profile) = try await Fixtures.seedReadyToChat(client: client)
        _ = fake
        let initial = SpaltConversationSettings(
            defaultProviderID: provider.id,
            defaultModelID: model.modelID,
            includeThinkingInHistory: true
        )
        ...
        let replacement = SpaltConversationSettings(includeThinkingInHistory: false)
    ...
    private func drainStream(client: SpaltClient, runID: String) async throws {
    ...
    /// We don't have a "list streams" API surfaced on SpaltKit, so we just
    /// give the server up to ~3s before returning. Good enough for tests.
    private func waitNoActiveStream(client: SpaltClient, conversationID: String) async throws {
