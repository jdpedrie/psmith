...
/// Layer 1 integration tests for AuthRepository against a real spaltd
...
struct AuthRepositoryTests {
    ...
    func loginBadPassword() async throws {
        ...
        let client = await TestSession.makeClient(server: server)
        await #expect(throws: SpaltError.self) {
    ...
    func loginRecordsClientLabel() async throws {
        ...
        let user = try await client.auth.login(
            username: username, password: password, clientLabel: "SpaltMac/integration-test"
        )
        #expect(user.username == username)
    ...
    @Test("probe against the running spaltd succeeds")
    func probeSuccess() async throws {
        let result = await probeSpaltServer(url: server.baseURL)
        switch result {
        case .ok(let serverName, _):
            #expect(serverName == "spaltd")
