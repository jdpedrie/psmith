...
struct AppModelTests {
    ...
    func bootstrapRestoresSession() async throws {
        ...
        // expose a direct reader for tokenStore on SpaltClient, so seed a
    ...
    private func mintSessionToken(server: TestSpaltdServer, username: String, password: String) async throws -> String {
    ...
    private func rawLoginToken(server: TestSpaltdServer, username: String, password: String) async throws -> String {
        ...
        let raw = Spalt_V1_AuthServiceClient(client: proto)
        var req = Spalt_V1_LoginRequest()
        req.username = username
        req.password = password
        let resp = await raw.login(request: req, headers: [:])
        guard let msg = resp.message else {
            throw SpaltError.from(resp.error ?? ConnectError(code: .unknown, message: "login failed"))
        }
