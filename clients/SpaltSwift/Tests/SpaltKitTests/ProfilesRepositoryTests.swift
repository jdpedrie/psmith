...
/// Layer 1 integration tests for ProfilesRepository against a real spaltd
...
struct ProfilesRepositoryTests {
    ...
    func listSeededOnly() async throws {
        ...
        // Assistant + Spalt Manager) — a fresh user is never empty.
        ...
        #expect(names == ["Personal Assistant", "Spalt Manager"])
    }
    ...
    func getResolvedFromParent() async throws {
        ...
        let parent = try await client.profiles.create(SpaltProfilePatch(
            name: "Parent",
            systemMessage: "I am the parent system prompt.",
            parentOnly: true
        ))
        let child = try await client.profiles.create(SpaltProfilePatch(
            name: "Child",
    ...
    @Test("create with full SpaltProfilePatch round-trips every field")
    func createFullPatch() async throws {
        let (client, _) = try await TestSession.freshUser(server: server, usernamePrefix: "prof-full")
        let parent = try await client.profiles.create(SpaltProfilePatch(name: "Parent", parentOnly: true))
        ...
        let titled = try await client.profiles.create(SpaltProfilePatch(
            name: "Titled",
            titleGuide: "Use 4 words.",
            titleProviderKind: SpaltTitleProviderKind.appleFoundation
        ))
        #expect(titled.titleGuide == "Use 4 words.")
        #expect(titled.titleProviderKind == SpaltTitleProviderKind.appleFoundation)
    }
    ...
    func createParentOnly() async throws {
        ...
        let created = try await client.profiles.create(
            SpaltProfilePatch(name: "Trunk", parentOnly: true)
        )
    ...
    func createFavorite() async throws {
        ...
        let created = try await client.profiles.create(
            SpaltProfilePatch(name: "Star", favorite: true)
        )
        #expect(created.favorite == true)
    }
    ...
    func createEmptyName() async throws {
        ...
        do {
            _ = try await client.profiles.create(SpaltProfilePatch(name: ""))
            Issue.record("expected InvalidArgument")
        } catch let SpaltError.rpc(code, _) {
            #expect(code == .invalidArgument)
        }
    ...
    func updatePartialPatch() async throws {
        ...
        let created = try await client.profiles.create(SpaltProfilePatch(
            name: "Before",
        ...
        let updated = try await client.profiles.update(
            id: created.id,
            patch: SpaltProfilePatch(name: "After")
        )
    ...
    func updateNotFoundCrossUser() async throws {
        ...
        do {
            _ = try await clientB.profiles.update(
                id: owned.id, patch: SpaltProfilePatch(name: "stolen")
            )
    ...
    func deleteWithChildrenFailsPrecondition() async throws {
        ...
        let parent = try await client.profiles.create(SpaltProfilePatch(name: "Parent", parentOnly: true))
        _ = try await client.profiles.create(SpaltProfilePatch(
