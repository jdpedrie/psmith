...
final class CallSettingsMergeTests: XCTestCase {
    ...
    func testScalarsHigherWins() {
        var higher = SpaltCallSettings()
        higher.temperature = 0.7
        higher.maxOutputTokens = 2048
        var lower = SpaltCallSettings()
    ...
    func testStopSequencesWholeFieldOverride() {
        ...
        let inheritedOut = CallSettingsMerge.merge(higher: SpaltCallSettings(), lower: lower)
    ...
    func testThinkingNestedMerge() {
        ...
        var higher = SpaltCallSettings()
        higher.thinking = SpaltThinkingSettings(enabled: true, budgetTokens: nil)
        var lower = SpaltCallSettings()
        lower.thinking = SpaltThinkingSettings(enabled: nil, budgetTokens: 8192)
    ...
    func testAnthropicNestedMerge() {
        var higher = SpaltCallSettings()
        higher.anthropic = SpaltAnthropicExtras(cacheEnabled: false, cacheTTL: nil)
        var lower = SpaltCallSettings()
        lower.anthropic = SpaltAnthropicExtras(cacheEnabled: true, cacheTTL: .oneHour)
    ...
    func testOpenAINestedMergeAndWholeFieldRules() {
        var higher = SpaltCallSettings()
        var ho = SpaltOpenAIExtras()
        ho.seed = 42
        ho.logitBias = [1: -5]
        higher.openai = ho

        var lower = SpaltCallSettings()
        var lo = SpaltOpenAIExtras()
        lo.seed = 7
    ...
    func testGoogleSafetyTwoLevelMerge() {
        var higher = SpaltCallSettings()
        var hg = SpaltGoogleExtras()
        hg.safetySettings = SpaltSafetySettings(harassment: .blockNone)
        higher.google = hg

        var lower = SpaltCallSettings()
        var lg = SpaltGoogleExtras()
        lg.responseMimeType = "application/json"
        lg.safetySettings = SpaltSafetySettings(harassment: .blockOnlyHigh, hateSpeech: .blockMediumAndAbove)
        lower.google = lg
    ...
    func testNilBlocksPassThrough() {
        var lower = SpaltCallSettings()
        lower.thinking = SpaltThinkingSettings(enabled: true, budgetTokens: 1024)
        lower.google = {
            var g = SpaltGoogleExtras()
            g.candidateCount = 3
            return g
        }()

        let out = CallSettingsMerge.merge(higher: SpaltCallSettings(), lower: lower)
        XCTAssertEqual(out.thinking?.enabled, true)
    ...
    func testThreeLayerChainAssociativity() {
        ...
        var convo = SpaltCallSettings()
        convo.temperature = 1.0
        var profile = SpaltCallSettings()
        profile.temperature = 0.5
        profile.maxOutputTokens = 1000
        var model = SpaltCallSettings()
