...
/// Renders one row per `SpaltConfigField` against a shared `[String: Any]`
...
public struct PluginConfigForm: View {
    ...
    /// chooser.
    let availableModels: [SpaltUserModel]
    ...
    public init(
        fields: [SpaltConfigField],
        config: Binding<[String: Any]>,
        availableModels: [SpaltUserModel] = [],
    ...
    private func groupedFields() -> [(String, [SpaltConfigField])] {
        var order: [String] = []
        var seen = Set<String>()
        var buckets: [String: [SpaltConfigField]] = [:]
        for f in fields {
    ...
    @ViewBuilder
    private func fieldRow(_ field: SpaltConfigField) -> some View {
    ...
    @ViewBuilder
    private func control(for field: SpaltConfigField) -> some View {
        switch field.type {
    ...
    private func modelPickerField(_ field: SpaltConfigField) -> some View {
    ...
    private func pickerSelection(for field: SpaltConfigField) -> (providerID: String, modelID: String)? {
    ...
    private func numberField(_ field: SpaltConfigField) -> some View {
    ...
    private func textField(_ field: SpaltConfigField) -> some View {
        let binding = Binding<String>(
            get: {
    ...
    private func textareaField(_ field: SpaltConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
                return decodeDefaultString(field) ?? ""
    ...
    private func booleanField(_ field: SpaltConfigField) -> some View {
    ...
    @ViewBuilder
    private func selectField(_ field: SpaltConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
    ...
    private func decodeDefaultString(_ field: SpaltConfigField) -> String? {
    ...
    private func decodeDefaultBool(_ field: SpaltConfigField) -> Bool? {
        guard !field.defaultJSON.isEmpty,
              let data = field.defaultJSON.data(using: .utf8) else { return nil }
...
private struct ModelPickerFieldControl: View {
    let field: SpaltConfigField
    let allModels: [SpaltUserModel]
    let providerLabels: [String: String]
    let providerTypes: [String: String]
    let providerPresetIDs: [String: String]
    ...
    private var matchingModels: [SpaltUserModel] {
