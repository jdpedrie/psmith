...
public struct OnboardingView: View {
    ...
    @State private var selectedTemplate: SpaltProviderTemplate?
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("To get started, connect at least one AI provider and enable a model. Spalt stores your credentials locally on this machine and routes every chat through providers you control.")
                .font(.callout)
    ...
    private var pickProviderStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Spalt supports the major LLM providers plus any OpenAI-compatible endpoint. Pick one to start — you can add more later.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private func templateCard(_ tmpl: SpaltProviderTemplate) -> some View {
        Button {
    ...
    private func createProvider() async {
        ...
        do {
            ...
            do {
                ...
                errorMessage = "Provider added, but model discovery failed: \(SpaltError.display(error))"
            }
            step = .enableModels
        } catch {
            errorMessage = "Couldn't add provider: \(SpaltError.display(error))"
        }
    }
    ...
    private func modelCheckbox(_ m: SpaltDiscoveredModel) -> some View {
    ...
    private func retryDiscover(providerID: String) async {
        ...
        do {
            ...
            errorMessage = "Discovery failed: \(SpaltError.display(error))"
        }
    }
    ...
    private func enableSelected() async {
        ...
        do {
            ...
            errorMessage = "Couldn't enable models: \(SpaltError.display(error))"
        }
    }
}
