...
/// Shared editor for `SpaltCallSettings`. Reused by every entry point (provider
...
public struct CallSettingsForm: View {
    @Binding var settings: SpaltCallSettings
    let inheritedSettings: SpaltCallSettings?
    let driverType: String
    let modelCapabilities: SpaltModelCapabilities?
    ...
    let modelConstraints: SpaltModelConstraints?
    ...
    public init(
        settings: Binding<SpaltCallSettings>,
        inheritedSettings: SpaltCallSettings?,
        driverType: String,
        modelCapabilities: SpaltModelCapabilities?,
        modelConstraints: SpaltModelConstraints? = nil,
    ...
    private var thinkingEnabledBinding: Binding<Bool?> {
        Binding(
            ...
            set: { newValue in
                var t = settings.thinking ?? SpaltThinkingSettings()
                t.enabled = newValue
            ...
            HStack {
                ...
                Picker("Cache TTL", selection: cacheTTLBinding) {
                    ...
                    Text("Inherit (\(inheritedSettings?.anthropic?.cacheTTL.map(cacheTTLShort) ?? "5 min, default"))")
                        .tag(SpaltCacheTTL?.none)
                    Text("5 min").tag(SpaltCacheTTL?.some(.fiveMinutes))
                    Text("1 hour").tag(SpaltCacheTTL?.some(.oneHour))
                }
                .adaptivePickerStyle()
                .labelsHidden()
                .fixedSize()
    ...
    private var cacheEnabledBinding: Binding<Bool?> {
        Binding(
            ...
            set: { newValue in
                var a = settings.anthropic ?? SpaltAnthropicExtras()
    ...
    private var cacheTTLBinding: Binding<SpaltCacheTTL?> {
        Binding(
    ...
    private func cacheTTLShort(_ v: SpaltCacheTTL) -> String {
        switch v {
                ...
                HStack {
                    ...
                    Picker("Service tier", selection: serviceTierBinding) {
                        Text(inheritPickerLabel(inheritedSettings?.openai?.serviceTier, serviceTierLabel))
                            .tag(SpaltServiceTier?.none)
                        Text("Auto").tag(SpaltServiceTier?.some(.auto))
                        Text("Standard").tag(SpaltServiceTier?.some(.standard))
                        Text("Priority").tag(SpaltServiceTier?.some(.priority))
                    }
                    .adaptivePickerStyle()
                    .labelsHidden()
                    .fixedSize()
    ...
    private func openaiBinding<T>(_ keyPath: WritableKeyPath<SpaltOpenAIExtras, T>) -> Binding<T> where T: Sendable {
        Binding(
            get: { (settings.openai ?? SpaltOpenAIExtras())[keyPath: keyPath] },
            set: { newValue in
                var o = settings.openai ?? SpaltOpenAIExtras()
    ...
    private var serviceTierBinding: Binding<SpaltServiceTier?> {
        Binding(
    ...
    private func serviceTierLabel(_ t: SpaltServiceTier) -> String {
        switch t {
    ...
    private func responseFormatLabel(_ rf: SpaltResponseFormat) -> String {
        switch rf {
        case .text:        return "Text"
    ...
    private func updateOpenAIResponseFormat(_ rf: SpaltResponseFormat) {
        var o = settings.openai ?? SpaltOpenAIExtras()
        o.responseFormat = rf
        settings.openai = o.isEmpty ? nil : o
    }
    ...
    private func optBoolBinding(_ keyPath: WritableKeyPath<SpaltCallSettings, Bool?>) -> Binding<Bool?> {
        Binding(
    ...
    private func updateGoogle<T>(_ keyPath: WritableKeyPath<SpaltGoogleExtras, T>, _ value: T) {
        var g = settings.google ?? SpaltGoogleExtras()
        g[keyPath: keyPath] = value
        settings.google = g.isEmpty ? nil : g
    }

    private func googleBinding<T>(_ keyPath: WritableKeyPath<SpaltGoogleExtras, T>) -> Binding<T> where T: Sendable {
        Binding(
            get: { (settings.google ?? SpaltGoogleExtras())[keyPath: keyPath] },
            set: { updateGoogle(keyPath, $0) }
        )
    }

    private func googleStringBinding(_ keyPath: WritableKeyPath<SpaltGoogleExtras, String?>) -> Binding<String> {
        Binding(
    ...
    private func googleSafetyBinding(_ keyPath: WritableKeyPath<SpaltSafetySettings, SpaltHarmThreshold?>) -> Binding<SpaltHarmThreshold?> {
        Binding(
            get: { settings.google?.safetySettings?[keyPath: keyPath] },
            set: { newValue in
                var g = settings.google ?? SpaltGoogleExtras()
                var s = g.safetySettings ?? SpaltSafetySettings()
    ...
    private func safetyThresholdRow(
        title: String,
        value: Binding<SpaltHarmThreshold?>,
        inherited: SpaltHarmThreshold?
    ) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                fieldLabel(title)
                ...
                Picker(title, selection: value) {
                    Text(inheritPickerLabel(inherited, harmLabel)).tag(SpaltHarmThreshold?.none)
                    Text("None").tag(SpaltHarmThreshold?.some(.blockNone))
                    Text("Low+").tag(SpaltHarmThreshold?.some(.blockLowAndAbove))
                    Text("Med+").tag(SpaltHarmThreshold?.some(.blockMediumAndAbove))
                    Text("High").tag(SpaltHarmThreshold?.some(.blockOnlyHigh))
                }
                .adaptivePickerStyle()
                .labelsHidden()
                .fixedSize()
    ...
    private func harmLabel(_ t: SpaltHarmThreshold) -> String {
        switch t {
