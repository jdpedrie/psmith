...
/// both enabled-model rows (`SpaltUserModel`) and discovery rows
/// (`SpaltDiscoveredModel`) feed the same widget without making each
/// caller inflate a full `SpaltUserModel`.
...
public extension SpaltUserModel {
...
public extension SpaltDiscoveredModel {
    func metaSnapshot(providerLabel: String?) -> ModelMetaSnapshot {
...
public struct ModelMetaStrip: View {
    ...
    private var pricing: SpaltModelPricing? { snapshot.pricing }
...
/// pricing isn't known.
public func costBucket(_ pricing: SpaltModelPricing?) -> String? {
