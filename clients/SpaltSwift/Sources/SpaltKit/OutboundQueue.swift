...
public struct OutboundQueueEntry: Sendable, Hashable, Codable, Identifiable {
    ...
    public let deviceFacts: [SpaltDeviceFact]
...
extension SpaltDeviceFact: Codable {
    enum CodingKeys: String, CodingKey { case key, value }
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        let raw = try c.decode(String.self, forKey: .key)
        guard let key = SpaltDeviceFactKey(rawString: raw) else {
            throw DecodingError.dataCorruptedError(
                forKey: .key, in: c,
                debugDescription: "unknown SpaltDeviceFactKey: \(raw)"
            )
        }
...
public final class OutboundQueue {
    ...
    /// queue size changes.
    public static let didChangeNotification = Notification.Name("dev.jdpedrie.spalt.OutboundQueue.didChange")
    ...
    public init(userDefaults: UserDefaults = .standard, storageKey: String = "spalt.outboundQueue") {
    ...
    public func drain(client: SpaltClient) async -> (sent: [OutboundQueueEntry], failed: OutboundQueueEntry?) {
