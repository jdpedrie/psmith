...
/// share a mental model. Per Spalt's Swift conventions this lives
/// in the shared SpaltKit package; views are platform-specific.
@Observable
@MainActor
public final class EmbedderViewModel {
    ...
    /// that window.
    public private(set) var saved: SpaltEmbedderConfig?
    ...
    public private(set) var testResult: SpaltEmbedderTestResult?

    /// Latest stats snapshot — drives the "X messages embedded"
    /// chip. Populated by `refreshStats`.
    public private(set) var stats: SpaltEmbedderStats?

    public init(client: SpaltClient) {
        self.client = client
    }
    ...
    public func test() async {
        ...
        do {
            ...
            testResult = SpaltEmbedderTestResult(
                ok: false,
