...
/// Top-level app state: holds the auth state and a configured SpaltClient.
...
/// sensible defaults (FileTokenStore + SPALT_HOST env override).
@Observable
@MainActor
public final class AppModel {
    public let authState: AuthState
    public let client: SpaltClient
    ...
    /// tool calls — e.g. an API key for Spalt Manager's
    ...
    public init(
        ...
        let cache: SpaltCache?
        do {
            if let accountID {
                cache = try Self.makeAccountScopedCache(accountID: accountID)
            } else {
                cache = try SpaltCache()
            }
        } catch {
            NSLog("SpaltCache init failed: \(error). Running without on-device cache.")
            cache = nil
        }
        let c = SpaltClient(host: host, tokenStore: tokenStore, authState: authState, cache: cache)
        self.client = c
    ...
    /// Builds a SpaltCache whose store URL embeds the account id.
    /// Two accounts (even on the same host) get distinct sqlite
    /// files so cache hits never cross identity boundaries.
    private static func makeAccountScopedCache(accountID: UUID) throws -> SpaltCache {
        ...
        let dir = support.appendingPathComponent("Spalt/Accounts/\(accountID.uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("cache.sqlite", isDirectory: false)
        return try SpaltCache(storeURL: storeURL)
    }

    /// Default factory: pulls the URL from `ServerURLStore` (which falls
    /// through to SPALT_HOST env, then http://127.0.0.1:8080) and uses a
