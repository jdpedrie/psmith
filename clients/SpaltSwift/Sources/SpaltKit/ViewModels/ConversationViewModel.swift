...
public final class ConversationViewModel {
    ...
    public private(set) var conversation: SpaltConversation
    ...
    public var localTitleProfilesByID: [String: SpaltProfile] = [:]

    // Load state
    public var activeContext: SpaltContext?
    public var messages: [SpaltMessage] = []
    ...
    // Edit state
    public var editingMessage: SpaltMessage?

    // Context switcher
    public var contexts: [SpaltContext] = []
    /// When true, the conversation pane swaps the message scroll for a
    ...
    public var conversationCallSettingsDraft: SpaltCallSettings = SpaltCallSettings()
    /// Snapshot of the resolved-from-below CallSettings (model + profile)
    /// used by the form's "Inherit (X)" mute previews. Loaded by
    /// `prepareSettingsView()`.
    public var resolvedCallSettings: SpaltCallSettings?
    ...
    public var settingsResolvedProfile: SpaltProfile?
    ...
    public var resolvedPluginPipeline: [SpaltResolvedPipelineEntry] = []
    ...
    public var conversationPluginOverrides: [SpaltConversationPlugin] = []
    /// Registered plugin types — used to render display names + look up
    /// config field schemas in the Plugins tab.
    public var registeredPluginTypes: [SpaltPluginType] = []

    // Model switcher
    public var availableModels: [SpaltUserModel] = []
    ...
    public var providerDefaultSettings: [String: SpaltCallSettings] = [:]
    ...
    public var activeProfileRequiredCapabilities: SpaltModelCapabilities?
    ...
    /// the per-context aggregate the server stamps on `SpaltContext.cumulativeCostUsd`.
    ...
    public init(
        conversation: SpaltConversation,
        client: SpaltClient,
        hub: StreamHub,
    ...
    public func selectModel(providerID: String, modelID: String) async {
        ...
        do {
            ...
            var s = live.settings ?? SpaltConversationSettings()
            ...
            loadError = SpaltError.display(error)
        }
    }
    ...
    public func maybeGenerateLocalTitle(profilesByID: [String: SpaltProfile]) async {
        ...
        guard resolvedTitleProviderKind(profilesByID: resolveMap) == SpaltTitleProviderKind.appleFoundation else { return }
    ...
    func resolvedTitleProviderKind(profilesByID: [String: SpaltProfile]) -> String? {
    ...
    private func resolvedTitleGuide(profilesByID: [String: SpaltProfile]) -> String? {
        guard var current = profilesByID[conversation.profileID] else { return nil }
    ...
    private func renderLocalTitleTranscript(_ msgs: [SpaltMessage]) -> String {
    ...
    public func loadAvailableModels() async {
        do {
            ...
            async let modelsTask: [SpaltUserModel] = withThrowingTaskGroup(of: [SpaltUserModel].self) { group in
                for p in providers {
                    group.addTask { try await self.client.modelProviders.listModels(providerID: p.id) }
                }
                var all: [SpaltUserModel] = []
            ...
            // remains selectable.
            async let profileTask: SpaltProfile? = {
                do {
    ...
    private func isLikelyTransientNetworkError(_ error: Error) -> Bool {
        ...
        // Best-effort string sniff for SpaltError-wrapped Connect
    ...
    public func prepareSettingsView() async {
        ...
        let liveConvo: SpaltConversation
        do {
        ...
        conversationCallSettingsDraft = liveConvo.settings?.callSettings ?? SpaltCallSettings()
        ...
        var resolved: SpaltCallSettings = SpaltCallSettings()
    ...
    public func toggleConversationDisableInherited(pluginName: String) async {
        ...
        if let i = next.firstIndex(where: { $0.pluginName == pluginName && $0.disabled }) {
            ...
            next[i] = SpaltConversationPlugin(pluginName: pluginName, ordinal: next[i].ordinal, config: Data(), disabled: true)
        } else {
            ...
            next.append(SpaltConversationPlugin(pluginName: pluginName, ordinal: Int32(next.count), config: Data(), disabled: true))
        }
    ...
    public func upsertConversationPluginOverride(pluginName: String, config: Data) async {
        ...
        if let i = next.firstIndex(where: { $0.pluginName == pluginName }) {
            let prev = next[i]
            next[i] = SpaltConversationPlugin(
            ...
            next.append(SpaltConversationPlugin(
                pluginName: pluginName,
    ...
    private func pushPluginOverrides(_ overrides: [SpaltConversationPlugin]) async {
        do {
    ...
    public func saveCallSettings() async {
        do {
            ...
            let (live, _) = try await client.conversations.get(id: conversation.id)
            var settings = live.settings ?? SpaltConversationSettings()
    ...
    private func mergeCallSettings(higher: SpaltCallSettings, lower: SpaltCallSettings) -> SpaltCallSettings {
    ...
    public func compact(
        ...
        do {
            ...
            // already false.
            compactError = SpaltError.display(error)
    ...
    public func editMessage(id: String, content: String, role: SpaltMessageRole? = nil) async {
        do {
    ...
    @MainActor
    private func handleStreamTerminal(_ run: SpaltStreamRun) async {
    ...
    public var treeMessages: [SpaltMessage] = [] {
    ...
    private var childrenByParentCache: [String: [SpaltMessage]] = [:]
    ...
    private func rebuildTreeCaches() {
        var children: [String: [SpaltMessage]] = [:]
    ...
    public func loadTree() async {
        ...
        do {
            ...
            // we just rendered (e.g. while spaltd is unreachable). Drop
