...
public final class ProfilesViewModel {
    ...
    public var pluginTypes: [SpaltPluginType] = []

    /// Per-profile plugin pipeline keyed by profile id. Populated by
    /// `loadPlugins(forProfileID:)` and replaced by `savePlugins(...)`.
    public var profilePlugins: [String: [SpaltProfilePlugin]] = [:]

    public init(client: SpaltClient) { self.client = client }
    ...
    public func selected() -> SpaltProfile? {
    ...
    public func loadAvailableModels() async {
        ...
        do {
            ...
            let models = try await withThrowingTaskGroup(of: [SpaltUserModel].self) { group in
                for p in providers {
    ...
    public func toggleModelFavorite(providerID: String, modelID: String) async {
        ...
        availableModels[idx] = SpaltUserModel(
    ...
    public func conciseName(for profile: SpaltProfile) -> String {
    ...
    public func parentChainName(for profile: SpaltProfile) -> String {
        var parts: [String] = []
        var current = profile.parentProfileID
        var seen: Set<String> = [profile.id]
        var depth = 0
    ...
    public var sortedForPicker: [SpaltProfile] {
    ...
    public var userPluginSettings: [String: SpaltUserPluginSettings] = [:]
    ...
    public func loadUserPluginSettings() async {
        do {
            ...
            var map: [String: SpaltUserPluginSettings] = [:]
    ...
    public func globalSettings(for pluginName: String) -> SpaltUserPluginSettings {
        userPluginSettings[pluginName] ?? SpaltUserPluginSettings(pluginName: pluginName, config: Data())
    }
    ...
    public func savePlugins(forProfileID id: String, plugins: [SpaltProfilePlugin]) async throws {
