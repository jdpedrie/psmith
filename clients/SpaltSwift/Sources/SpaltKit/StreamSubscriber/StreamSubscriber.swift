...
public enum StreamEvent: Sendable {
    case chunk(SpaltChunk)
    case terminal(SpaltStreamRun)
    case failed(SpaltError)
}

public struct SpaltChunk: Sendable, Hashable {
    public let sequence: Int64
    public let type: SpaltChunkType
...
public enum SpaltChunkType: Sendable, Hashable {
    ...
    init(from p: Spalt_V1_ChunkType) {
        switch p {
...
public enum SpaltStreamStatus: Sendable, Hashable {
    case running, completed, errored, cancelled, interrupted, unknown

    init(from p: Spalt_V1_StreamRunStatus) {
        switch p {
...
public enum SpaltStreamPurpose: Sendable, Hashable {
    case unspecified
    case assistantResponse
    case compression

    init(from p: Spalt_V1_StreamRunPurpose) {
        switch p {
...
public struct SpaltStreamRun: Sendable, Hashable, Identifiable {
    public let id: String
    public let conversationID: String
    public let contextID: String
    public let status: SpaltStreamStatus
    public let purpose: SpaltStreamPurpose
    ...
    public init(
        ...
        contextID: String,
        status: SpaltStreamStatus,
        purpose: SpaltStreamPurpose = .unspecified,
...
extension SpaltChunk {
    init(from p: Spalt_V1_Chunk) {
        self.init(sequence: p.sequence, type: SpaltChunkType(from: p.type), payload: p.payload)
    }
}

extension SpaltStreamRun {
    init(from p: Spalt_V1_StreamRun) {
        self.init(
            id: p.id,
            conversationID: p.conversationID,
            contextID: p.contextID,
            status: SpaltStreamStatus(from: p.status),
            purpose: SpaltStreamPurpose(from: p.purpose),
...
public final class StreamSubscriber: Sendable {
    private let client: Spalt_V1_StreamsServiceClientInterface

    public init(client: Spalt_V1_StreamsServiceClientInterface) {
        self.client = client
    }
        ...
        AsyncStream { continuation in
            let task = Task {
                ...
                while !Task.isCancelled, !sawTerminal {
                    ...
                    do {
                        var req = Spalt_V1_SubscribeStreamRequest()
                    ...
                    for await result in stream.results() {
                        ...
                        case .message(let resp):
                            ...
                            case .terminal(let run):
                                sawTerminal = true
                                continuation.yield(.terminal(SpaltStreamRun(from: run)))
                            case .none:
                                continue
                            }
    ...
    public func cancel(streamRunID: String) async throws {
        var req = Spalt_V1_CancelStreamRequest()
        req.streamRunID = streamRunID
    ...
    public func listActiveRuns(conversationID: String? = nil) async throws -> [SpaltStreamRun] {
        var req = Spalt_V1_ListActiveRunsRequest()
        if let conversationID, !conversationID.isEmpty {
            req.conversationID = conversationID
        }
        let resp = await client.listActiveRuns(request: req, headers: [:])
        guard let msg = resp.message else {
            throw resp.error.map(SpaltError.from) ?? SpaltError.missingPayload("list active runs")
        }
        return msg.runs.map(SpaltStreamRun.init(from:))
    }
}
