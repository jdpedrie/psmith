...
/// Persists the chosen spaltd server URL across app launches via
...
///   2. SPALT_HOST environment variable (kept for parity with the
...
public final class ServerURLStore {
    ...
    private static let defaultsKey = "spalt.server.url"
    ...
    private init() {
        if let saved = UserDefaults.standard.string(forKey: Self.defaultsKey),
           let url = URL(string: saved) {
            self.current = url
        } else if let env = ProcessInfo.processInfo.environment["SPALT_HOST"],
    ...
    public func reset() {
        ...
        if let env = ProcessInfo.processInfo.environment["SPALT_HOST"],
           let url = URL(string: env) {
            self.current = url
        } else {
            self.current = URL(string: "http://127.0.0.1:8080")!
