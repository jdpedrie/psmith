...
public struct SpaltConversation: Sendable, Hashable, Identifiable, Codable {
    ...
    public let settings: SpaltConversationSettings?
    ...
    public let streamingComponents: [SpaltStreamingComponentTag]
    ...
    public init(
        ...
        streamingComponents: [SpaltStreamingComponentTag] = []
    ) {
        self.id = id
...
public struct SpaltStreamingComponentTag: Sendable, Hashable, Codable {
    ...
    init(from p: Spalt_V1_StreamingComponentTag) {
        self.tag = p.tag
        self.component = p.component
    }
}

extension SpaltConversation {
    init(from p: Spalt_V1_Conversation) {
        self.init(
            id: p.id,
            ...
            settings: p.hasSettings ? SpaltConversationSettings(from: p.settings) : nil,
            streamingComponents: p.streamingComponents.map(SpaltStreamingComponentTag.init(from:))
        )
    }
}

/// Mirror of `Spalt_V1_ConversationOrder` for the `ConversationsRepository.list`
/// surface — keeps the proto enum out of UI/view-model code.
public enum SpaltConversationOrder: Sendable, Hashable {
    case recentlyUsed
    case recentlyCreated

    var proto: Spalt_V1_ConversationOrder {
        switch self {
...
/// Mirrors `Spalt_V1_ConversationSettings` but exposes a Swift-friendly
/// shape (no proto-generated has-getters / value defaults).
public struct SpaltConversationSettings: Sendable, Hashable, Codable {
    ...
    public var callSettings: SpaltCallSettings?

    public init(
        defaultProviderID: String? = nil,
        defaultModelID: String? = nil,
        includeThinkingInHistory: Bool? = nil,
        callSettings: SpaltCallSettings? = nil
    ) {
    ...
    public init(from p: Spalt_V1_ConversationSettings) {
        self.init(
            ...
            callSettings:             p.hasCallSettings             ? SpaltCallSettings(from: p.callSettings) : nil
        )
    }

    var proto: Spalt_V1_ConversationSettings {
        var s = Spalt_V1_ConversationSettings()
...
public struct SpaltContext: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    ...
    /// constructs via `init(from p: Spalt_V1_Context)`.
    public init(
        id: String,
...
extension SpaltContext {
    init(from p: Spalt_V1_Context) {
        self.init(
            id: p.id,
...
public enum SpaltMessageRole: String, Sendable, Hashable, Codable {
    case system, context, user, assistant, compressionSummary, unknown

    init(from p: Spalt_V1_MessageRole) {
        switch p {
    ...
    func toProto() -> Spalt_V1_MessageRole {
        switch self {
...
public struct SpaltMessageUsage: Sendable, Hashable, Codable {
    ...
    /// Production code uses `init(from p: Spalt_V1_MessageUsage)`.
    public init(
...
extension SpaltMessageUsage {
    init(from p: Spalt_V1_MessageUsage) {
...
public struct SpaltToolCall: Sendable, Hashable, Identifiable, Codable {
    public let id: String
...
extension SpaltToolCall {
    init(from p: Spalt_V1_ToolCall) {
        self.init(
            id: p.id,
            name: p.name,
            input: p.input,
...
public struct SpaltMessage: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    public let contextID: String
    public let parentID: String?
    public let role: SpaltMessageRole
    ...
    public let usage: SpaltMessageUsage?
    ...
    public let toolCalls: [SpaltToolCall]
    ...
    public let attachments: [SpaltMessageAttachment]
    ...
    public let uiFragments: [SpaltUIFragment]
    ...
    /// directly. Production code uses `init(from p: Spalt_V1_Message)`.
    public init(
        id: String,
        contextID: String,
        parentID: String? = nil,
        role: SpaltMessageRole,
        content: String,
        displayContent: String? = nil,
        siblingCount: Int32 = 0,
        providerID: String? = nil,
        modelID: String? = nil,
        usage: SpaltMessageUsage? = nil,
        editedAt: Date? = nil,
        errorText: String? = nil,
        thinkingRenderedText: String? = nil,
        thinkingDurationMs: Int32? = nil,
        toolCalls: [SpaltToolCall] = [],
        finishReason: String? = nil,
        createdAt: Date = Date(timeIntervalSince1970: 0),
        attachments: [SpaltMessageAttachment] = [],
        uiFragments: [SpaltUIFragment] = [],
...
/// pipeline. Mirrors `Spalt_V1_UIFragment`. The `component` keys
...
public struct SpaltUIFragment: Sendable, Hashable, Codable {
    public let component: String
    ...
    init(from p: Spalt_V1_UIFragment) {
...
/// to an image loader.
public struct SpaltMessageAttachment: Sendable, Hashable, Codable, Identifiable {
...
extension SpaltMessageAttachment {
    init(from p: Spalt_V1_MessageAttachment) {
        self.init(
            fileID: p.fileID,
            kind: p.kind,
            mimeType: p.mimeType,
...
extension SpaltMessage {
    init(from p: Spalt_V1_Message) {
        self.init(
            id: p.id,
            contextID: p.contextID,
            parentID: p.hasParentID ? p.parentID : nil,
            role: SpaltMessageRole(from: p.role),
            content: p.content,
            ...
            usage: p.hasUsage ? SpaltMessageUsage(from: p.usage) : nil,
            ...
            toolCalls: p.toolCalls.map(SpaltToolCall.init(from:)),
            finishReason: p.hasFinishReason && !p.finishReason.isEmpty ? p.finishReason : nil,
            createdAt: p.hasCreatedAt ? p.createdAt.date : Date(timeIntervalSince1970: 0),
            attachments: p.attachments.map(SpaltMessageAttachment.init(from:)),
            uiFragments: p.uiFragments.map(SpaltUIFragment.init(from:)),
            isWelcome: p.isWelcome
        )
    }
}

public enum SpaltCompressionMode: Sendable, Hashable, Codable {
    case unspecified
    case replace
    case append

    func toProto() -> Spalt_V1_CompressionMode {
        switch self {
...
public struct SpaltProfileDefaults: Sendable, Hashable, Codable {
    public var defaultProviderID: String?
    public var defaultModelID: String?
    public var includeThinkingInHistory: Bool?
...
public enum SpaltTitleProviderKind {
...
public struct SpaltProfile: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    public let name: String
    ...
    public let compressionMode: SpaltCompressionMode?
    public let compressionProviderID: String?
    public let compressionModelID: String?
    public let defaultSettings: SpaltProfileDefaults?
    ...
    /// `SpaltTitleProviderKind.appleFoundation`). When set, the server
    ...
    public let requiredModelCapabilities: SpaltModelCapabilities?
    ...
    public init(
        ...
        createdAt: Date? = nil,
        updatedAt: Date? = nil,
        requiredModelCapabilities: SpaltModelCapabilities? = nil,
...
extension SpaltProfile {
    init(from p: Spalt_V1_Profile) {
        let mode: SpaltCompressionMode? = p.hasCompressionMode
            ? {
            ...
            : nil
        let defaults: SpaltProfileDefaults? = p.hasDefaultSettings
            ? SpaltProfileDefaults(
                ...
                callSettings:             p.defaultSettings.hasCallSettings ? SpaltCallSettings(from: p.defaultSettings.callSettings) : nil
            )
            : nil
        self.init(
            id: p.id,
            ...
            requiredModelCapabilities: p.hasRequiredModelCapabilities
                ? SpaltModelCapabilities(from: p.requiredModelCapabilities)
                : nil,
