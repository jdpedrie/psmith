...
public struct SpaltPluginCapabilities: Sendable, Hashable {
...
public enum SpaltConfigFieldType: Sendable, Hashable {
...
public enum SpaltConfigFieldMerge: Sendable, Hashable {
...
/// Mirror of `spalt.v1.ModelPickerFilter`. Drives which user_models
/// the chooser surfaces for a `.modelPicker` field. Any flag set to
/// true is required; flags AND together. Empty = no filter.
public struct SpaltModelPickerFilter: Sendable, Hashable {
...
public struct SpaltConfigOption: Sendable, Hashable {
...
public struct SpaltConfigField: Sendable, Hashable, Identifiable {
    public var id: String { name }
    public let name: String
    public let display: String
    public let description: String
    public let type: SpaltConfigFieldType
    /// JSON-encoded default literal (e.g. "1", "\"<choices>\"", "true").
    /// Empty string means no default.
    public let defaultJSON: String
    public let options: [SpaltConfigOption]
    ...
    public let modelPickerFilter: SpaltModelPickerFilter?
    ...
    public let merge: SpaltConfigFieldMerge
    ...
    public init(
        ...
        display: String,
        description: String,
        type: SpaltConfigFieldType,
        defaultJSON: String,
        options: [SpaltConfigOption],
        required: Bool = false,
        global: Bool = false,
        modelPickerFilter: SpaltModelPickerFilter? = nil,
        merge: SpaltConfigFieldMerge = .replace,
        category: String = ""
    ) {
        self.name = name
...
public struct SpaltPluginType: Sendable, Hashable, Identifiable {
    public var id: String { name }
    public let name: String
    ...
    public let description: String
    public let capabilities: SpaltPluginCapabilities
    public let configFields: [SpaltConfigField]
    ...
    public var profileScopedConfigFields: [SpaltConfigField] {
        configFields.filter { !$0.global }
    }

    /// Fields that live on the global "Plugin settings" surface.
    public var globalConfigFields: [SpaltConfigField] {
        configFields.filter { $0.global }
    }
}
...
public struct SpaltUserPluginSettings: Sendable, Hashable, Identifiable {
...
extension SpaltUserPluginSettings {
    init(from p: Spalt_V1_UserPluginSettings) {
        self.init(pluginName: p.pluginName, config: p.config)
    }
}

public struct SpaltProfilePlugin: Sendable, Hashable, Identifiable {
...
/// Conversation-scoped plugin override. Same shape as `SpaltProfilePlugin`
...
public struct SpaltConversationPlugin: Sendable, Hashable, Identifiable {
    public var id: String { "\(ordinal)-\(pluginName)" }
    public let pluginName: String
    public let ordinal: Int32
    public let config: Data
...
public enum SpaltResolvedPipelineSource: Sendable, Hashable {
    case profile
    case conversation
    case unspecified
}
...
public struct SpaltResolvedPipelineEntry: Sendable, Hashable, Identifiable {
    public var id: String { "\(ordinal)-\(pluginName)" }
    public let pluginName: String
    public let ordinal: Int32
    public let config: Data
    public let source: SpaltResolvedPipelineSource

    public init(pluginName: String, ordinal: Int32, config: Data, source: SpaltResolvedPipelineSource) {
        self.pluginName = pluginName
        self.ordinal = ordinal
        self.config = config
        self.source = source
...
extension SpaltPluginCapabilities {
    init(from p: Spalt_V1_PluginCapabilities) {
        self.init(
...
extension SpaltConfigFieldType {
    init(from p: Spalt_V1_ConfigField.TypeEnum) {
        switch p {
...
extension SpaltModelPickerFilter {
    init(from p: Spalt_V1_ModelPickerFilter) {
        self.init(
...
extension SpaltConfigOption {
    init(from p: Spalt_V1_ConfigOption) {
        self.init(value: p.value, label: p.label)
    }
}

extension SpaltConfigField {
    init(from p: Spalt_V1_ConfigField) {
        let filter: SpaltModelPickerFilter? = p.hasModelPickerFilter
            ? SpaltModelPickerFilter(from: p.modelPickerFilter)
            : nil
        let merge: SpaltConfigFieldMerge
        switch p.merge {
        ...
        self.init(
            ...
            type: SpaltConfigFieldType(from: p.type),
            defaultJSON: p.defaultJson,
            options: p.options.map(SpaltConfigOption.init(from:)),
            required: p.required,
            global: p.global,
...
extension SpaltPluginType {
    init(from p: Spalt_V1_PluginType) {
        self.init(
            name: p.name,
            displayName: p.displayName.isEmpty ? p.name : p.displayName,
            description: p.description_p,
            capabilities: SpaltPluginCapabilities(from: p.capabilities),
            configFields: p.configFields.map(SpaltConfigField.init(from:))
        )
    }
}

extension SpaltProfilePlugin {
    init(from p: Spalt_V1_ProfilePlugin) {
        self.init(
    ...
    var proto: Spalt_V1_ProfilePlugin {
        var p = Spalt_V1_ProfilePlugin()
...
extension SpaltConversationPlugin {
    init(from p: Spalt_V1_ConversationPlugin) {
        self.init(
            pluginName: p.pluginName,
            ordinal: p.ordinal,
            config: p.config,
    ...
    var proto: Spalt_V1_ConversationPlugin {
        var p = Spalt_V1_ConversationPlugin()
        p.pluginName = pluginName
        p.ordinal = ordinal
        p.config = config
        p.disabled = disabled
...
extension SpaltResolvedPipelineSource {
    init(from p: Spalt_V1_ResolvedPipelineSource) {
        switch p {
...
extension SpaltResolvedPipelineEntry {
    init(from p: Spalt_V1_ResolvedPipelineEntry) {
        self.init(
            pluginName: p.pluginName,
            ordinal: p.ordinal,
            config: p.config,
            source: SpaltResolvedPipelineSource(from: p.source)
        )
    }
}
