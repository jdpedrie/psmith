...
public struct SpaltProviderType: Sendable, Identifiable, Hashable, Codable {
...
extension SpaltProviderType {
    init(from p: Spalt_V1_ProviderType) {
        name        = p.name
...
public struct SpaltProviderTemplate: Sendable, Identifiable, Hashable, Codable {
...
extension SpaltProviderTemplate {
    init(from p: Spalt_V1_ProviderTemplate) {
...
public struct SpaltUserModelProvider: Sendable, Identifiable, Hashable, Codable {
    ...
    public let defaultSettings: SpaltCallSettings?
    ...
    public init(
        ...
        createdAt: Date,
        updatedAt: Date,
        defaultSettings: SpaltCallSettings? = nil,
        baseURL: String? = nil,
        presetID: String? = nil
    ) {
        self.id = id
...
extension SpaltUserModelProvider {
    init(from p: Spalt_V1_UserModelProvider) {
        id        = p.id
        type      = p.type
        label     = p.label
        ...
        defaultSettings = p.hasDefaultSettings ? SpaltCallSettings(from: p.defaultSettings) : nil
...
public struct SpaltModelPricing: Sendable, Hashable, Codable {
...
extension SpaltModelPricing {
    init(from p: Spalt_V1_ModelPricing) {
...
public struct SpaltModelCapabilities: Sendable, Hashable, Codable {
...
extension SpaltModelCapabilities {
    init(from p: Spalt_V1_ModelCapabilities) {
    ...
    public static let empty = SpaltModelCapabilities(
        streaming: false,
        thinking: false,
        toolUse: false,
        vision: false,
    ...
    public func shortfall(against actual: SpaltModelCapabilities) -> [String] {
        var out: [String] = []
...
public struct SpaltUserModel: Sendable, Identifiable, Hashable, Codable {
    ...
    public let pricing: SpaltModelPricing?
    public let knowledgeCutoff: String?
    public let modalities: [String]
    public let capabilities: SpaltModelCapabilities?
    ...
    /// rejection inline.
    public let constraints: SpaltModelConstraints?
    ...
    public init(
        ...
        favorite: Bool,
        defaultSettings: SpaltCallSettings? = nil,
        constraints: SpaltModelConstraints? = nil
    ) {
...
extension SpaltUserModel {
    init(from p: Spalt_V1_UserModel) {
        ...
        pricing        = p.hasPricing        ? SpaltModelPricing(from: p.pricing)        : nil
        knowledgeCutoff = p.hasKnowledgeCutoff ? p.knowledgeCutoff : nil
        modalities     = p.modalities
        capabilities   = p.hasCapabilities   ? SpaltModelCapabilities(from: p.capabilities) : nil
        favorite       = p.favorite
        defaultSettings = p.hasDefaultSettings ? SpaltCallSettings(from: p.defaultSettings) : nil
        constraints   = p.hasConstraints    ? SpaltModelConstraints(from: p.constraints)  : nil
    }
}
...
/// + `proto Spalt_V1_ModelConstraints`. Source-of-truth table lives in
/// `internal/modelmeta/constraints.go`.
public struct SpaltModelConstraints: Sendable, Hashable, Codable {
    /// Accepted temperature interval. nil = no constraint; UI offers
    /// the full slider range.
    public let temperature: SpaltRange?
    ...
    public init(temperature: SpaltRange? = nil, unsupported: [String] = []) {
        self.temperature = temperature
        self.unsupported = unsupported
    }

    init(from p: Spalt_V1_ModelConstraints) {
        temperature = p.hasTemperature ? SpaltRange(from: p.temperature) : nil
...
/// temperature at 1.0).
public struct SpaltRange: Sendable, Hashable, Codable {
...
public struct SpaltDiscoveredModel: Sendable, Identifiable, Hashable {
    public var id: String { modelID }
    public let modelID: String
    public let displayName: String
    public let contextWindow: Int32?
    ...
    public init(
        ...
        pricing: SpaltModelPricing? = nil,
        capabilities: SpaltModelCapabilities? = nil,
...
extension SpaltDiscoveredModel {
    init(from p: Spalt_V1_DiscoveredModel) {
        modelID       = p.modelID
        displayName   = p.displayName
        contextWindow = p.hasContextWindow ? p.contextWindow : nil
        pricing       = p.hasPricing       ? SpaltModelPricing(from: p.pricing)        : nil
...
public struct SpaltProviderTestResult: Sendable, Hashable {
    public let ok: Bool
...
extension SpaltProviderTestResult {
    init(from p: Spalt_V1_TestUserModelProviderResponse) {
        ok = p.ok
...
public struct SpaltModelTestResult: Sendable, Hashable {
    public let ok: Bool
    public let errorMessage: String
    public let latencyMs: Int64
...
extension SpaltModelTestResult {
    init(from p: Spalt_V1_TestUserModelResponse) {
        ok = p.ok
        errorMessage = p.errorMessage
        latencyMs = p.latencyMs
