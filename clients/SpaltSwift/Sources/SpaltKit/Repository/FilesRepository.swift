...
public struct SpaltFile: Sendable, Hashable, Identifiable {
...
public final class FilesRepository: Sendable {
    private let client: Spalt_V1_FilesServiceClientInterface
    /// spaltd host used to resolve relative signed URLs. When the
    /// server doesn't have SPALT_PUBLIC_BASE_URL set, GetFileURL
    ...
    public init(client: Spalt_V1_FilesServiceClientInterface, host: URL) {
        self.client = client
        self.host = host
    }

    /// Upload `data` as one file. Returns the server-assigned
    /// `SpaltFile` on success. Errors surface as `SpaltError` (via
    /// `SpaltError.display`) so callers don't have to know about
    /// Connect's error envelope.
    public func upload(
        data: Data,
        mimeType: String,
        originalFilename: String?
    ) async throws -> SpaltFile {
        ...
        // land on disk.
        var header = Spalt_V1_UploadFileHeader()
        ...
        var headerReq = Spalt_V1_UploadFileRequest()
        ...
        while offset < data.count {
            ...
            var chunkReq = Spalt_V1_UploadFileRequest()
        ...
        var response: Spalt_V1_UploadFileResponse?
        ...
        if let streamError {
            ...
            // practice; SpaltError.from converts that. If we got
            // something else (rare), fall through to a generic
            // missingPayload.
            if let ce = streamError as? ConnectError {
                throw SpaltError.from(ce)
            }
            throw SpaltError.missingPayload("upload: \(SpaltError.display(streamError))")
        }
        guard let response else {
            throw SpaltError.missingPayload("upload file response")
        }
    ...
    /// relative path (the default when SPALT_PUBLIC_BASE_URL isn't
    /// set), we resolve it against the configured spaltd host so
    /// the caller can hand it straight to AsyncImage.
    public func signedURL(fileID: String) async throws -> URL {
        var req = Spalt_V1_GetFileURLRequest()
        req.fileID = fileID
        let resp = await client.getFileURL(request: req, headers: [:])
        guard let msg = resp.message else {
            throw resp.error.map(SpaltError.from) ?? .missingPayload("get file url")
        }
        // Parse the raw string. If the server gave us an absolute
        // URL (production / SPALT_PUBLIC_BASE_URL set), use it as-
        ...
        throw SpaltError.missingPayload("server returned an invalid file url")
    }

    /// List the caller's recent files. `limit` defaults to the
    /// server's own default (currently 50).
    public func list(limit: Int32? = nil) async throws -> [SpaltFile] {
        var req = Spalt_V1_ListFilesRequest()
        if let limit { req.limit = limit }
        let resp = await client.listFiles(request: req, headers: [:])
        guard let msg = resp.message else {
            throw resp.error.map(SpaltError.from) ?? .missingPayload("list files")
        }
