...
public final class SpaltClient: Sendable {
    ...
    /// conversation data when spaltd is unreachable.
    public let cache: SpaltCache?

    public init(host: URL, tokenStore: TokenStore, authState: AuthState, cache: SpaltCache? = nil) {
        ...
        self.auth = AuthRepository(
            client: Spalt_V1_AuthServiceClient(client: protocolClient),
            tokenStore: tokenStore,
            authState: authState,
            cache: cache
        )
        self.conversations = ConversationsRepository(
            client: Spalt_V1_ConversationsServiceClient(client: protocolClient),
            cache: cache
        )
        self.profiles = ProfilesRepository(
            client: Spalt_V1_ProfilesServiceClient(client: protocolClient),
            cache: cache
        )
        self.streams = StreamSubscriber(
            client: Spalt_V1_StreamsServiceClient(client: protocolClient)
        )
        self.modelProviders = ModelProvidersRepository(
            client: Spalt_V1_ModelProvidersServiceClient(client: protocolClient),
            cache: cache
        )
        self.files = FilesRepository(
            client: Spalt_V1_FilesServiceClient(client: protocolClient),
            host: host
        )
        self.langfuse = LangfuseRepository(
            client: Spalt_V1_LangfuseServiceClient(client: protocolClient)
        )
        self.embedder = EmbedderRepository(
            client: Spalt_V1_EmbedderServiceClient(client: protocolClient)
        )
        self.deviceTools = DeviceToolsRepository(
            client: Spalt_V1_DeviceToolsServiceClient(client: protocolClient),
            host: host,
            tokenStore: tokenStore
        )
        self.events = EventsSubscriber(
            client: Spalt_V1_EventsServiceClient(client: protocolClient)
        )
    }
}
