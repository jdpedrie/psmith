...
public enum SpaltError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .rpc(code, message):
            ...
            return SpaltError.humanise(rpcCode: code, message: message)
        case let .missingPayload(field):
            return "Missing field in server response: \(field)"
        case let .localStorage(err):
            return "Couldn't read local storage: \(SpaltError.display(err))"
        }
    }
    ...
    public static func display(_ error: Error) -> String {
        if let ce = error as? SpaltError, let s = ce.errorDescription, !s.isEmpty {
            return s
        }
...
/// Result of probing a candidate spaltd URL. The login screen uses
/// this to decide whether to advance to the username/password form
/// (`.ok`) or surface a tailored failure message.
public enum SpaltProbeResult: Sendable, Equatable {
    /// Server answered cleanly. `serverName` should be "spaltd" for the
    ...
    /// service running at this URL, or an old spaltd that doesn't yet
    /// implement Probe.
...
/// Probes a candidate spaltd URL without authentication. Doesn't hold
/// onto state — constructs a one-shot SpaltClient against the given
...
@MainActor
public func probeSpaltServer(url: URL) async -> SpaltProbeResult {
    let probeStore = InMemoryTokenStore()
    let probeAuth = AuthState()
    let client = SpaltClient(host: url, tokenStore: probeStore, authState: probeAuth)
...
public final class AuthRepository: Sendable {
    private let client: Spalt_V1_AuthServiceClientInterface
    private let tokenStore: TokenStore
    private let authState: AuthState
    private let cache: SpaltCache?

    public init(
        client: Spalt_V1_AuthServiceClientInterface,
    ...
    /// Calls AuthService.Probe. Returns a typed SpaltProbeResult so the
    ...
    /// three cases.
    public func probe() async -> SpaltProbeResult {
        let resp = await client.probe(request: Spalt_V1_ProbeRequest(), headers: [:])
        ...
        if let err = resp.error {
            ...
            // signal (something is at this URL but it isn't a spaltd that
    ...
    public func login(username: String, password: String, clientLabel: String? = nil) async throws -> SpaltUser {
        var req = Spalt_V1_LoginRequest()
        ...
        guard let msg = resp.message else {
            throw map(error: resp.error) ?? SpaltError.missingPayload("login response")
        }
        do { try tokenStore.save(msg.sessionToken) } catch { throw SpaltError.localStorage(error) }

        let user = SpaltUser(from: msg.user)
        await MainActor.run { authState.setAuthenticated(user) }
        return user
    }

    public func whoAmI() async throws -> SpaltUser {
        let resp = await client.whoAmI(request: Spalt_V1_WhoAmIRequest(), headers: [:])
        guard let msg = resp.message else {
            throw map(error: resp.error) ?? SpaltError.missingPayload("whoami response")
        }
        let user = SpaltUser(from: msg.user)
        await MainActor.run { authState.setAuthenticated(user) }
    ...
    public func logout() async throws {
        let resp = await client.logout(request: Spalt_V1_LogoutRequest(), headers: [:])
    ...
    public func restoreSession() async -> SpaltUser? {
        ...
        do {
            ...
            if case let SpaltError.rpc(code, _) = error,
            ...
            if let cache,
               let cached: SpaltUser = await cache.get(SpaltUser.self, kind: CacheKind.currentUser, id: "me") {
    ...
    private func map(error: ConnectError?) -> SpaltError? {
