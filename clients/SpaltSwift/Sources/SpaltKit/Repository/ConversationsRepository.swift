...
public final class ConversationsRepository: Sendable {
    private let client: Spalt_V1_ConversationsServiceClientInterface
    private let cache: SpaltCache?

    public init(
        client: Spalt_V1_ConversationsServiceClientInterface,
    ...
    public func list(
        ...
        order: SpaltConversationOrder? = nil,
        titleQuery: String? = nil,
        profileID: String? = nil
    ) async throws -> (items: [SpaltConversation], nextPageToken: String?) {
        var req = Spalt_V1_ListConversationsRequest()
        ...
        if let msg = resp?.message {
            let items = msg.conversations.map(SpaltConversation.init(from:))
        ...
        if pageToken == nil && titleQuery == nil && profileID == nil,
           let cache,
           let cached: [SpaltConversation] = await cache.get([SpaltConversation].self, kind: CacheKind.conversationsList, id: "all") {
            return (cached, nil)
        }
        throw resp?.error.map(SpaltError.from) ?? .missingPayload("list conversations")
    }

    public func get(id: String) async throws -> (SpaltConversation, SpaltContext) {
        var req = Spalt_V1_GetConversationRequest()
        req.id = id
        let resp = await client.getConversation(request: req, headers: [:])
        if let msg = resp.message {
            let conv = SpaltConversation(from: msg.conversation)
            let ctx = SpaltContext(from: msg.activeContext)
            if let cache {
        ...
        if let cache,
           let conv: SpaltConversation = await cache.get(SpaltConversation.self, kind: "conversation", id: id),
           let ctx: SpaltContext = await cache.get(SpaltContext.self, kind: "activeContext", id: id) {
            return (conv, ctx)
        }
        throw resp.error.map(SpaltError.from) ?? .missingPayload("get conversation")
    }
    ...
    public func create(
    ...
    ) async throws -> SpaltConversation {
        var req = Spalt_V1_CreateConversationRequest()
        ...
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("create conversation") }
        return SpaltConversation(from: msg.conversation)
    }

    public func delete(id: String) async throws {
        var req = Spalt_V1_DeleteConversationRequest()
        req.id = id
    ...
    @discardableResult
    public func updateTitle(id: String, title: String) async throws -> SpaltConversation {
        var req = Spalt_V1_UpdateConversationRequest()
        req.id = id
        req.title = title
        let resp = await client.updateConversation(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("update conversation") }
        return SpaltConversation(from: msg.conversation)
    }
    ...
    public func updateSettings(
        id: String,
        title: String? = nil,
        settings: SpaltConversationSettings
    ) async throws -> SpaltConversation {
        var req = Spalt_V1_UpdateConversationRequest()
        req.id = id
        if let title { req.title = title }
    ...
    public func sendMessage(
        ...
        deviceFacts: [SpaltDeviceFact] = []
    ) async throws -> (userMessage: SpaltMessage, streamRun: SpaltStreamRun) {
        var req = Spalt_V1_SendMessageRequest()
        ...
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("send message") }
        return (SpaltMessage(from: msg.userMessage), SpaltStreamRun(from: msg.streamRun))
    }
    ...
    public func regenerateAssistant(
        ...
        providerID: String? = nil,
        modelID: String? = nil
    ) async throws -> (parentMessage: SpaltMessage, streamRun: SpaltStreamRun) {
        var req = Spalt_V1_SendMessageRequest()
        req.conversationID = conversationID
        ...
        if let modelID { req.modelID = modelID }
        let resp = await client.sendMessage(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("regenerate") }
        return (SpaltMessage(from: msg.userMessage), SpaltStreamRun(from: msg.streamRun))
    }
    ...
    public func listMessages(
        ...
        fullTree: Bool = false
    ) async throws -> [SpaltMessage] {
        var req = Spalt_V1_ListMessagesRequest()
        ...
        if let msg = resp.message {
            let items = msg.messages.map(SpaltMessage.init(from:))
        ...
        if !fullTree && leafMessageID == nil,
           let cache,
           let cached: [SpaltMessage] = await cache.get([SpaltMessage].self, kind: CacheKind.messagesByContext, id: contextID) {
            return cached
        }
        throw resp.error.map(SpaltError.from) ?? .missingPayload("list messages")
    }
    ...
    public func setCurrentLeaf(contextID: String, messageID: String?) async throws {
        var req = Spalt_V1_SetCurrentLeafRequest()
        req.contextID = contextID
    ...
    public func countContextTokens(contextID: String, providerID: String, modelID: String) async throws -> (tokenCount: Int32, contextWindow: Int32) {
        var req = Spalt_V1_CountContextTokensRequest()
        req.contextID = contextID
        req.providerID = providerID
        req.modelID = modelID
        let resp = await client.countContextTokens(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("count context tokens") }
    ...
    public func compact(
        ...
        providerID: String? = nil,
        modelID: String? = nil
    ) async throws -> SpaltStreamRun {
        var req = Spalt_V1_CompactRequest()
        req.conversationID = conversationID
        ...
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("compact") }
        return SpaltStreamRun(from: msg.streamRun)
    }

    public func promoteCompactionToNewContext(messageID: String) async throws -> SpaltContext {
        var req = Spalt_V1_PromoteCompactionToNewContextRequest()
        req.messageID = messageID
        let resp = await client.promoteCompactionToNewContext(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("promote compaction") }
        return SpaltContext(from: msg.context)
    }
    ...
    public func createContextManual(
        ...
        mode: SpaltCompressionMode
    ) async throws -> (context: SpaltContext, userMessage: SpaltMessage?) {
        var req = Spalt_V1_CreateContextManualRequest()
        req.conversationID = conversationID
        ...
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("create context manual") }
        let userMsg: SpaltMessage? = msg.hasUserMessage ? SpaltMessage(from: msg.userMessage) : nil
        return (SpaltContext(from: msg.context), userMsg)
    }

    public func editMessage(
        id: String,
        content: String,
        role: SpaltMessageRole? = nil
    ) async throws -> SpaltMessage {
        var req = Spalt_V1_EditMessageRequest()
        req.id = id
        req.content = content
        ...
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("edit message") }
        return SpaltMessage(from: msg.message)
    }

    public func deleteMessage(id: String, cascade: Bool = false) async throws {
        var req = Spalt_V1_DeleteMessageRequest()
        req.id = id
        req.cascade = cascade
    ...
    public func listContexts(conversationID: String) async throws -> [SpaltContext] {
        var req = Spalt_V1_ListContextsRequest()
        req.conversationID = conversationID
        let resp = await client.listContexts(request: req, headers: [:])
        if let msg = resp.message {
            let items = msg.contexts.map(SpaltContext.init(from:))
            if let cache {
        ...
        if let cache,
           let cached: [SpaltContext] = await cache.get([SpaltContext].self, kind: CacheKind.contextsByConversation, id: conversationID) {
            return cached
        }
        throw resp.error.map(SpaltError.from) ?? .missingPayload("list contexts")
    }

    public func activateContext(contextID: String) async throws -> SpaltContext {
        var req = Spalt_V1_ActivateContextRequest()
        req.contextID = contextID
        let resp = await client.activateContext(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("activate context") }
        return SpaltContext(from: msg.context)
    }
    ...
    public func getPlugins(conversationID: String) async throws -> [SpaltConversationPlugin] {
        var req = Spalt_V1_GetConversationPluginsRequest()
        req.conversationID = conversationID
        let resp = await client.getConversationPlugins(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("get conversation plugins") }
        return msg.plugins.map(SpaltConversationPlugin.init(from:))
    }
    ...
    @discardableResult
    public func setPlugins(conversationID: String, plugins: [SpaltConversationPlugin]) async throws -> [SpaltConversationPlugin] {
        var req = Spalt_V1_SetConversationPluginsRequest()
        req.conversationID = conversationID
        req.plugins = plugins.map { $0.proto }
        let resp = await client.setConversationPlugins(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("set conversation plugins") }
        return msg.plugins.map(SpaltConversationPlugin.init(from:))
    }
    ...
    public func resolvePipeline(conversationID: String) async throws -> [SpaltResolvedPipelineEntry] {
        var req = Spalt_V1_ResolveConversationPipelineRequest()
        req.conversationID = conversationID
        let resp = await client.resolveConversationPipeline(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(SpaltError.from) ?? .missingPayload("resolve conversation pipeline") }
        return msg.entries.map(SpaltResolvedPipelineEntry.init(from:))
    }
}

extension SpaltError {
    public static func from(_ err: ConnectError) -> SpaltError {
