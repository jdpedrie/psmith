...
public struct SpaltEmbedderConfig: Sendable, Hashable, Codable {
    ...
    init(from p: Spalt_V1_EmbedderConfig) {
        type = p.type
        baseURL = p.baseURL
        model = p.model
...
/// render it inline.
public struct SpaltEmbedderTestResult: Sendable {
    public let ok: Bool
...
/// settings page.
public struct SpaltEmbedderStats: Sendable {
...
/// so views consume `SpaltEmbedderConfig` instead of the raw
/// proto.
public final class EmbedderRepository: Sendable {
    private let client: Spalt_V1_EmbedderServiceClientInterface

    public init(client: Spalt_V1_EmbedderServiceClientInterface) {
        self.client = client
    }
    ...
    /// absence.
    public func get() async throws -> SpaltEmbedderConfig {
        let resp = await client.getEmbedderConfig(request: Spalt_V1_GetEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw SpaltError.from(err) }
        return SpaltEmbedderConfig(from: resp.message?.config ?? Spalt_V1_EmbedderConfig())
    }
    ...
    public func update(
        ...
        apiKey: String? = nil,
        enabled: Bool? = nil
    ) async throws -> SpaltEmbedderConfig {
        var req = Spalt_V1_UpdateEmbedderConfigRequest()
    ...
    public func delete() async throws {
        let resp = await client.deleteEmbedderConfig(request: Spalt_V1_DeleteEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw SpaltError.from(err) }
    }
    ...
    public func test() async throws -> SpaltEmbedderTestResult {
        let resp = await client.testEmbedderConfig(request: Spalt_V1_TestEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw SpaltError.from(err) }
        let m = resp.message ?? Spalt_V1_TestEmbedderConfigResponse()
        return SpaltEmbedderTestResult(ok: m.ok, errorMessage: m.errorMessage, latencyMs: m.latencyMs)
    }
    ...
    public func listTypes() async throws -> [String] {
        let resp = await client.listEmbedderTypes(request: Spalt_V1_ListEmbedderTypesRequest(), headers: [:])
        if let err = resp.error { throw SpaltError.from(err) }
    ...
    public func stats() async throws -> SpaltEmbedderStats {
        let resp = await client.getEmbedderStats(request: Spalt_V1_GetEmbedderStatsRequest(), headers: [:])
        if let err = resp.error { throw SpaltError.from(err) }
        let m = resp.message ?? Spalt_V1_GetEmbedderStatsResponse()
        return SpaltEmbedderStats(unembeddedCount: m.unembeddedCount, workerActive: m.workerActive)
    }
}
