...
/// connected SpaltClient — typically held on the AppModel.
///
/// Lifecycle:
///
...
public final class DeviceToolDispatcher {
    ...
    private let log = Logger(subsystem: "dev.jdpedrie.spalt", category: "DeviceTools")

    public init(client: SpaltClient, registry: DeviceToolRegistry = .shared) {
        self.client = client
    ...
    /// freely.
    public func handleChunk(_ chunk: SpaltChunk, conversationID: String) {
...
private func runHandler(
    ...
    info: SpaltChunk.DeviceToolUseInfo,
    log: Logger
