...
public struct OutboundQueueEntry: Sendable, Hashable, Codable, Identifiable {
    ...
    public let deviceFacts: [PsmithDeviceFact]
...
extension PsmithDeviceFact: Codable {
    enum CodingKeys: String, CodingKey { case key, value }
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        let raw = try c.decode(String.self, forKey: .key)
        guard let key = PsmithDeviceFactKey(rawString: raw) else {
            throw DecodingError.dataCorruptedError(
                forKey: .key, in: c,
                debugDescription: "unknown PsmithDeviceFactKey: \(raw)"
            )
        }
...
public final class OutboundQueue {
    ...
    /// queue size changes.
    public static let didChangeNotification = Notification.Name("dev.jdpedrie.psmith.OutboundQueue.didChange")
    ...
    public init(userDefaults: UserDefaults = .standard, storageKey: String = "psmith.outboundQueue") {
    ...
    public func drain(client: PsmithClient) async -> (sent: [OutboundQueueEntry], failed: OutboundQueueEntry?) {
