...
/// pipeline asks for, returning a list of `PsmithDeviceFact`
...
public final class DeviceFactsProvider {
    ...
    public func gather(_ requested: Set<PsmithDeviceFactKey> = .defaults) -> [PsmithDeviceFact] {
        var out: [PsmithDeviceFact] = []
        out.reserveCapacity(requested.count)
        for key in requested {
            if let value = currentValue(for: key) {
                out.append(PsmithDeviceFact(key: key, value: value))
            }
        }
        ...
        if LocationFactPreference.enabled {
            ...
            if let fact = MainActor.assumeIsolated({ LocationProvider.shared.freshFact() }) {
                if requested.contains(.locationCoords) {
                    out.append(PsmithDeviceFact(key: .locationCoords, value: fact.coords))
                }
                if requested.contains(.locationCity), let city = fact.city {
                    out.append(PsmithDeviceFact(key: .locationCity, value: city))
                }
            }
        }
        #endif
    ...
    private func currentValue(for key: PsmithDeviceFactKey) -> String? {
        switch key {
...
extension Set where Element == PsmithDeviceFactKey {
    ...
    /// render them.
    public static var defaults: Set<PsmithDeviceFactKey> {
...
public enum LocationFactPreference {
    private static let key = "psmith.deviceFacts.locationEnabled"
