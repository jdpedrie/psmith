...
public struct PsmithConversation: Sendable, Hashable, Identifiable, Codable {
    ...
    public let settings: PsmithConversationSettings?
...
public struct PsmithStreamingComponentTag: Sendable, Hashable, Codable {
    ...
    init(from p: Psmith_V1_StreamingComponentTag) {
        self.tag = p.tag
        self.component = p.component
    }
}

extension PsmithConversation {
    init(from p: Psmith_V1_Conversation) {
        self.init(
            id: p.id,
...
/// Mirror of `Psmith_V1_ConversationOrder` for the `ConversationsRepository.list`
/// surface — keeps the proto enum out of UI/view-model code.
public enum PsmithConversationOrder: Sendable, Hashable {
    case recentlyUsed
    case recentlyCreated

    var proto: Psmith_V1_ConversationOrder {
        switch self {
...
/// Mirrors `Psmith_V1_ConversationSettings` but exposes a Swift-friendly
/// shape (no proto-generated has-getters / value defaults).
public struct PsmithConversationSettings: Sendable, Hashable, Codable {
    ...
    public var callSettings: PsmithCallSettings?

    public init(
        defaultProviderID: String? = nil,
        defaultModelID: String? = nil,
        includeThinkingInHistory: Bool? = nil,
        callSettings: PsmithCallSettings? = nil
    ) {
    ...
    public init(from p: Psmith_V1_ConversationSettings) {
        self.init(
            ...
            callSettings:             p.hasCallSettings             ? PsmithCallSettings(from: p.callSettings) : nil
        )
    }

    var proto: Psmith_V1_ConversationSettings {
        var s = Psmith_V1_ConversationSettings()
...
public struct PsmithContext: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    ...
    /// constructs via `init(from p: Psmith_V1_Context)`.
    public init(
        id: String,
...
extension PsmithContext {
    init(from p: Psmith_V1_Context) {
        self.init(
            id: p.id,
...
public enum PsmithMessageRole: String, Sendable, Hashable, Codable {
    case system, context, user, assistant, compressionSummary, unknown

    init(from p: Psmith_V1_MessageRole) {
        switch p {
    ...
    func toProto() -> Psmith_V1_MessageRole {
        switch self {
...
public struct PsmithMessageUsage: Sendable, Hashable, Codable {
    ...
    /// Production code uses `init(from p: Psmith_V1_MessageUsage)`.
    public init(
...
extension PsmithMessageUsage {
    init(from p: Psmith_V1_MessageUsage) {
...
public struct PsmithToolCall: Sendable, Hashable, Identifiable, Codable {
    public let id: String
...
extension PsmithToolCall {
    init(from p: Psmith_V1_ToolCall) {
        self.init(
            id: p.id,
            name: p.name,
            input: p.input,
...
public struct PsmithMessage: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    public let contextID: String
    public let parentID: String?
    public let role: PsmithMessageRole
    ...
    public let usage: PsmithMessageUsage?
    ...
    public let toolCalls: [PsmithToolCall]
    ...
    public let attachments: [PsmithMessageAttachment]
    ...
    public let uiFragments: [PsmithUIFragment]
    ...
    /// directly. Production code uses `init(from p: Psmith_V1_Message)`.
    public init(
        id: String,
        contextID: String,
        parentID: String? = nil,
        role: PsmithMessageRole,
        content: String,
        displayContent: String? = nil,
        siblingCount: Int32 = 0,
        providerID: String? = nil,
        modelID: String? = nil,
        usage: PsmithMessageUsage? = nil,
        editedAt: Date? = nil,
        errorText: String? = nil,
        thinkingRenderedText: String? = nil,
        thinkingDurationMs: Int32? = nil,
        toolCalls: [PsmithToolCall] = [],
        finishReason: String? = nil,
        createdAt: Date = Date(timeIntervalSince1970: 0),
        attachments: [PsmithMessageAttachment] = [],
        uiFragments: [PsmithUIFragment] = [],
...
/// pipeline. Mirrors `Psmith_V1_UIFragment`. The `component` keys
...
public struct PsmithUIFragment: Sendable, Hashable, Codable {
    public let component: String
    ...
    init(from p: Psmith_V1_UIFragment) {
...
/// to an image loader.
public struct PsmithMessageAttachment: Sendable, Hashable, Codable, Identifiable {
...
extension PsmithMessageAttachment {
    init(from p: Psmith_V1_MessageAttachment) {
        self.init(
            fileID: p.fileID,
            kind: p.kind,
            mimeType: p.mimeType,
...
extension PsmithMessage {
    init(from p: Psmith_V1_Message) {
        self.init(
            id: p.id,
            contextID: p.contextID,
            parentID: p.hasParentID ? p.parentID : nil,
            role: PsmithMessageRole(from: p.role),
            content: p.content,
            ...
            usage: p.hasUsage ? PsmithMessageUsage(from: p.usage) : nil,
            ...
            toolCalls: p.toolCalls.map(PsmithToolCall.init(from:)),
            finishReason: p.hasFinishReason && !p.finishReason.isEmpty ? p.finishReason : nil,
            createdAt: p.hasCreatedAt ? p.createdAt.date : Date(timeIntervalSince1970: 0),
            attachments: p.attachments.map(PsmithMessageAttachment.init(from:)),
            uiFragments: p.uiFragments.map(PsmithUIFragment.init(from:)),
            isWelcome: p.isWelcome
        )
    }
}

public enum PsmithCompressionMode: Sendable, Hashable, Codable {
    case unspecified
    case replace
    case append

    func toProto() -> Psmith_V1_CompressionMode {
        switch self {
...
public struct PsmithProfileDefaults: Sendable, Hashable, Codable {
    public var defaultProviderID: String?
    public var defaultModelID: String?
    public var includeThinkingInHistory: Bool?
...
public enum PsmithTitleProviderKind {
...
public struct PsmithProfile: Sendable, Hashable, Identifiable, Codable {
    public let id: String
    public let name: String
    ...
    public let compressionMode: PsmithCompressionMode?
    public let compressionProviderID: String?
    public let compressionModelID: String?
    public let defaultSettings: PsmithProfileDefaults?
    ...
    /// `PsmithTitleProviderKind.appleFoundation`). When set, the server
    ...
    public let requiredModelCapabilities: PsmithModelCapabilities?
    ...
    public init(
        ...
        createdAt: Date? = nil,
        updatedAt: Date? = nil,
        requiredModelCapabilities: PsmithModelCapabilities? = nil,
...
extension PsmithProfile {
    init(from p: Psmith_V1_Profile) {
        let mode: PsmithCompressionMode? = p.hasCompressionMode
            ? {
            ...
            : nil
        let defaults: PsmithProfileDefaults? = p.hasDefaultSettings
            ? PsmithProfileDefaults(
                ...
                callSettings:             p.defaultSettings.hasCallSettings ? PsmithCallSettings(from: p.defaultSettings.callSettings) : nil
            )
            : nil
        self.init(
            id: p.id,
            ...
            requiredModelCapabilities: p.hasRequiredModelCapabilities
                ? PsmithModelCapabilities(from: p.requiredModelCapabilities)
                : nil,
