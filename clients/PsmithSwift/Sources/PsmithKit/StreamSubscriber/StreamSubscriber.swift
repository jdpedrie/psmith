...
public enum StreamEvent: Sendable {
    case chunk(PsmithChunk)
    case terminal(PsmithStreamRun)
    case failed(PsmithError)
}

public struct PsmithChunk: Sendable, Hashable {
    public let sequence: Int64
    public let type: PsmithChunkType
...
public enum PsmithChunkType: Sendable, Hashable {
...
public enum PsmithStreamStatus: Sendable, Hashable {
    case running, completed, errored, cancelled, interrupted, unknown

    init(from p: Psmith_V1_StreamRunStatus) {
        switch p {
...
public enum PsmithStreamPurpose: Sendable, Hashable {
    case unspecified
    case assistantResponse
    case compression

    init(from p: Psmith_V1_StreamRunPurpose) {
        switch p {
...
public struct PsmithStreamRun: Sendable, Hashable, Identifiable {
    public let id: String
    public let conversationID: String
    public let contextID: String
    public let status: PsmithStreamStatus
    ...
    public init(
        ...
        contextID: String,
        status: PsmithStreamStatus,
        purpose: PsmithStreamPurpose = .unspecified,
...
extension PsmithChunk {
    init(from p: Psmith_V1_Chunk) {
        self.init(sequence: p.sequence, type: PsmithChunkType(from: p.type), payload: p.payload)
    }
}

extension PsmithStreamRun {
    init(from p: Psmith_V1_StreamRun) {
        self.init(
            id: p.id,
            conversationID: p.conversationID,
            contextID: p.contextID,
            status: PsmithStreamStatus(from: p.status),
            purpose: PsmithStreamPurpose(from: p.purpose),
...
public final class StreamSubscriber: Sendable {
    private let client: Psmith_V1_StreamsServiceClientInterface

    public init(client: Psmith_V1_StreamsServiceClientInterface) {
        self.client = client
    }
        ...
        AsyncStream { continuation in
            let task = Task {
                ...
                while !Task.isCancelled, !sawTerminal {
                    ...
                    do {
                        var req = Psmith_V1_SubscribeStreamRequest()
                    ...
                    for await result in stream.results() {
                        ...
                        case .message(let resp):
                            ...
                            case .terminal(let run):
                                sawTerminal = true
                                continuation.yield(.terminal(PsmithStreamRun(from: run)))
                            case .none:
                                continue
                            }
    ...
    public func cancel(streamRunID: String) async throws {
        var req = Psmith_V1_CancelStreamRequest()
        req.streamRunID = streamRunID
    ...
    public func listActiveRuns(conversationID: String? = nil) async throws -> [PsmithStreamRun] {
        var req = Psmith_V1_ListActiveRunsRequest()
        ...
        return msg.runs.map(PsmithStreamRun.init(from:))
    }
}
