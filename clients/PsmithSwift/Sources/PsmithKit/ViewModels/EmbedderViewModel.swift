...
/// share a mental model. Per Psmith's Swift conventions this lives
/// in the shared PsmithKit package; views are platform-specific.
@Observable
@MainActor
public final class EmbedderViewModel {
    ...
    /// that window.
    public private(set) var saved: PsmithEmbedderConfig?
    ...
    public private(set) var testResult: PsmithEmbedderTestResult?

    /// Latest stats snapshot — drives the "X messages embedded"
    /// chip. Populated by `refreshStats`.
    public private(set) var stats: PsmithEmbedderStats?
    ...
    public func test() async {
        ...
        do {
            ...
            testResult = PsmithEmbedderTestResult(
                ok: false,
