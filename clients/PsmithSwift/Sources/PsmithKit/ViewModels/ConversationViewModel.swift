...
public final class ConversationViewModel {
    ...
    public private(set) var conversation: PsmithConversation
    ...
    public var localTitleProfilesByID: [String: PsmithProfile] = [:]
    ...
    public var conversationCallSettingsDraft: PsmithCallSettings = PsmithCallSettings()
    /// Snapshot of the resolved-from-below CallSettings (model + profile)
    /// used by the form's "Inherit (X)" mute previews. Loaded by
    /// `prepareSettingsView()`.
    public var resolvedCallSettings: PsmithCallSettings?
    ...
    public var settingsResolvedProfile: PsmithProfile?
    ...
    public var resolvedPluginPipeline: [PsmithResolvedPipelineEntry] = []
    ...
    public var conversationPluginOverrides: [PsmithConversationPlugin] = []
    /// Registered plugin types — used to render display names + look up
    /// config field schemas in the Plugins tab.
    public var registeredPluginTypes: [PsmithPluginType] = []
    ...
    public var activeProfileRequiredCapabilities: PsmithModelCapabilities?
    ...
    /// the per-context aggregate the server stamps on `PsmithContext.cumulativeCostUsd`.
    ...
    public init(
        conversation: PsmithConversation,
        client: PsmithClient,
        hub: StreamHub,
    ...
    public func selectModel(providerID: String, modelID: String) async {
        ...
        do {
            ...
            var s = live.settings ?? PsmithConversationSettings()
    ...
    public func maybeGenerateLocalTitle(profilesByID: [String: PsmithProfile]) async {
    ...
    func resolvedTitleProviderKind(profilesByID: [String: PsmithProfile]) -> String? {
    ...
    private func resolvedTitleGuide(profilesByID: [String: PsmithProfile]) -> String? {
        guard var current = profilesByID[conversation.profileID] else { return nil }
    ...
    private func renderLocalTitleTranscript(_ msgs: [PsmithMessage]) -> String {
    ...
    public func loadAvailableModels() async {
        do {
            ...
            async let modelsTask: [PsmithUserModel] = withThrowingTaskGroup(of: [PsmithUserModel].self) { group in
                for p in providers {
            ...
            // remains selectable.
            async let profileTask: PsmithProfile? = {
                do {
    ...
    private func isLikelyTransientNetworkError(_ error: Error) -> Bool {
        ...
        // Best-effort string sniff for PsmithError-wrapped Connect
    ...
    public func prepareSettingsView() async {
        ...
        let liveConvo: PsmithConversation
        do {
        ...
        conversationCallSettingsDraft = liveConvo.settings?.callSettings ?? PsmithCallSettings()
        ...
        var resolved: PsmithCallSettings = PsmithCallSettings()
    ...
    public func toggleConversationDisableInherited(pluginName: String) async {
        ...
        if let i = next.firstIndex(where: { $0.pluginName == pluginName && $0.disabled }) {
            ...
            next[i] = PsmithConversationPlugin(pluginName: pluginName, ordinal: next[i].ordinal, config: Data(), disabled: true)
        } else {
            ...
            next.append(PsmithConversationPlugin(pluginName: pluginName, ordinal: Int32(next.count), config: Data(), disabled: true))
        }
    ...
    public func upsertConversationPluginOverride(pluginName: String, config: Data) async {
        ...
        if let i = next.firstIndex(where: { $0.pluginName == pluginName }) {
            let prev = next[i]
            next[i] = PsmithConversationPlugin(
            ...
            next.append(PsmithConversationPlugin(
                pluginName: pluginName,
    ...
    private func pushPluginOverrides(_ overrides: [PsmithConversationPlugin]) async {
        do {
    ...
    public func saveCallSettings() async {
        do {
            ...
            let (live, _) = try await client.conversations.get(id: conversation.id)
            var settings = live.settings ?? PsmithConversationSettings()
    ...
    private func mergeCallSettings(higher: PsmithCallSettings, lower: PsmithCallSettings) -> PsmithCallSettings {
    ...
    public func compact(
        ...
        do {
            ...
            // already false.
            compactError = PsmithError.display(error)
    ...
    @MainActor
    private func handleStreamTerminal(_ run: PsmithStreamRun) async {
    ...
    public var treeMessages: [PsmithMessage] = [] {
    ...
    private var childrenByParentCache: [String: [PsmithMessage]] = [:]
    ...
    private func rebuildTreeCaches() {
        var children: [String: [PsmithMessage]] = [:]
    ...
    public func loadTree() async {
        ...
        do {
            ...
            // we just rendered (e.g. while psmithd is unreachable). Drop
