...
/// `update` instead.
public struct PsmithLangfuseConfig: Sendable, Hashable, Codable {
    ...
    init(from p: Psmith_V1_LangfuseConfig) {
        host = p.host
...
/// render it inline.
public struct PsmithLangfuseTestResult: Sendable {
    public let ok: Bool
...
/// the wrapper exists so views consume `PsmithLangfuseConfig`
/// instead of the proto type directly.
public final class LangfuseRepository: Sendable {
    private let client: Psmith_V1_LangfuseServiceClientInterface

    public init(client: Psmith_V1_LangfuseServiceClientInterface) {
        self.client = client
    }
    ...
    /// throws on absence.
    public func get() async throws -> PsmithLangfuseConfig {
        let resp = await client.getLangfuseConfig(request: Psmith_V1_GetLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        return PsmithLangfuseConfig(from: resp.message?.config ?? Psmith_V1_LangfuseConfig())
    }
    ...
    public func update(
        ...
        enabled: Bool? = nil
    ) async throws -> PsmithLangfuseConfig {
        var req = Psmith_V1_UpdateLangfuseConfigRequest()
    ...
    public func delete() async throws {
        let resp = await client.deleteLangfuseConfig(request: Psmith_V1_DeleteLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
    }
    ...
    public func test() async throws -> PsmithLangfuseTestResult {
        let resp = await client.testLangfuseConfig(request: Psmith_V1_TestLangfuseConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        let m = resp.message ?? Psmith_V1_TestLangfuseConfigResponse()
        return PsmithLangfuseTestResult(ok: m.ok, errorMessage: m.errorMessage, latencyMs: m.latencyMs)
    }
}
