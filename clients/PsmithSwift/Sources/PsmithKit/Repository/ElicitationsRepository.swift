...
/// wire.
public enum PsmithElicitAction: String, Sendable, Hashable, Codable {
    case accept
    case decline
    case cancel
}
...
public final class ElicitationsRepository: Sendable {
    ...
    public func respond(
        ...
        guard let http = response as? HTTPURLResponse else {
            throw PsmithElicitationError.transport(URLError(.badServerResponse))
        }
        switch http.statusCode {
        case 200, 204:
            return
        case 401:
            throw PsmithElicitationError.unauthorized
        case 404:
            ...
            throw PsmithElicitationError.notFound
        default:
            let msg = String(data: data, encoding: .utf8) ?? "(no body)"
            throw PsmithElicitationError.serverError(status: http.statusCode, body: msg)
        }
    }
}

public enum PsmithElicitationError: Error, Sendable {
