...
public final class ModelProvidersRepository: Sendable {
    private let client: Psmith_V1_ModelProvidersServiceClientInterface
    private let cache: PsmithCache?

    public init(
        client: Psmith_V1_ModelProvidersServiceClientInterface,
        cache: PsmithCache? = nil
    ) {
        self.client = client
        self.cache = cache
    }

    public func listProviderTypes() async throws -> [PsmithProviderType] {
        let resp = await client.listProviderTypes(request: .init(), headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("list provider types") }
        return msg.types.map(PsmithProviderType.init(from:))
    }

    public func listTemplates() async throws -> [PsmithProviderTemplate] {
        let resp = await client.listProviderTemplates(request: .init(), headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("list provider templates") }
        return msg.templates.map(PsmithProviderTemplate.init(from:))
    }

    public func list() async throws -> [PsmithUserModelProvider] {
        ...
        if let msg = resp?.message {
            let items = msg.providers.map(PsmithUserModelProvider.init(from:))
            if let cache {
        ...
        if let cache,
           let cached: [PsmithUserModelProvider] = await cache.get([PsmithUserModelProvider].self, kind: CacheKind.providers, id: "all") {
            return cached
        }
        throw resp?.error.map(PsmithError.from) ?? .missingPayload("list providers")
    }

    public func get(id: String) async throws -> (PsmithUserModelProvider, [PsmithUserModel]) {
        var req = Psmith_V1_GetUserModelProviderRequest()
        req.id = id
        let resp = await client.getUserModelProvider(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("get provider") }
        return (PsmithUserModelProvider(from: msg.provider), msg.enabledModels.map(PsmithUserModel.init(from:)))
    }

    public func create(type: String, label: String, config: Data) async throws -> PsmithUserModelProvider {
        var req = Psmith_V1_CreateUserModelProviderRequest()
        req.type   = type
        req.label  = label
        req.config = config
        let resp = await client.createUserModelProvider(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("create provider") }
        return PsmithUserModelProvider(from: msg.provider)
    }

    public func update(id: String, label: String? = nil, config: Data? = nil) async throws -> PsmithUserModelProvider {
        var req = Psmith_V1_UpdateUserModelProviderRequest()
        req.id = id
        if let l = label  { req.label  = l }
        if let c = config { req.config = c }
        let resp = await client.updateUserModelProvider(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("update provider") }
        return PsmithUserModelProvider(from: msg.provider)
    }

    public func delete(id: String) async throws {
        var req = Psmith_V1_DeleteUserModelProviderRequest()
        req.id = id
    ...
    public func discoverModels(providerID: String) async throws -> [PsmithDiscoveredModel] {
        var req = Psmith_V1_DiscoverModelsRequest()
        req.userModelProviderID = providerID
        let resp = await client.discoverModels(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("discover models") }
        return msg.models.map(PsmithDiscoveredModel.init(from:))
    }

    public func enableModels(providerID: String, modelIDs: [String]) async throws -> [PsmithUserModel] {
        var req = Psmith_V1_EnableModelsRequest()
        req.userModelProviderID = providerID
        req.modelIds = modelIDs
        let resp = await client.enableModels(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("enable models") }
        return msg.enabled.map(PsmithUserModel.init(from:))
    }

    public func disableModels(providerID: String, modelIDs: [String]) async throws {
        var req = Psmith_V1_DisableModelsRequest()
        req.userModelProviderID = providerID
        req.modelIds = modelIDs
    ...
    public func listModels(providerID: String) async throws -> [PsmithUserModel] {
        var req = Psmith_V1_ListUserModelsRequest()
        req.userModelProviderID = providerID
        let resp = await client.listUserModels(request: req, headers: [:])
        if let msg = resp.message {
            let items = msg.models.map(PsmithUserModel.init(from:))
            if let cache {
        ...
        if let cache,
           let cached: [PsmithUserModel] = await cache.get([PsmithUserModel].self, kind: CacheKind.availableModels, id: providerID) {
            return cached
        }
        throw resp.error.map(PsmithError.from) ?? .missingPayload("list models")
    }

    public func toggleModelFavorite(providerID: String, modelID: String, favorite: Bool) async throws -> PsmithUserModel {
        var req = Psmith_V1_ToggleUserModelFavoriteRequest()
        req.userModelProviderID = providerID
        req.modelID = modelID
        req.favorite = favorite
        let resp = await client.toggleUserModelFavorite(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("toggle favorite") }
        return PsmithUserModel(from: msg.model)
    }
    ...
    /// "unset," letting upper layers contribute. Pass `PsmithCallSettings()` to
    /// effectively clear the column.
    public func updateProviderDefaultSettings(providerID: String, settings: PsmithCallSettings) async throws -> PsmithUserModelProvider {
        var req = Psmith_V1_UpdateUserModelProviderRequest()
        req.id = providerID
        req.defaultSettings = settings.proto
        let resp = await client.updateUserModelProvider(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("update provider default_settings") }
        return PsmithUserModelProvider(from: msg.provider)
    }
    ...
    public func updateModel(providerID: String, modelID: String, settings: PsmithCallSettings) async throws -> PsmithUserModel {
        var req = Psmith_V1_UpdateUserModelRequest()
        req.userModelProviderID = providerID
        req.modelID = modelID
        req.defaultSettings = settings.proto
        let resp = await client.updateUserModel(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("update model") }
    ...
    public func updateModelFull(
        ...
        defaultSettings: PsmithCallSettings?
    ) async throws -> PsmithUserModel {
        var req = Psmith_V1_UpdateUserModelRequest()
        req.userModelProviderID = providerID
        req.modelID = modelID
        ...
        if let c = capabilities {
            var cap = Psmith_V1_ModelCapabilities()
    ...
    public func addManualModel(
        ...
        var req = Psmith_V1_AddManualModelRequest()
        req.userModelProviderID = providerID
        req.modelID = modelID
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("add manual model") }
        return PsmithUserModel(from: msg.userModel)
    }
    ...
    /// (couldn't reach psmithd at all) still throw.
    public func testProvider(providerID: String) async throws -> PsmithProviderTestResult {
        var req = Psmith_V1_TestUserModelProviderRequest()
        req.userModelProviderID = providerID
        let resp = await client.testUserModelProvider(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("test provider") }
        return PsmithProviderTestResult(from: msg)
    }
    ...
    public func testModel(providerID: String, modelID: String) async throws -> PsmithModelTestResult {
        var req = Psmith_V1_TestUserModelRequest()
        req.userModelProviderID = providerID
        req.modelID = modelID
        let resp = await client.testUserModel(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("test model") }
        return PsmithModelTestResult(from: msg)
    }
    ...
    public func listProviderCosts(
        since: Date? = nil,
        until: Date? = nil
    ) async throws -> (providers: [PsmithProviderCost], grandTotal: Double) {
        var req = Psmith_V1_ListProviderCostsRequest()
        if let since {
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("list provider costs") }
        return (msg.providers.map(PsmithProviderCost.init(from:)), msg.grandTotalUsd)
    }
}
...
public struct PsmithProviderCost: Sendable, Hashable, Identifiable, Codable {
    ...
    init(from p: Psmith_V1_ProviderCost) {
        self.init(
