...
public struct PsmithEmbedderConfig: Sendable, Hashable, Codable {
    ...
    init(from p: Psmith_V1_EmbedderConfig) {
        type = p.type
        baseURL = p.baseURL
        model = p.model
...
/// render it inline.
public struct PsmithEmbedderTestResult: Sendable {
    public let ok: Bool
...
/// settings page.
public struct PsmithEmbedderStats: Sendable {
...
/// so views consume `PsmithEmbedderConfig` instead of the raw
/// proto.
public final class EmbedderRepository: Sendable {
    private let client: Psmith_V1_EmbedderServiceClientInterface

    public init(client: Psmith_V1_EmbedderServiceClientInterface) {
        self.client = client
    }
    ...
    /// absence.
    public func get() async throws -> PsmithEmbedderConfig {
        let resp = await client.getEmbedderConfig(request: Psmith_V1_GetEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        return PsmithEmbedderConfig(from: resp.message?.config ?? Psmith_V1_EmbedderConfig())
    }
    ...
    public func update(
        ...
        apiKey: String? = nil,
        enabled: Bool? = nil
    ) async throws -> PsmithEmbedderConfig {
        var req = Psmith_V1_UpdateEmbedderConfigRequest()
    ...
    public func delete() async throws {
        let resp = await client.deleteEmbedderConfig(request: Psmith_V1_DeleteEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
    }
    ...
    public func test() async throws -> PsmithEmbedderTestResult {
        let resp = await client.testEmbedderConfig(request: Psmith_V1_TestEmbedderConfigRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        let m = resp.message ?? Psmith_V1_TestEmbedderConfigResponse()
        return PsmithEmbedderTestResult(ok: m.ok, errorMessage: m.errorMessage, latencyMs: m.latencyMs)
    }
    ...
    public func listTypes() async throws -> [String] {
        let resp = await client.listEmbedderTypes(request: Psmith_V1_ListEmbedderTypesRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
    ...
    public func stats() async throws -> PsmithEmbedderStats {
        let resp = await client.getEmbedderStats(request: Psmith_V1_GetEmbedderStatsRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        let m = resp.message ?? Psmith_V1_GetEmbedderStatsResponse()
        return PsmithEmbedderStats(unembeddedCount: m.unembeddedCount, workerActive: m.workerActive)
    }
}
