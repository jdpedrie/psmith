...
public final class ConversationsRepository: Sendable {
    private let client: Psmith_V1_ConversationsServiceClientInterface
    private let cache: PsmithCache?

    public init(
        client: Psmith_V1_ConversationsServiceClientInterface,
    ...
    public func list(
        ...
        if let msg = resp?.message {
            let items = msg.conversations.map(PsmithConversation.init(from:))
        ...
        if pageToken == nil && titleQuery == nil && profileID == nil,
           let cache,
           let cached: [PsmithConversation] = await cache.get([PsmithConversation].self, kind: CacheKind.conversationsList, id: "all") {
            return (cached, nil)
        }
    ...
    public func get(id: String) async throws -> (PsmithConversation, PsmithContext) {
        ...
        if let msg = resp.message {
            let conv = PsmithConversation(from: msg.conversation)
            let ctx = PsmithContext(from: msg.activeContext)
            if let cache {
        ...
        throw resp.error.map(PsmithError.from) ?? .missingPayload("get conversation")
    }
    ...
    ) async throws -> PsmithConversation {
        var req = Psmith_V1_CreateConversationRequest()
    ...
    public func delete(id: String) async throws {
        var req = Psmith_V1_DeleteConversationRequest()
        req.id = id
    ...
    @discardableResult
    public func updateTitle(id: String, title: String) async throws -> PsmithConversation {
        var req = Psmith_V1_UpdateConversationRequest()
        req.id = id
        req.title = title
        let resp = await client.updateConversation(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("update conversation") }
        return PsmithConversation(from: msg.conversation)
    }
    ...
    public func updateSettings(
        id: String,
        title: String? = nil,
        settings: PsmithConversationSettings
    ) async throws -> PsmithConversation {
        var req = Psmith_V1_UpdateConversationRequest()
        req.id = id
        if let title { req.title = title }
    ...
    public func sendMessage(
        ...
        deviceFacts: [PsmithDeviceFact] = []
    ) async throws -> (userMessage: PsmithMessage, streamRun: PsmithStreamRun) {
        var req = Psmith_V1_SendMessageRequest()
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("send message") }
        return (PsmithMessage(from: msg.userMessage), PsmithStreamRun(from: msg.streamRun))
    }
    ...
    public func regenerateAssistant(
        ...
        providerID: String? = nil,
        modelID: String? = nil
    ) async throws -> (parentMessage: PsmithMessage, streamRun: PsmithStreamRun) {
        var req = Psmith_V1_SendMessageRequest()
        req.conversationID = conversationID
        ...
        if let modelID { req.modelID = modelID }
        let resp = await client.sendMessage(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("regenerate") }
        return (PsmithMessage(from: msg.userMessage), PsmithStreamRun(from: msg.streamRun))
    }
    ...
    public func listMessages(
        ...
        if !fullTree && leafMessageID == nil,
           let cache,
           let cached: [PsmithMessage] = await cache.get([PsmithMessage].self, kind: CacheKind.messagesByContext, id: contextID) {
            return cached
        }
        throw resp.error.map(PsmithError.from) ?? .missingPayload("list messages")
    }
    ...
    public func setCurrentLeaf(contextID: String, messageID: String?) async throws {
        var req = Psmith_V1_SetCurrentLeafRequest()
        req.contextID = contextID
    ...
    public func countContextTokens(contextID: String, providerID: String, modelID: String) async throws -> (tokenCount: Int32, contextWindow: Int32) {
        var req = Psmith_V1_CountContextTokensRequest()
        req.contextID = contextID
        req.providerID = providerID
        req.modelID = modelID
        let resp = await client.countContextTokens(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("count context tokens") }
    ...
    public func compact(
        ...
        providerID: String? = nil,
        modelID: String? = nil
    ) async throws -> PsmithStreamRun {
        var req = Psmith_V1_CompactRequest()
        req.conversationID = conversationID
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("compact") }
        return PsmithStreamRun(from: msg.streamRun)
    }

    public func promoteCompactionToNewContext(messageID: String) async throws -> PsmithContext {
        var req = Psmith_V1_PromoteCompactionToNewContextRequest()
        req.messageID = messageID
        let resp = await client.promoteCompactionToNewContext(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("promote compaction") }
    ...
    public func createContextManual(
        ...
        mode: PsmithCompressionMode
    ) async throws -> (context: PsmithContext, userMessage: PsmithMessage?) {
        var req = Psmith_V1_CreateContextManualRequest()
        req.conversationID = conversationID
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("create context manual") }
        let userMsg: PsmithMessage? = msg.hasUserMessage ? PsmithMessage(from: msg.userMessage) : nil
    ...
    public func editMessage(
        id: String,
        content: String,
        role: PsmithMessageRole? = nil
    ) async throws -> PsmithMessage {
        var req = Psmith_V1_EditMessageRequest()
        req.id = id
        req.content = content
        ...
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("edit message") }
        return PsmithMessage(from: msg.message)
    }

    public func deleteMessage(id: String, cascade: Bool = false) async throws {
        var req = Psmith_V1_DeleteMessageRequest()
        req.id = id
        req.cascade = cascade
    ...
    public func listContexts(conversationID: String) async throws -> [PsmithContext] {
        var req = Psmith_V1_ListContextsRequest()
        req.conversationID = conversationID
        let resp = await client.listContexts(request: req, headers: [:])
        if let msg = resp.message {
            let items = msg.contexts.map(PsmithContext.init(from:))
            if let cache {
        ...
        if let cache,
           let cached: [PsmithContext] = await cache.get([PsmithContext].self, kind: CacheKind.contextsByConversation, id: conversationID) {
            return cached
        }
        throw resp.error.map(PsmithError.from) ?? .missingPayload("list contexts")
    }

    public func activateContext(contextID: String) async throws -> PsmithContext {
        var req = Psmith_V1_ActivateContextRequest()
        req.contextID = contextID
        let resp = await client.activateContext(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("activate context") }
        return PsmithContext(from: msg.context)
    }
    ...
    public func getPlugins(conversationID: String) async throws -> [PsmithConversationPlugin] {
        var req = Psmith_V1_GetConversationPluginsRequest()
        req.conversationID = conversationID
        let resp = await client.getConversationPlugins(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("get conversation plugins") }
        return msg.plugins.map(PsmithConversationPlugin.init(from:))
    }
    ...
    @discardableResult
    public func setPlugins(conversationID: String, plugins: [PsmithConversationPlugin]) async throws -> [PsmithConversationPlugin] {
        var req = Psmith_V1_SetConversationPluginsRequest()
        req.conversationID = conversationID
        req.plugins = plugins.map { $0.proto }
        let resp = await client.setConversationPlugins(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("set conversation plugins") }
        return msg.plugins.map(PsmithConversationPlugin.init(from:))
    }
    ...
    public func resolvePipeline(conversationID: String) async throws -> [PsmithResolvedPipelineEntry] {
        var req = Psmith_V1_ResolveConversationPipelineRequest()
        req.conversationID = conversationID
        let resp = await client.resolveConversationPipeline(request: req, headers: [:])
        guard let msg = resp.message else { throw resp.error.map(PsmithError.from) ?? .missingPayload("resolve conversation pipeline") }
        return msg.entries.map(PsmithResolvedPipelineEntry.init(from:))
    }
}

extension PsmithError {
    public static func from(_ err: ConnectError) -> PsmithError {
