...
/// `Psmith_V1_SupportedTool`. Settings UI uses this to render per-
/// tool documentation; the dispatcher uses `name` as the handler
/// key.
public struct PsmithSupportedTool: Sendable, Hashable {
    ...
    init(from p: Psmith_V1_SupportedTool) {
        name = p.name
...
public final class DeviceToolsRepository: Sendable {
    private let client: Psmith_V1_DeviceToolsServiceClientInterface
    private let host: URL
    private let tokenStore: TokenStore
    private let session: URLSession

    public init(
        client: Psmith_V1_DeviceToolsServiceClientInterface,
        host: URL,
    ...
    public func registerCapabilities(
        ...
        var req = Psmith_V1_RegisterCapabilitiesRequest()
    ...
    public func listSupportedTools() async throws -> [PsmithSupportedTool] {
        let resp = await client.listSupportedTools(
            request: Psmith_V1_ListSupportedToolsRequest(), headers: [:])
        if let err = resp.error { throw PsmithError.from(err) }
        let tools = resp.message?.tools ?? []
        return tools.map(PsmithSupportedTool.init(from:))
    }
...
/// One audit-log row. Mirrors `Psmith_V1_DeviceToolCall`. Used by
/// the Settings → Device tool activity scroll on iOS.
public struct PsmithDeviceToolCall: Sendable, Hashable, Identifiable {
    ...
    init(from p: Psmith_V1_DeviceToolCall) {
        id = p.id
...
public extension DeviceToolsRepository {
    ...
    func listCalls(
    ...
    ) async throws -> [PsmithDeviceToolCall] {
        var req = Psmith_V1_ListDeviceToolCallsRequest()
        if let before {
        ...
        if let err = resp.error { throw PsmithError.from(err) }
        return (resp.message?.calls ?? []).map(PsmithDeviceToolCall.init(from:))
    }
}
