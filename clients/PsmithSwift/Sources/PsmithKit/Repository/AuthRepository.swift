...
public enum PsmithError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .rpc(code, message):
            ...
            return PsmithError.humanise(rpcCode: code, message: message)
        case let .missingPayload(field):
            return "Missing field in server response: \(field)"
        case let .localStorage(err):
            return "Couldn't read local storage: \(PsmithError.display(err))"
        }
    }
    ...
    public static func display(_ error: Error) -> String {
        if let ce = error as? PsmithError, let s = ce.errorDescription, !s.isEmpty {
            return s
        }
...
/// Result of probing a candidate psmithd URL. The login screen uses
/// this to decide whether to advance to the username/password form
/// (`.ok`) or surface a tailored failure message.
public enum PsmithProbeResult: Sendable, Equatable {
    /// Server answered cleanly. `serverName` should be "psmithd" for the
    ...
    /// service running at this URL, or an old psmithd that doesn't yet
    /// implement Probe.
...
/// Probes a candidate psmithd URL without authentication. Doesn't hold
/// onto state — constructs a one-shot PsmithClient against the given
...
@MainActor
public func probePsmithServer(url: URL) async -> PsmithProbeResult {
    let probeStore = InMemoryTokenStore()
    let probeAuth = AuthState()
    let client = PsmithClient(host: url, tokenStore: probeStore, authState: probeAuth)
...
public final class AuthRepository: Sendable {
    private let client: Psmith_V1_AuthServiceClientInterface
    private let tokenStore: TokenStore
    private let authState: AuthState
    private let cache: PsmithCache?

    public init(
        client: Psmith_V1_AuthServiceClientInterface,
    ...
    /// Calls AuthService.Probe. Returns a typed PsmithProbeResult so the
    ...
    /// three cases.
    public func probe() async -> PsmithProbeResult {
        let resp = await client.probe(request: Psmith_V1_ProbeRequest(), headers: [:])
        ...
        if let err = resp.error {
            ...
            // signal (something is at this URL but it isn't a psmithd that
    ...
    public func login(username: String, password: String, clientLabel: String? = nil) async throws -> PsmithUser {
        var req = Psmith_V1_LoginRequest()
        ...
        guard let msg = resp.message else {
            throw map(error: resp.error) ?? PsmithError.missingPayload("login response")
        }
        do { try tokenStore.save(msg.sessionToken) } catch { throw PsmithError.localStorage(error) }

        let user = PsmithUser(from: msg.user)
        await MainActor.run { authState.setAuthenticated(user) }
        return user
    }

    public func whoAmI() async throws -> PsmithUser {
        let resp = await client.whoAmI(request: Psmith_V1_WhoAmIRequest(), headers: [:])
        guard let msg = resp.message else {
            throw map(error: resp.error) ?? PsmithError.missingPayload("whoami response")
        }
        let user = PsmithUser(from: msg.user)
        await MainActor.run { authState.setAuthenticated(user) }
    ...
    public func logout() async throws {
        let resp = await client.logout(request: Psmith_V1_LogoutRequest(), headers: [:])
    ...
    public func restoreSession() async -> PsmithUser? {
        ...
        do {
            ...
            if case let PsmithError.rpc(code, _) = error,
            ...
            if let cache,
               let cached: PsmithUser = await cache.get(PsmithUser.self, kind: CacheKind.currentUser, id: "me") {
    ...
    private func map(error: ConnectError?) -> PsmithError? {
        guard let error else { return nil }
        return .rpc(code: error.code, message: error.message ?? error.code.name)
    }
}

extension PsmithUser {
    init(from p: Psmith_V1_User) {
        self.init(
            id: p.id,
            username: p.username,
