...
/// Renders one row per `PsmithConfigField` against a shared `[String: Any]`
...
public struct PluginConfigForm: View {
    ...
    /// chooser.
    let availableModels: [PsmithUserModel]
    ...
    public init(
        fields: [PsmithConfigField],
        config: Binding<[String: Any]>,
        availableModels: [PsmithUserModel] = [],
    ...
    private func groupedFields() -> [(String, [PsmithConfigField])] {
        var order: [String] = []
        var seen = Set<String>()
        var buckets: [String: [PsmithConfigField]] = [:]
        for f in fields {
    ...
    @ViewBuilder
    private func fieldRow(_ field: PsmithConfigField) -> some View {
    ...
    @ViewBuilder
    private func control(for field: PsmithConfigField) -> some View {
        switch field.type {
    ...
    private func modelPickerField(_ field: PsmithConfigField) -> some View {
    ...
    private func pickerSelection(for field: PsmithConfigField) -> (providerID: String, modelID: String)? {
    ...
    private func numberField(_ field: PsmithConfigField) -> some View {
    ...
    private func textField(_ field: PsmithConfigField) -> some View {
        let binding = Binding<String>(
            get: {
    ...
    private func textareaField(_ field: PsmithConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
                return decodeDefaultString(field) ?? ""
    ...
    private func booleanField(_ field: PsmithConfigField) -> some View {
    ...
    @ViewBuilder
    private func selectField(_ field: PsmithConfigField) -> some View {
        let binding = Binding<String>(
            get: {
                if let s = config[field.name] as? String { return s }
    ...
    private func decodeDefaultString(_ field: PsmithConfigField) -> String? {
    ...
    private func decodeDefaultBool(_ field: PsmithConfigField) -> Bool? {
        guard !field.defaultJSON.isEmpty,
              let data = field.defaultJSON.data(using: .utf8) else { return nil }
...
private struct ModelPickerFieldControl: View {
    let field: PsmithConfigField
    let allModels: [PsmithUserModel]
    let providerLabels: [String: String]
    let providerTypes: [String: String]
    let providerPresetIDs: [String: String]
    ...
    private var matchingModels: [PsmithUserModel] {
