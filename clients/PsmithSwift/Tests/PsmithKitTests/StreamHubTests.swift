...
struct StreamHubTests {
    ...
    private func freshDefaults() -> UserDefaults {
        let name = "psmith.test.streamhub.\(UUID().uuidString)"
    ...
    private func makeHub(defaults: UserDefaults) async throws -> (StreamHub, PsmithClient) {
    ...
    func markViewing_addsToViewing_andClearsUnseen() async throws {
        ...
        defaults.set(["c-1"], forKey: "psmith.streamHub.unseenConversationIDs.v1")
        ...
        let persisted = defaults.array(forKey: "psmith.streamHub.unseenConversationIDs.v1") as? [String] ?? []
    ...
    func markSeen_clearsUnseen_withoutTouchingViewing() async throws {
        let defaults = freshDefaults()
        defaults.set(["c-1", "c-2"], forKey: "psmith.streamHub.unseenConversationIDs.v1")
        let (hub, _) = try await makeHub(defaults: defaults)
        ...
        #expect(!hub.viewingConversationIDs.contains("c-1"))
        let persisted = Set(defaults.array(forKey: "psmith.streamHub.unseenConversationIDs.v1") as? [String] ?? [])
