...
/// Layer 1 integration tests for AuthRepository against a real psmithd
...
struct AuthRepositoryTests {
    ...
    func loginRecordsClientLabel() async throws {
        ...
        let user = try await client.auth.login(
            username: username, password: password, clientLabel: "PsmithMac/integration-test"
        )
        #expect(user.username == username)
    ...
    @Test("probe against the running psmithd succeeds")
