...
struct AppModelTests {
    ...
    func bootstrapRestoresSession() async throws {
        ...
        // expose a direct reader for tokenStore on PsmithClient, so seed a
    ...
    private func mintSessionToken(server: TestPsmithdServer, username: String, password: String) async throws -> String {
    ...
    private func rawLoginToken(server: TestPsmithdServer, username: String, password: String) async throws -> String {
        ...
        let raw = Psmith_V1_AuthServiceClient(client: proto)
        var req = Psmith_V1_LoginRequest()
        req.username = username
        req.password = password
        let resp = await raw.login(request: req, headers: [:])
        guard let msg = resp.message else {
            throw PsmithError.from(resp.error ?? ConnectError(code: .unknown, message: "login failed"))
        }
