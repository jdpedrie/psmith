...
/// Layer 1 integration tests for ProfilesRepository against a real psmithd
...
struct ProfilesRepositoryTests {
    ...
    func listSeededOnly() async throws {
        ...
        // Assistant + Psmith Manager) — a fresh user is never empty.
        ...
        #expect(names == ["Personal Assistant", "Psmith Manager"])
    }
    ...
    func getResolvedFromParent() async throws {
        ...
        let parent = try await client.profiles.create(PsmithProfilePatch(
            name: "Parent",
            systemMessage: "I am the parent system prompt.",
            parentOnly: true
        ))
        let child = try await client.profiles.create(PsmithProfilePatch(
            name: "Child",
    ...
    @Test("create with full PsmithProfilePatch round-trips every field")
    func createFullPatch() async throws {
        let (client, _) = try await TestSession.freshUser(server: server, usernamePrefix: "prof-full")
        let parent = try await client.profiles.create(PsmithProfilePatch(name: "Parent", parentOnly: true))
        let patch = PsmithProfilePatch(
            name: "Child",
            parentProfileID: parent.id,
        ...
        let titled = try await client.profiles.create(PsmithProfilePatch(
            name: "Titled",
            titleGuide: "Use 4 words.",
            titleProviderKind: PsmithTitleProviderKind.appleFoundation
        ))
        #expect(titled.titleGuide == "Use 4 words.")
        #expect(titled.titleProviderKind == PsmithTitleProviderKind.appleFoundation)
    }
    ...
    func createParentOnly() async throws {
        ...
        let created = try await client.profiles.create(
            PsmithProfilePatch(name: "Trunk", parentOnly: true)
        )
    ...
    func createFavorite() async throws {
        ...
        let created = try await client.profiles.create(
            PsmithProfilePatch(name: "Star", favorite: true)
        )
        #expect(created.favorite == true)
    }
    ...
    func createEmptyName() async throws {
        ...
        do {
            _ = try await client.profiles.create(PsmithProfilePatch(name: ""))
            Issue.record("expected InvalidArgument")
        } catch let PsmithError.rpc(code, _) {
            #expect(code == .invalidArgument)
        }
    ...
    func updatePartialPatch() async throws {
        ...
        let created = try await client.profiles.create(PsmithProfilePatch(
            name: "Before",
        ...
        let updated = try await client.profiles.update(
            id: created.id,
            patch: PsmithProfilePatch(name: "After")
        )
    ...
    func updateNotFoundCrossUser() async throws {
        ...
        do {
            _ = try await clientB.profiles.update(
                id: owned.id, patch: PsmithProfilePatch(name: "stolen")
            )
    ...
    func deleteWithChildrenFailsPrecondition() async throws {
        ...
        let parent = try await client.profiles.create(PsmithProfilePatch(name: "Parent", parentOnly: true))
        _ = try await client.profiles.create(PsmithProfilePatch(
