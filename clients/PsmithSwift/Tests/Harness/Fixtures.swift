...
public enum Fixtures {
    ...
    public static func minimalProfilePatch(name: String = "Test Profile") -> PsmithProfilePatch {
        PsmithProfilePatch(name: name)
    }

    /// Full-fat patch covering every optional field — used by tests that
    /// verify create() round-trips the entire shape.
    public static func fullProfilePatch(name: String = "Full Profile") -> PsmithProfilePatch {
        PsmithProfilePatch(
            name: name,
    ...
    public static func unknownPlugin(ordinal: Int32 = 0) -> PsmithProfilePlugin {
        PsmithProfilePlugin(
    ...
    public static func seedFakeProvider(
        client: PsmithClient,
        label: String = "Fake"
    ) async throws -> (provider: PsmithUserModelProvider, fake: FakeProvider) {
    ...
    ) async throws -> (fake: FakeProvider, provider: PsmithUserModelProvider, model: PsmithUserModel, profile: PsmithProfile) {
