...
/// Boots a real psmithd subprocess against an isolated, freshly-migrated
...
///   1. `PSMITH_TEST_BIN` env var if set,
///   2. `<repoRoot>/bin/psmithd-test` (pre-built by the harness on first call),
///   3. otherwise, `go build -o bin/psmithd-test ./cmd/psmithd` is run lazily.
///
/// The server is created lazily via `TestPsmithdServer.shared()` so XCTest's
...
public final class TestPsmithdServer: @unchecked Sendable {
    ...
    nonisolated(unsafe) private static var sharedInstance: TestPsmithdServer?

    /// Returns the process-wide shared server, booting it on first call.
    /// Subsequent callers wait for the boot to complete.
    public static func shared() throws -> TestPsmithdServer {
        lock.lock()
        defer { lock.unlock() }
        if let s = sharedInstance { return s }
        let server = try TestPsmithdServer.start()
        sharedInstance = server
        // Tear down on process exit. atexit can't capture self by reference,
        // so we read from the static slot.
        atexit {
            TestPsmithdServer.lock.lock()
            let s = TestPsmithdServer.sharedInstance
            TestPsmithdServer.sharedInstance = nil
            TestPsmithdServer.lock.unlock()
            s?.shutdown()
        }
        return server
    }
    ...
    public static func start() throws -> TestPsmithdServer {
        ...
        let dbName = "psmith_swift_test_\(UUID().uuidString.lowercased().replacingOccurrences(of: "-", with: ""))"
        ...
        env["PSMITH_DSN"] = testDSN
        env["PSMITH_ADDR"] = listenAddr
        env["PSMITH_BOOTSTRAP_ADMIN_USERNAME"] = "admin"
        env["PSMITH_BOOTSTRAP_ADMIN_PASSWORD"] = "admin-password-not-secret"
        // Disable the catalog refresh loop — tests don't need it and it
        // makes outbound network calls.
        env["PSMITH_CATALOG_REFRESH_INTERVAL"] = "0"
        ...
        let server = TestPsmithdServer(
            ...
            dsn: testDSN,
            dbName: dbName,
            adminUsername: env["PSMITH_BOOTSTRAP_ADMIN_USERNAME"]!,
            adminPassword: env["PSMITH_BOOTSTRAP_ADMIN_PASSWORD"]!,
            process: proc,
            stdoutPipe: outPipe,
            stderrPipe: errPipe,
            stdoutBuffer: outBuf,
    ...
    private func waitUntilReady(timeoutSeconds: Double) throws {
        ...
        while Date() < deadline {
            if !process.isRunning {
                ...
                throw HarnessError.serverFailedToStart("psmithd exited before becoming ready:\n\(err)")
            }
        ...
        throw HarnessError.serverFailedToStart(
            "psmithd did not bind \(port) within \(timeoutSeconds)s. stderr:\n\(stderrBuffer.snapshotString())"
        )
    }
    ...
    public func shutdown() {
        ...
        do {
            try TestPsmithdServer.dropDatabase(
                dbName: dbName,
...
enum HarnessError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .binaryBuildFailed(let s): return "go build of psmithd failed: \(s)"
...
private func ensureBinary(repoRoot: String) throws -> String {
    if let override = ProcessInfo.processInfo.environment["PSMITH_TEST_BIN"], !override.isEmpty {
        return override
    }
    ...
    proc.arguments = ["go", "build", "-o", "bin/psmithd-test", "./cmd/psmithd"]
    proc.currentDirectoryURL = URL(fileURLWithPath: repoRoot)
    let pipe = Pipe()
...
extension TestPsmithdServer {
    fileprivate static func dropDatabase(dbName: String, rootDSN: String) throws {
        ...
        // psmithd hasn't fully released its pool.
        let (rc, out) = try runPSQLInContainer(
            user: user, password: pass, db: root,
