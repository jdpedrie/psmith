...
struct ConversationSettingsViewSnapshots {
    ...
    private func conversationFor(providerID: String, modelID: String) -> SpaltConversation {
        var settings = SpaltConversationSettings()
        settings.defaultProviderID = providerID
        settings.defaultModelID = modelID
        return SpaltConversation(
            id: "conv-1",
    ...
    func noOverrides() {
        ...
        let inherited = SpaltCallSettings(
            temperature: 1.0,
            topP: 0.95,
            maxOutputTokens: 4_096,
            stopSequences: [],
            topK: 50,
            thinking: SpaltThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
            conversationCallSettingsDraft: SpaltCallSettings(),
    ...
    func temperatureOverride() {
        ...
        let inherited = SpaltCallSettings(temperature: 1.0, topP: 0.95, maxOutputTokens: 4_096)
        let model = SnapshotStubs.makeConversationViewModel(
            messages: [SnapshotFixtures.systemMessage()],
            showingSettingsView: true,
    ...
    func thinkingOverride() {
        var draft = SpaltCallSettings()
        draft.thinking = SpaltThinkingSettings(enabled: true, budgetTokens: 4_096)
        let inherited = SpaltCallSettings(
            temperature: 1.0,
            thinking: SpaltThinkingSettings(enabled: false, budgetTokens: 1_024)
        )
    ...
    func anthropicProvider() {
        ...
        var draft = SpaltCallSettings()
        draft.anthropic = SpaltAnthropicExtras(cacheEnabled: false, cacheTTL: .oneHour)
        let inherited = SpaltCallSettings(
            temperature: 1.0,
            anthropic: SpaltAnthropicExtras(cacheEnabled: true, cacheTTL: .fiveMinutes)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func openaiProvider() {
        ...
        let openaiModel = SpaltUserModel(
        ...
        var draft = SpaltCallSettings()
        draft.openai = SpaltOpenAIExtras(seed: 42, frequencyPenalty: 0.5)
        let inherited = SpaltCallSettings(
            temperature: 0.8,
            openai: SpaltOpenAIExtras(seed: 0, frequencyPenalty: 0)
        )
        let model = SnapshotStubs.makeConversationViewModel(
    ...
    func googleProvider() {
        ...
        let googleModel = SpaltUserModel(
        ...
        var draft = SpaltCallSettings()
        draft.google = SpaltGoogleExtras(
        ...
        let inherited = SpaltCallSettings(
            temperature: 0.7,
            google: SpaltGoogleExtras(
                safetySettings: SpaltSafetySettings(harassment: .blockNone),
                candidateCount: 1
            )
        )
        let model = SnapshotStubs.makeConversationViewModel(
