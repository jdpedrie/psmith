...
struct ConversationView: View {
    let conversation: SpaltConversation
    ...
    private var liveConversation: SpaltConversation {
...
struct ConversationBody: View {
    ...
    let liveConversation: SpaltConversation
    @Environment(AppModel.self) private var app
    ...
    var body: some View {
        ...
        // window's solid backgroundColor (set in SpaltMacApp); the AppKit
    ...
    private func contextLabel(_ ctx: SpaltContext) -> String {
    ...
    private var groupedModels: [(providerID: String, label: String, models: [SpaltUserModel])] {
...
private struct MessageRow: View {
    ...
    @State private var editRoleDraft: SpaltMessageRole = .user
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if isEditableRole {
                Picker("Role", selection: $editRoleDraft) {
                    Text("User").tag(SpaltMessageRole.user)
                    Text("Assistant").tag(SpaltMessageRole.assistant)
                }
    ...
    private func cacheEfficiencyGrade(_ u: SpaltMessageUsage) -> CacheGrade? {
    ...
    private func usageSummary(_ u: SpaltMessageUsage) -> String {
        var parts: [String] = []
    ...
    private func thinkingDurationSeconds(for message: SpaltMessage) -> Double? {
...
private struct MessageUsagePopover: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let u = message.usage {
                ...
                // (Gemini's automatic) from explicit ones (Spalt
    ...
    private func cacheSavings(message: SpaltMessage, cacheReadTokens: Int32) -> Double? {
