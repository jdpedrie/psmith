...
private struct ProfileRow: View {
    let profile: SpaltProfile
    let profiles: [SpaltProfile]
...
private struct ProfileViewer: View {
    ...
    init(profile: SpaltProfile, model: ProfilesViewModel) {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    if hasTitleOverrides {
                        section("Auto-titling") {
                            VStack(alignment: .leading, spacing: 4) {
                                if profile.titleProviderKind == SpaltTitleProviderKind.appleFoundation {
                                    HStack(spacing: 6) {
    ...
    private func modeLabel(_ m: SpaltCompressionMode) -> String {
        switch m {
...
private struct ProfileForm: View {
    ...
    @State private var callSettingsDraft = SpaltCallSettings()
    ...
    /// `SpaltTitleProviderKind.appleFoundation` ("apple_foundation") is
    ...
    private var titleGeneratorPicker: some View {
        let isLocal = titleProviderKind == SpaltTitleProviderKind.appleFoundation
    ...
    private var profileModelCapabilities: SpaltModelCapabilities? {
    ...
    private var profileInheritedCallSettings: SpaltCallSettings? {
        guard let pid = defaultProviderID, let mid = defaultModelID else { return nil }
        return model.availableModels
            .first(where: { $0.providerID == pid && $0.modelID == mid })?
            .defaultSettings
    ...
    private func save() async {
        ...
        let trimmedCallSettings: SpaltCallSettings? = callSettingsDraft.isEmpty ? nil : callSettingsDraft
        let hasAnyDefault = defaultProviderID != nil || defaultModelID != nil || trimmedCallSettings != nil
        if hasAnyDefault {
            patch.defaultSettings = SpaltProfileDefaults(
        ...
        if titleProviderKind == SpaltTitleProviderKind.appleFoundation {
            ...
            patch.titleProviderKind = SpaltTitleProviderKind.appleFoundation
        } else {
    ...
    @ViewBuilder
    private func capabilityChips(_ caps: SpaltPluginCapabilities?) -> some View {
        if let caps {
            HStack(spacing: 4) {
    ...
    private func pluginTypeCard(_ pluginType: SpaltPluginType) -> some View {
        Button {
