...
private struct ModelTestResultChip: View {
    ...
    private func successText(_ r: SpaltModelTestResult) -> String {
        var bits: [String] = []
...
private enum AddProviderSelection: Equatable {
    case template(SpaltProviderTemplate)
    case custom
}
...
private struct EditProviderForm: View {
    ...
    private var openAITemplates: [SpaltProviderTemplate] {
...
private struct DiscoverModelsInline: View {
    ...
    private var filtered: [SpaltDiscoveredModel] {
...
private struct DiscoveredModelRow: View {
    let model: SpaltDiscoveredModel
    let isSelected: Bool
    let onToggle: () -> Void
    @Environment(\.theme) private var theme
...
private struct ProviderDefaultSettingsTab: View {
    let provider: SpaltUserModelProvider
    @Bindable var model: ProvidersViewModel
    @State private var draft = SpaltCallSettings()
    ...
    var body: some View {
        ...
        .onDisappear {
            ...
            let original = provider.defaultSettings ?? SpaltCallSettings()
...
enum ModelEditMode {
    case adding(provider: SpaltUserModelProvider)
    case editing(model: SpaltUserModel, provider: SpaltUserModelProvider)

    var provider: SpaltUserModelProvider {
        switch self {
    ...
    var existingModel: SpaltUserModel? {
...
private struct ModelEditForm: View {
    ...
    @State private var defaultSettings = SpaltCallSettings()
    ...
    private var provider: SpaltUserModelProvider { formMode.provider }
    ...
    /// Builds a SpaltModelPricing from the four price text fields. Returns nil
    /// if all four are empty/unparseable — caller can decide what nil means
    /// (clear-pricing for editing, omit-pricing for adding).
    private func pricingFromFields() -> SpaltModelPricing? {
    ...
    private var capabilitiesValue: SpaltModelCapabilities {
    ...
    private func seedFromExistingIfNeeded() {
        ...
        defaultSettings = m.defaultSettings ?? SpaltCallSettings()
    }
