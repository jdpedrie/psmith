...
// Name is the registered name for the MCP-server bridge plugin.
//
...
func newMCP(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := mcpConfig{}
	...
	case mcpTransportStdio, mcpTransportHTTP, mcpTransportInproc:
		// nil-config pluginapi.Build (used by pluginapi.Describe) is OK — we register
...
func init() {
	pluginapi.Register(Name, newMCP)
}

func (p *mcpPlugin) Name() string { return Name }
...
func (p *mcpPlugin) ConfigFields() []pluginapi.ConfigField {
...
func (p *mcpPlugin) Tools() []pluginapi.ToolDef {
	if !p.configValid() {
		return nil
	}
	...
	out := make([]pluginapi.ToolDef, len(tools))
	for i, t := range tools {
		out[i] = pluginapi.ToolDef{
...
func (p *mcpPlugin) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	if !p.configValid() {
		return pluginapi.ToolResult{}, errors.New("mcp: plugin not fully configured")
	}
	realName := name
	if p.cfg.ToolPrefix != "" {
...
// tool names (unprefixed). Unlike pluginapi.Plugin.Tools() — which swallows
...
func TestMCPConnection(ctx context.Context, configBytes json.RawMessage) ([]string, error) {
	pl, err := pluginapi.Build(Name, configBytes)
	if err != nil {
		return nil, err
	}
	mp, ok := pl.(*mcpPlugin)
...
func (s *mcpServer) loadTools(ctx context.Context) error {
	...
	tools := make([]pluginapi.ToolDef, 0, len(resp.Tools))
	for _, t := range resp.Tools {
		tools = append(tools, pluginapi.ToolDef{
			Name:        t.Name,
			Description: t.Description,
			InputSchema: t.InputSchema,
		})
...
func (s *mcpServer) toolsSnapshot() []pluginapi.ToolDef {
	s.toolsMu.RLock()
	defer s.toolsMu.RUnlock()
	out := make([]pluginapi.ToolDef, len(s.tools))
	copy(out, s.tools)
	return out
}

func (s *mcpServer) callTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	args := input
	if len(args) == 0 {
	...
	return pluginapi.ToolResult{Output: encoded, Attachments: attachments}, nil
}
...
func (t *inprocTransport) call(ctx context.Context, method string, params any, into any) error {
	...
	if d == nil {
		return errors.New("mcp inproc: no dispatcher registered (cmd/psmithd should call mcp.RegisterInprocMCPDispatcher at startup)")
	}
	id := t.nextID.Add(1)
