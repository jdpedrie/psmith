...
// Name is the registered name for the
// component-builder plugin.
const Name = "component_builder"
...
//   - a pluginapi.ContentRenderer parser that scans the assistant's
...
func newComponentBuilder(configBytes json.RawMessage) (pluginapi.Plugin, error) {
...
func init() {
	pluginapi.Register(Name, newComponentBuilder)
}

func (p *componentBuilder) Name() string        { return Name }
...
// PluginConfigEditor in PsmithUI) that builds the structured
...
func (p *componentBuilder) ConfigFields() []pluginapi.ConfigField {
...
func (p *componentBuilder) TransformHistoryMessage(msg providers.WireMessage, pos pluginapi.HistoryPos) providers.WireMessage {
...
func (p *componentBuilder) StreamingTags() []pluginapi.StreamingTag {
	out := make([]pluginapi.StreamingTag, 0, len(p.defs))
	for _, d := range p.defs {
		name, ok := extractAngleTagName(d.OpenTag, d.CloseTag)
		if !ok {
			continue
		}
		out = append(out, pluginapi.StreamingTag{Tag: name, Component: d.Component})
	}
	return out
}
...
// rely on it).
func (p *componentBuilder) RenderContent(parts []pluginapi.ContentPart, role string) []pluginapi.ContentPart {
	if role != "assistant" || len(p.defs) == 0 {
		return parts
	}
	for _, d := range p.defs {
		def := d
		parts = pluginapi.WalkText(parts, func(text string) []pluginapi.ContentPart {
...
// silent omission).
func scanForDefinition(text string, def componentDef) []pluginapi.ContentPart {
	if !strings.Contains(text, def.OpenTag) {
		return []pluginapi.ContentPart{pluginapi.NewTextPart(text)}
	}
	...
	for cursor < len(text) {
		...
		if openIdx < 0 {
			out = append(out, pluginapi.NewTextPart(text[cursor:]))
			break
		}
		openIdx += cursor
		...
		if openIdx > cursor {
			out = append(out, pluginapi.NewTextPart(text[cursor:openIdx]))
		}
		...
		if !json.Valid([]byte(body)) {
			...
			out = append(out, pluginapi.NewTextPart(text[openIdx:blockEnd]))
		} else {
			out = append(out, pluginapi.NewFragmentPart(def.Component, json.RawMessage(body), ""))
		}
		cursor = blockEnd
	}
