...
// Name is the registered name for the device-files plugin.
const Name = "files"
...
func newFiles(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := filesConfig{}
...
func init() {
	pluginapi.Register(Name, newFiles)
}

func (p *files) Name() string        { return Name }
...
func (p *files) ConfigFields() []pluginapi.ConfigField {
	out := make([]pluginapi.ConfigField, 0, len(filesCatalog))
...
func (p *files) Tools() []pluginapi.ToolDef {
	out := make([]pluginapi.ToolDef, 0, len(filesCatalog))
	for _, t := range filesCatalog {
		if !p.isEnabled(t) {
			continue
		}
...
func (p *files) ToolsForClient(supported map[string]struct{}) []pluginapi.ToolDef {
	if supported == nil {
		return p.Tools()
	}
	out := make([]pluginapi.ToolDef, 0, len(filesCatalog))
...
func (p *files) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	tool := p.findCatalog(name)
	if tool == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("files: unknown tool %q", name)
	}
	if !p.isEnabled(*tool) {
		return pluginapi.ToolResult{}, fmt.Errorf("files: tool %q is disabled for this profile", name)
	}

	broker := host.DeviceToolBrokerFrom(ctx)
	if broker == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("files: no DeviceToolBroker in context — server not wired")
	}
