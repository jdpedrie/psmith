...
func TestTextInjector_DefaultsAreNoOps(t *testing.T) {
	...
	out := ti.TransformHistoryMessage(in, pluginapi.HistoryPos{FromHead: 0, FromHeadSameRole: 0})
...
func TestTextInjector_UserPrefixSuffixOnEveryUserMessage(t *testing.T) {
	...
	out := ti.TransformHistoryMessage(in, pluginapi.HistoryPos{FromHead: 4, FromHeadSameRole: 2})
	want := "PRE\n\nwhat time is it\n\nPOST"
	if out.Content != want {
...
func TestTextInjector_HeadReminderOnlyAtHead(t *testing.T) {
	...
	head := ti.TransformHistoryMessage(
		...
		pluginapi.HistoryPos{FromHead: 0, FromHeadSameRole: 0},
	)
	...
	older := ti.TransformHistoryMessage(
		providers.WireMessage{Role: "user", Content: "ok"},
		pluginapi.HistoryPos{FromHead: 4, FromHeadSameRole: 2},
	)
	if older.Content != "ok" {
...
func TestTextInjector_ConfigFieldsCoverAllOptions(t *testing.T) {
	...
	for _, f := range fields {
		got[f.Name] = true
		if f.Type != pluginapi.ConfigFieldTextarea {
...
func TestTextInjector_RegisteredAtInit(t *testing.T) {
	t.Parallel()
	pl, err := pluginapi.Build(Name, []byte(`{"system_prefix": "x"}`))
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
	if _, ok := pl.(pluginapi.SystemPrompter); !ok {
		t.Error("text_injector must implement pluginapi.SystemPrompter")
	}
	if _, ok := pl.(pluginapi.HistoryTransformer); !ok {
		t.Error("text_injector must implement pluginapi.HistoryTransformer")
	}
	if _, ok := pl.(pluginapi.Configurable); !ok {
		t.Error("text_injector must implement pluginapi.Configurable")
	}
}
