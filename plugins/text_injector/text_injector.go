...
// Name is the registered name for the text-injector plugin.
const Name = "text_injector"
...
//
//   - pluginapi.SystemPrompter prepend → `system_prefix`
//   - pluginapi.SystemPrompter append  → `system_suffix`
//   - pluginapi.HistoryTransformer on every user message:
//   - `user_prefix` prepended
//   - `user_suffix` appended
//   - pluginapi.HistoryTransformer on the head user message only:
//   - `user_head_reminder` appended
//
// Every user-side hook lives in pluginapi.HistoryTransformer (NOT
...
type textInjectorConfig struct {
	...
	// every send. Routed through pluginapi.SystemPrompter so it composes
...
func newTextInjector(configBytes json.RawMessage) (pluginapi.Plugin, error) {
...
func init() {
	pluginapi.Register(Name, newTextInjector)
}

func (p *textInjector) Name() string        { return Name }
...
func (p *textInjector) ConfigFields() []pluginapi.ConfigField {
	...
	// `Merge: pluginapi.MergeAppendString` so the resolver concatenates
	...
	return []pluginapi.ConfigField{
		{
			...
			Merge:       pluginapi.MergeAppendString,
		},
		{
			Name:        "system_suffix",
...
//
// The output isn't persisted (pluginapi.HistoryTransformer runs at wire-build
// time on a per-send basis), so editing config affects only future
// sends — the messages table never sees these additions.
func (p *textInjector) TransformHistoryMessage(msg providers.WireMessage, pos pluginapi.HistoryPos) providers.WireMessage {
	if msg.Role != "user" {
		return msg
	}
