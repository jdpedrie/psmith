...
func TestBasicGrounding_DefaultsAndDescriptor(t *testing.T) {
	...
	if pl.Name() != Name {
		t.Errorf("Name() = %q, want %q", pl.Name(), Name)
	}
	if pl.DisplayName() == "" {
	...
	c, ok := pl.(pluginapi.Configurable)
	if !ok {
		t.Fatal("plugin must implement pluginapi.Configurable")
	}
	fields := c.ConfigFields()
	if len(fields) == 0 {
		t.Error("expected at least one pluginapi.ConfigField")
	}
	...
	for _, f := range fields {
		if !expectedNames[f.Name] {
			t.Errorf("unexpected pluginapi.ConfigField %q", f.Name)
		}
	}
}
...
func TestBasicGrounding_HeaderBlock(t *testing.T) {
	...
	header, trailer := bg.OutgoingMessageEnvelope(context.Background(), nil)
	if trailer != "" {
...
func TestBasicGrounding_DisplayStripsLegacyEmbeddedBlock(t *testing.T) {
	...
	header, _ := bg.OutgoingMessageEnvelope(context.Background(), nil)
	legacy := header + "\n\n" + original
...
func TestBasicGrounding_TimezoneFactUsedWhenConfigEmpty(t *testing.T) {
	t.Parallel()
	bg := newFixedClockGrounding(t, ``)
	out, _ := bg.OutgoingMessageEnvelope(context.Background(), map[string]string{
		pluginapi.DeviceFactKeyTimezone: "America/New_York",
	})
...
func TestBasicGrounding_NoTimezoneFallsBackToUTC(t *testing.T) {
	...
	bg := newFixedClockGrounding(t, ``)
	out, _ := bg.OutgoingMessageEnvelope(context.Background(), nil)
...
func TestBasicGrounding_BadTimezoneFactFallsBackToUTC(t *testing.T) {
	...
	bg := newFixedClockGrounding(t, ``)
	out, _ := bg.OutgoingMessageEnvelope(context.Background(), map[string]string{
		pluginapi.DeviceFactKeyTimezone: "Pluto/Olympus",
	})
	if !strings.Contains(out, "Current time: 2026-05-02T14:45:00Z") {
...
func TestBasicGrounding_RequestsTimezoneFactOnlyWhenNeeded(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := pl.(pluginapi.DeviceFactRequester)
			got := r.RequestedDeviceFacts()
			has := false
			for _, k := range got {
				if k == pluginapi.DeviceFactKeyTimezone {
					has = true
					break
				}
			}
...
func TestBasicGrounding_RendersDeviceFacts(t *testing.T) {
	...
	out, _ := bg.OutgoingMessageEnvelope(context.Background(), map[string]string{
		pluginapi.DeviceFactKeyLocale:         "en-US",
		pluginapi.DeviceFactKeyPlatform:       "iOS 26.5 / iPhone 17 Pro",
		pluginapi.DeviceFactKeyLocationCity:   "Brooklyn, NY",
		pluginapi.DeviceFactKeyLocationCoords: "40.6782,-73.9442",
	})
	wants := []string{
...
func TestBasicGrounding_RequestedFactsReflectsConfig(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "all_off_except_datetime", config: `{"include_locale":false,"include_platform":false,"include_location":false}`,
			want: []string{pluginapi.DeviceFactKeyTimezone}},
		{name: "locale_only_with_datetime", config: `{"include_locale":true,"include_platform":false,"include_location":false}`,
			want: []string{pluginapi.DeviceFactKeyTimezone, pluginapi.DeviceFactKeyLocale}},
		{name: "all_on", config: `{"include_locale":true,"include_platform":true,"include_location":true}`,
			want: []string{pluginapi.DeviceFactKeyTimezone, pluginapi.DeviceFactKeyLocale, pluginapi.DeviceFactKeyPlatform, pluginapi.DeviceFactKeyLocationCity, pluginapi.DeviceFactKeyLocationCoords}},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r, ok := pl.(pluginapi.DeviceFactRequester)
			if !ok {
				t.Fatal("basic_grounding should implement pluginapi.DeviceFactRequester")
			}
			got := r.RequestedDeviceFacts()
...
func TestBasicGrounding_HeaderRendersOnceAtWriteTime(t *testing.T) {
	...
	// builder's tests.
	bg := newFixedClockGrounding(t, `{"timezone":"UTC"}`)
	stored, _ := bg.OutgoingMessageEnvelope(context.Background(), nil)
	if !strings.HasPrefix(stored, "<grounding>") {
		t.Fatalf("stored header should carry the block, got %q", stored)
	}
	again, _ := bg.OutgoingMessageEnvelope(context.Background(), nil)
	if again != stored {
