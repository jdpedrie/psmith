...
func TestAppTools_Descriptor(t *testing.T) {
	...
	cfg, ok := pluginapi.Plugin(p).(pluginapi.Configurable)
	if !ok {
		t.Fatal("app_tools must implement pluginapi.Configurable")
	}
	fields := cfg.ConfigFields()
	if len(fields) == 0 {
		t.Error("expected one pluginapi.ConfigField per catalog tool, got 0")
	}

	tp, ok := pluginapi.Plugin(p).(pluginapi.ToolProvider)
	if !ok {
		t.Fatal("app_tools must implement pluginapi.ToolProvider")
	}
