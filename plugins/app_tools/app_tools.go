...
// Name is the registered name for the device-tools plugin.
...
func newAppTools(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := appToolsConfig{}
...
func init() {
	pluginapi.Register(Name, newAppTools)
}

func (p *appTools) Name() string        { return Name }
...
// their preference.
func (p *appTools) ConfigFields() []pluginapi.ConfigField {
	tools := devicetools.All()
	out := make([]pluginapi.ConfigField, 0, len(tools))
	for _, t := range tools {
...
func (p *appTools) Tools() []pluginapi.ToolDef {
	...
	tools := devicetools.All()
	out := make([]pluginapi.ToolDef, 0, len(tools))
	for _, t := range tools {
		if !p.isEnabled(t) {
			continue
		}
		out = append(out, pluginapi.ToolDef{
			Name:        t.Name,
			Description: t.Description,
...
// detects an app_tools plugin in the pipeline; other pluginapi.ToolProvider
// plugins are unaffected.
func (p *appTools) ToolsForClient(supported map[string]struct{}) []pluginapi.ToolDef {
	if supported == nil {
		return p.Tools()
	}
	tools := devicetools.All()
...
func (p *appTools) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	tool := devicetools.Find(name)
	if tool == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("app_tools: unknown tool %q", name)
	}
	if !p.isEnabled(*tool) {
		return pluginapi.ToolResult{}, fmt.Errorf("app_tools: tool %q is disabled for this profile", name)
	}

	broker := host.DeviceToolBrokerFrom(ctx)
	if broker == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("app_tools: no DeviceToolBroker in context — server not wired")
	}
	if supported := broker.SupportedTools(ctx); supported != nil {
		if _, ok := supported[name]; !ok {
			return pluginapi.ToolResult{}, fmt.Errorf("app_tools: tool %q is not supported by the connected device", name)
		}
	}
	...
	return pluginapi.ToolResult{Output: out}, nil
}
