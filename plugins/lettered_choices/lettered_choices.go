...
// Name is the registered name for the lettered-choices plugin.
const Name = "lettered_choices"
...
// `choice_list` pluginapi.UIFragment via the ContentRenderer pipeline so the
...
// json.Unmarshal to turn it into a choice_list pluginapi.UIFragment. Letters
...
type letteredChoicesConfig struct {
	...
	// `choice_list` pluginapi.UIFragment with letters auto-assigned by index
...
func newLetteredChoices(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := letteredChoicesConfig{
		KeepLastN:  1,
...
func init() {
	pluginapi.Register(Name, newLetteredChoices)
}

func (p *letteredChoices) Name() string        { return Name }
...
func (p *letteredChoices) ConfigFields() []pluginapi.ConfigField {
	return []pluginapi.ConfigField{
		{
			Name:        "keep_last_n",
			Display:     "Keep last N",
			Description: "Number of trailing assistant turns whose choice blocks are kept intact.",
			Type:        pluginapi.ConfigFieldNumber,
			Default:     1,
		},
		{
...
func (p *letteredChoices) TransformHistoryMessage(msg providers.WireMessage, pos pluginapi.HistoryPos) providers.WireMessage {
...
func (p *letteredChoices) StreamingTags() []pluginapi.StreamingTag {
	if p.cfg.OutputMode != lcOutputModeComponent {
		return nil
	}
	return []pluginapi.StreamingTag{{Tag: "choices", Component: "choice_list"}}
}

// --- ContentRenderer ---

// RenderContent finds every choices block in assistant content and
// replaces it with a `choice_list` pluginapi.UIFragment. The text before/after
...
func (p *letteredChoices) RenderContent(parts []pluginapi.ContentPart, role string) []pluginapi.ContentPart {
	if p.cfg.OutputMode != lcOutputModeComponent {
		return parts
	}
	if role != "assistant" {
...
// converts each into a `choice_list` pluginapi.UIFragment. Surrounding prose
...
func (p *letteredChoices) splitChoiceBlocks(text string) []pluginapi.ContentPart {
	...
	for {
		...
		if open < 0 {
			if rest != "" {
				out = append(out, pluginapi.ContentPart{Text: rest})
			}
			return out
		}
		...
		if pre := strings.TrimRight(rest[:open], " \t\n\r"); pre != "" {
			out = append(out, pluginapi.ContentPart{Text: pre})
		}
		...
		if len(items) > 0 {
			...
			if err == nil {
				out = append(out, pluginapi.ContentPart{Fragment: &pluginapi.UIFragment{
			...
			// the block entirely.
			out = append(out, pluginapi.ContentPart{Text: trimmed})
		}
