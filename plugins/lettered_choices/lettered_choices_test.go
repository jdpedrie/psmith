...
func TestLetteredChoices_HistoryTransformer_KeepRecent(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 1, FromHeadSameRole: 0})
...
func TestLetteredChoices_HistoryTransformer_StripOlder(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 3, FromHeadSameRole: 1})
	want := "Body text."
	if got.Content != want {
...
func TestLetteredChoices_HistoryTransformer_AnthropicNeverStrips(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{
		FromHead:         3,
		FromHeadSameRole: 1,
...
func TestLetteredChoices_HistoryTransformer_KeepLastN_2KeepsTwo(t *testing.T) {
	...
	for _, sr := range []int{0, 1} {
		got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 99, FromHeadSameRole: sr})
		if got.Content != msg.Content {
			t.Errorf("FromHeadSameRole=%d should be kept; got %q", sr, got.Content)
		}
	}
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 99, FromHeadSameRole: 2})
	if got.Content != "Body." {
...
func TestLetteredChoices_HistoryTransformer_OlderUserUntouched(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 5, FromHeadSameRole: 5})
	if got.Content != msg.Content {
...
func TestLetteredChoices_HistoryTransformer_HeadUserGetsReminder(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 0, FromHeadSameRole: 0})
...
func TestLetteredChoices_HistoryTransformer_SecondUserNoReminder(t *testing.T) {
	...
	got := lc.TransformHistoryMessage(msg, pluginapi.HistoryPos{FromHead: 2, FromHeadSameRole: 1})
	if got.Content != msg.Content {
...
func TestLetteredChoices_ConfigFieldsCoverConfigShape(t *testing.T) {
	...
	if byName["keep_last_n"].Type != pluginapi.ConfigFieldNumber {
		t.Errorf("keep_last_n type = %q want %q", byName["keep_last_n"].Type, pluginapi.ConfigFieldNumber)
	}
	if byName["system_instruction_override"].Type != pluginapi.ConfigFieldTextarea {
		t.Errorf("system_instruction_override type = %q want %q", byName["system_instruction_override"].Type, pluginapi.ConfigFieldTextarea)
	}
	...
	if byName["output_mode"].Type != pluginapi.ConfigFieldSelect {
		t.Errorf("output_mode type = %q want %q", byName["output_mode"].Type, pluginapi.ConfigFieldSelect)
	}
...
func TestLetteredChoices_RegisteredByDefault(t *testing.T) {
	t.Parallel()
	pl, err := pluginapi.Build(Name, nil)
	if err != nil {
