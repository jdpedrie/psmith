...
// Name is the registered name for the Brave web-search tool plugin.
...
func newBraveSearch(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := braveSearchConfig{
...
func init() {
	pluginapi.Register(Name, newBraveSearch)
}

func (p *braveSearch) Name() string        { return Name }
...
func (p *braveSearch) ConfigFields() []pluginapi.ConfigField {
...
func (p *braveSearch) Tools() []pluginapi.ToolDef {
	schema := []byte(`{
  "type": "object",
  "properties": {
    "query": {
...
func (p *braveSearch) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	if name != "web_search" {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: unknown tool %q", name)
	}
	if p.cfg.APIKey == "" {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: api_key is not configured")
	}

	var in braveSearchInput
	if err := json.Unmarshal(input, &in); err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: parse input: %w", err)
	}
	in.Query = strings.TrimSpace(in.Query)
	if in.Query == "" {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: query is required")
	}
	...
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: build request: %w", err)
	}
	...
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: http call: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: read body: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		...
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: %s — %s", resp.Status, excerpt)
	}

	var raw braveAPIResponse
	if err := json.Unmarshal(body, &raw); err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: decode response: %w", err)
	}
	...
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("brave_search: encode output: %w", err)
	}
	return pluginapi.ToolResult{Output: encoded}, nil
}
