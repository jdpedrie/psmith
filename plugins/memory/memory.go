...
// Name is the registered name for the conversation-memory
...
func newMemory(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	cfg := memoryConfig{
		DefaultCount: 5,
		MaxDistance:  0.6,
	}
...
func init() {
	pluginapi.Register(Name, newMemory)
}

func (p *memory) Name() string        { return Name }
...
func (p *memory) ConfigFields() []pluginapi.ConfigField {
...
func (p *memory) Tools() []pluginapi.ToolDef {
...
func (p *memory) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	if name != memoryToolName {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: unknown tool %q", name)
	}
	var in memoryToolInput
	if len(input) > 0 {
		if err := json.Unmarshal(input, &in); err != nil {
			return pluginapi.ToolResult{}, fmt.Errorf("memory: parse input: %w", err)
		}
	}
	if strings.TrimSpace(in.Query) == "" {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: query is required")
	}

	searcher := host.SearcherFrom(ctx)
	if searcher == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: no Searcher in context — server not wired (set PSMITH_EMBEDDER)")
	}

	// host.CallerInfo tells us who's running this tool. The dispatch site
	// attaches both ProviderResolver and host.CallerInfo; the latter
	// carries the user_id we need to scope the search.
	caller := host.CallerInfoFrom(ctx)
	if caller.UserID == uuid.Nil {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: no host.CallerInfo in context — server not wired")
	}
	...
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: search: %w", err)
	}
	...
	if err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("memory: marshal output: %w", err)
	}
	return pluginapi.ToolResult{Output: body}, nil
}
