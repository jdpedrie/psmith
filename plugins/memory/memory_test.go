...
func TestMemory_Descriptor(t *testing.T) {
	...
	tp, ok := pluginapi.Plugin(pl).(pluginapi.ToolProvider)
	if !ok {
		t.Fatal("memory must implement pluginapi.ToolProvider")
	}
	tools := tp.Tools()
...
func TestMemory_ExecuteTool_HappyPath(t *testing.T) {
	...
	ctx := host.WithSearcher(context.Background(), stub)
	ctx = host.WithCallerInfo(ctx, host.CallerInfo{
...
func TestMemory_ExecuteTool_NoSearcherErrors(t *testing.T) {
	t.Parallel()
	ctx := host.WithCallerInfo(context.Background(),
		host.CallerInfo{UserID: uuid.New(), ConversationID: uuid.New()})
	pl := newMemoryForTest(t)
...
func TestMemory_ExecuteTool_NoCallerInfoErrors(t *testing.T) {
	t.Parallel()
	ctx := host.WithSearcher(context.Background(), &stubSearcher{})
	pl := newMemoryForTest(t)
	_, err := pl.ExecuteTool(ctx, memoryToolName, json.RawMessage(`{"query":"x"}`))
...
func TestMemory_ExecuteTool_EmptyQueryRejected(t *testing.T) {
	t.Parallel()
	ctx := host.WithSearcher(context.Background(), &stubSearcher{})
	ctx = host.WithCallerInfo(ctx, host.CallerInfo{UserID: uuid.New(), ConversationID: uuid.New()})
	pl := newMemoryForTest(t)
