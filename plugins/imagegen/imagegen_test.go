...
// via host.WithProviderResolver to drive imagegen's dispatch.
type fakeResolver struct {
	providerType string
	apiKey       string
	baseURL      string
	pricing      host.ResolvedPricing
}

func (f fakeResolver) ResolveModel(_ context.Context, providerID, modelID string) (host.ResolvedModel, error) {
	if f.apiKey == "" {
		return host.ResolvedModel{}, fmt.Errorf("fake resolver: no key configured")
	}
...
func TestImagegen_DefaultsAndDescriptor(t *testing.T) {
	...
	if _, ok := pl.(pluginapi.ToolProvider); !ok {
		t.Error("must implement pluginapi.ToolProvider")
	}
	c, ok := pl.(pluginapi.Configurable)
	if !ok {
		t.Fatal("must implement pluginapi.Configurable")
	}
	fields := c.ConfigFields()
	// First field is the model picker with the
	// generates_images filter — UIs must see this to render the
	// chooser, so pin it.
	if len(fields) == 0 || fields[0].Name != "model" || fields[0].Type != pluginapi.ConfigFieldModelPicker {
...
func TestImagegen_RejectsMissingModel(t *testing.T) {
	...
	tp := pl.(pluginapi.ToolProvider)
	ctx := host.WithProviderResolver(context.Background(), fakeResolver{providerType: "openai-compatible", apiKey: "k"})
...
func TestImagegen_OpenAIDispatch(t *testing.T) {
	...
	pl, _ := newImagegen(cfg)
	tp := pl.(pluginapi.ToolProvider)
	ctx := host.WithProviderResolver(context.Background(), fakeResolver{
...
func TestImagegen_OpenAICostFromUsage(t *testing.T) {
	...
	ctx := host.WithProviderResolver(context.Background(), fakeResolver{
		providerType: "openai-compatible",
		apiKey:       "sk",
		pricing:      host.ResolvedPricing{InputPerMillion: 5.0, OutputPerMillion: 40.0},
	})
...
func TestImagegen_GoogleCostFromUsageMetadata(t *testing.T) {
	...
	ctx := host.WithProviderResolver(context.Background(), fakeResolver{
		providerType: "google",
		apiKey:       "AIza-test",
		pricing:      host.ResolvedPricing{InputPerMillion: 0.30, OutputPerMillion: 30.0},
	})
	out, err := tp.ExecuteTool(ctx, "generate_image", json.RawMessage(`{"prompt":"x"}`))
	if err != nil {
		t.Fatalf("ExecuteTool: %v", err)
