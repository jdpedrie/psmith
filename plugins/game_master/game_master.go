...
import (
	...
	"github.com/jdpedrie/psmith/plugins/game_master/engine"
)

// Name is the registered plugin name. Stable forever — it is
// a database value in profile_plugins and in plugin_state.plugin_name.
const Name = "game_master"
...
type gameMaster struct {
	...
	lastTransition *engine.Transition
}
...
func newGameMaster(configBytes json.RawMessage) (pluginapi.Plugin, error) {
	// Defaults live here, not only in pluginapi.ConfigField.Default — pluginapi.Describe
...
func init() { pluginapi.Register(Name, newGameMaster) }

func (p *gameMaster) Name() string        { return Name }
...
func (p *gameMaster) ConfigFields() []pluginapi.ConfigField {
...
func (p *gameMaster) AppendSystemMessage() string {
	return strings.Join([]string{
		...
		"the real ones. pluginapi.Describe consequences in words instead: \"the treasury is badly strained\".",
		"",
		...
		"difficulty band (" + strings.Join(engine.DifficultyNames(), ", ") + "), a stakes tier (" +
			strings.Join(engine.StakesNames(), ", ") + "), the stat it advances on success, and the stat it costs.",
...
// --- pluginapi.ToolProvider ---

func (p *gameMaster) Tools() []pluginapi.ToolDef {
	return []pluginapi.ToolDef{
		{
...
func (p *gameMaster) ExecuteTool(ctx context.Context, name string, input json.RawMessage) (pluginapi.ToolResult, error) {
	store := host.PluginStateStoreFrom(ctx)
	if store == nil {
		return pluginapi.ToolResult{}, fmt.Errorf("game_master: no host.PluginStateStore in context — server not wired for stateful plugins")
	}
	switch name {
	case "game_inspect":
	...
	return pluginapi.ToolResult{}, fmt.Errorf("game_master: unknown tool %q", name)
}
...
type commitTurnInput struct {
	...
	Next                 *engine.SituationDraft `json:"next_situation,omitempty"`
	...
	StartClocks   []engine.ClockDraft `json:"start_clocks,omitempty"`
	...
	// choice_id.
	Improvised *engine.ChoiceDraft `json:"improvised_action,omitempty"`
}

func (p *gameMaster) commitTurn(ctx context.Context, store host.PluginStateStore, raw json.RawMessage) (pluginapi.ToolResult, error) {
	var in commitTurnInput
	if err := json.Unmarshal(raw, &in); err != nil {
		return pluginapi.ToolResult{}, fmt.Errorf("game_master: parse input: %w", err)
	}
	...
	p.mu.Unlock()
	if alreadyCommitted {
		return pluginapi.ToolResult{}, fmt.Errorf(
	...
	case "initialize":
		...
		if exists && current.Protocol.Phase != engine.PhaseUninitialized {
			return pluginapi.ToolResult{}, fmt.Errorf(
				"game_master: this campaign is already underway (turn %d) — use kind=\"resolve\"", current.Meta.Turn)
		}
		if in.Scenario == nil {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: initialize requires a scenario")
		}
		seed, err := seedFor(ctx)
		if err != nil {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: %w", err)
		}
		next, err := engine.Initialize(*in.Scenario, seed)
		if err != nil {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: %w", err)
		}
	...
	case "resolve":
		if !exists || current.Protocol.Phase == engine.PhaseUninitialized {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: no campaign yet — call kind=\"initialize\" first")
		}
		if current.Protocol.Phase == engine.PhaseFinished {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: this campaign ended on turn %d", current.Meta.Turn)
		}
		...
		if in.Improvised != nil {
			if in.ChoiceID != "" {
				return pluginapi.ToolResult{}, fmt.Errorf("game_master: pass either choice_id or improvised_action, not both")
			}
			withImprov, err := engine.AttachImprovised(current, *in.Improvised)
			if err != nil {
				return pluginapi.ToolResult{}, fmt.Errorf("game_master: %w", err)
			}
			resolveAgainst, choiceID = withImprov, engine.ImprovisedChoiceID
		}
		tr, next, err := engine.Resolve(resolveAgainst, choiceID)
		if err != nil {
			return pluginapi.ToolResult{}, fmt.Errorf("game_master: %w", err)
		}
		// A finished campaign needs no next situation; otherwise one is
		// required or the player would have nothing to do.
		if next.Protocol.Phase != engine.PhaseFinished {
			if in.Next == nil {
				return pluginapi.ToolResult{}, fmt.Errorf("game_master: resolve requires next_situation unless the campaign ended")
			}
			sit, err := engine.PriceSituation(next, *in.Next)
			if err != nil {
				return pluginapi.ToolResult{}, fmt.Errorf("game_master: next_situation: %w", err)
			}
	...
	return pluginapi.ToolResult{}, fmt.Errorf("game_master: kind must be \"initialize\" or \"resolve\", got %q", in.Kind)
}
...
func (p *gameMaster) priceAction(ctx context.Context, store host.PluginStateStore, raw json.RawMessage) (pluginapi.ToolResult, error) {
	var in struct {
	...
	if st.Protocol.Phase == engine.PhaseFinished {
		return pluginapi.ToolResult{}, fmt.Errorf("game_master: this campaign has ended")
	}

	quote, err := engine.PriceImprovised(st, engine.ChoiceDraft{
		ID:         engine.ImprovisedChoiceID,
		Label:      in.Summary,
		Rating:     in.Rating,
...
func applyClockChanges(st *engine.State, in commitTurnInput) error {
	...
	for _, d := range in.StartClocks {
		...
		c, err := engine.PriceClock(d, stats)
		if err != nil {
			return err
		}
...
func (p *gameMaster) loadState(ctx context.Context, store host.PluginStateStore) (engine.State, bool, error) {
	st, exists, err := p.loadStateOptional(ctx, store)
	if err != nil {
		return engine.State{}, false, err
	}
	if !exists {
		return engine.State{}, false, fmt.Errorf("game_master: no campaign has been started in this conversation")
	}
	return st, true, nil
}

func (p *gameMaster) loadStateOptional(ctx context.Context, store host.PluginStateStore) (engine.State, bool, error) {
	...
	if err != nil {
		if err == host.ErrNoPluginState {
			return engine.State{}, false, nil
		}
		return engine.State{}, false, fmt.Errorf("game_master: load state: %w", err)
	}
	var st engine.State
	if err := json.Unmarshal(raw, &st); err != nil {
		return engine.State{}, false, fmt.Errorf("game_master: stored state is unreadable: %w", err)
	}
	if err := st.Validate(); err != nil {
		return engine.State{}, false, fmt.Errorf("game_master: %w", err)
	}
	return st, true, nil
}

func (p *gameMaster) setPending(st *engine.State, tr *engine.Transition) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.pending = st
	p.lastTransition = tr
...
func (p *gameMaster) BuildTurnContext(ctx context.Context, turn pluginapi.TurnInfo) (string, error) {
	store := host.PluginStateStoreFrom(ctx)
	if store == nil {
		return "", nil
	}
...
func (p *gameMaster) StreamingTags() []pluginapi.StreamingTag {
...
func (p *gameMaster) RenderContent(parts []pluginapi.ContentPart, role string) []pluginapi.ContentPart {
	if role != "assistant" {
		return parts
	}
	return pluginapi.WalkText(parts, func(text string) []pluginapi.ContentPart {
		// pluginapi.WalkText splices the return value in place of the part, so
		...
		unchanged := []pluginapi.ContentPart{pluginapi.NewTextPart(text)}
		...
		var out []pluginapi.ContentPart
		if prose := strings.TrimRight(text[:start], "\n"); prose != "" {
			out = append(out, pluginapi.NewTextPart(prose))
		}
		if len(block.Stats) > 0 {
			...
			if props, err := json.Marshal(map[string]any{"pairs": pairs}); err == nil {
				out = append(out, pluginapi.NewTextPart("**"+title+"**"))
				out = append(out, pluginapi.NewFragmentPart("key_value", props, "game_status"))
			}
		}
		if len(block.Resolution) > 0 {
			out = append(out, pluginapi.NewTextPart("_"+strings.Join(block.Resolution, " ")+"_"))
		}
		if len(block.Choices) > 0 {
			...
			if props, err := json.Marshal(map[string]any{"items": items}); err == nil {
				out = append(out, pluginapi.NewFragmentPart("choice_list", props, "game_choices"))
			}
		}
		if block.Ending != "" {
			verdict := "Defeat"
			...
			out = append(out, pluginapi.NewTextPart(fmt.Sprintf("**%s — %s**", verdict, block.Ending)))
		}
		if tail := strings.TrimLeft(text[bodyEnd+len(gameCloseTag):], "\n"); tail != "" {
			out = append(out, pluginapi.NewTextPart(tail))
		}
		return out
	})
}
...
func seedFor(ctx context.Context) (uint64, error) {
	...
	if info.ConversationID == uuid.Nil {
		...
		// same contract.
		return 0, fmt.Errorf("no host.CallerInfo in context — cannot seed a campaign")
	}
...
func jsonResult(v any) (pluginapi.ToolResult, error) {
	blob, err := json.Marshal(v)
	if err != nil {
		return pluginapi.ToolResult{}, err
	}
	return pluginapi.ToolResult{Output: blob}, nil
}
