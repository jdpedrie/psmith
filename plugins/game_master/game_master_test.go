...
func gameCtx(store host.PluginStateStore) context.Context {
	ctx := host.WithPluginStateStore(context.Background(), store)
	return host.WithCallerInfo(ctx, host.CallerInfo{
		UserID:          uuid.New(),
...
func TestGameMaster_Descriptor(t *testing.T) {
	...
	tp, ok := pluginapi.Plugin(g).(pluginapi.ToolProvider)
	if !ok {
		t.Fatal("must implement pluginapi.ToolProvider")
	}
	tools := tp.Tools()
	got := map[string]bool{}
	...
	desc, err := pluginapi.Describe(Name)
	if err != nil {
...
// TestGameMaster_NilConfigIsUsable matters because pluginapi.Describe builds
...
func TestGameMaster_RequiresStore(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "no host.PluginStateStore") {
...
func TestGameMaster_RendersNativeComponents(t *testing.T) {
	...
	parts := g.RenderContent([]pluginapi.ContentPart{pluginapi.NewTextPart(content)}, "assistant")
	...
	userParts := g.RenderContent([]pluginapi.ContentPart{pluginapi.NewTextPart(content)}, "user")
...
// mode: pluginapi.WalkText splices the callback's return in place of the part, so
...
func TestGameMaster_PlainProseSurvivesRendering(t *testing.T) {
	...
	parts := g.RenderContent([]pluginapi.ContentPart{pluginapi.NewTextPart(prose)}, "assistant")
...
func TestGameMaster_MalformedBlockSurvivesAsText(t *testing.T) {
	...
	parts := g.RenderContent([]pluginapi.ContentPart{pluginapi.NewTextPart(raw)}, "assistant")
...
func TestPipeline_CollectsPendingState(t *testing.T) {
	...
	var quiet pluginapi.Plugin = silentPlugin{}

	pending := pluginapi.Pipeline{quiet, idle, played}.PendingPluginStates()
	if len(pending) != 1 {
		t.Fatalf("only the plugin that played should report state; got %d", len(pending))
	}
	if pending[0].PluginName != Name {
...
// contract. The seed previously fell back to a constant when host.CallerInfo
...
func TestGameMaster_RefusesToSeedWithoutCallerInfo(t *testing.T) {
	t.Parallel()
	g := newGameForTest(t, "")
	// Store wired, host.CallerInfo deliberately absent.
	ctx := host.WithPluginStateStore(context.Background(), &stubStateStore{})
...
func TestGameMaster_SeedVariesByConversation(t *testing.T) {
	...
	for i := 0; i < 64; i++ {
		ctx := host.WithCallerInfo(context.Background(), host.CallerInfo{ConversationID: uuid.New()})
		s, err := seedFor(ctx)
		if err != nil {
			t.Fatalf("seed: %v", err)
		}
...
// silentPlugin stands in for any plugin that carries no state. It exists to
// prove PendingPluginStates reports only the plugins that actually played,
// which needs at least one participant that did not.
